...
extern void save_yuv_file(char *buf, UINT buf_len);
void D3D9RenderImpl::ConvertFormat(BYTE* pPlanes[4], int pitches[4], D3DLOCKED_RECT & stRectDest)
{
	...
	//if (g_start_getdata && g_net_buf != NULL)
	if (0)
	{
		int nScaleSrcSzie = (m_videoWidth * m_videoHeight * 3) >> 1;
		BYTE *m_pScaleSrc = NULL;
		...
		m_pScaleSrc = new BYTE[nScaleSrcSzie];
		...
		{ 
			...
			//memcpy(g_net_buf, m_pScaleSrc, nScaleSrcSzie);

			save_yuv_file((char *)m_pScaleSrc, nScaleSrcSzie);
			delete[] m_pScaleSrc;
			m_pScaleSrc = NULL;
			g_haved_getdata = TRUE;
			//Sleep(1000);

		}
	}

	g_video_width = m_videoWidth;
	g_video_height = m_videoHeight;
