...
//extern char g_video_buf[MAX_VIDEO_LEN];
extern char *g_video_buf;
extern UINT g_video_buf_len;
extern BOOL g_start_record_flag;
...
int CFfmpegEnvoy::Decode_video(RAWDATA * data)
{
	...
	//do {
		...
		//if(g_I_frame_flag && g_start_record_flag && avpkt.size > 0)
		if(g_start_record_flag && avpkt.size > 0 )
		{
			if (g_file_fp.m_hFile != INVALID_HANDLE_VALUE)
			{
				g_file_fp.Write(avpkt.data,avpkt.size);
			}
			
			//memset(g_video_buf, 0, MAX_VIDEO_LEN);
			//memcpy(g_video_buf,avpkt.data,avpkt.size);
			//g_video_buf_len = avpkt.size;
		}
