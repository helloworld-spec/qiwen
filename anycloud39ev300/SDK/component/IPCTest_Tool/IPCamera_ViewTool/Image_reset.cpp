...
BEGIN_MESSAGE_MAP(CImage_reset, CDialog)
	...
	ON_EN_CHANGE(IDC_EDIT_Y, &CImage_reset::OnEnChangeEditY)
END_MESSAGE_MAP()
...
void CImage_reset::OnBnClickedButtonComeback()
{
	...
	if (!pP->Send_cmd(TEST_COMMAND, 0, file_name, NULL, 0, 0))
	{
...
void CImage_reset::OnBnClickedButtonCorrectImage()
{
	...
	if (g_test_config.image_x%16 != 0 || g_test_config.image_y%8 != 0)
	{
		AfxMessageBox(_T("图像宽必需为16的倍数，高必需为8的倍数，请重输"), MB_OK);
		g_cut_out_flag = FALSE;
		return;
	}
	...
	if (!pP->Send_cmd(TEST_COMMAND, 0, file_name, test_param, strlen(test_param), 0))
	{
...
void CImage_reset::OnEnChangeEditY()
{
	// TODO:  如果该控件是 RICHEDIT 控件，则它将不会
	// 发送该通知，除非重写 CDialog::OnInitDialog()
