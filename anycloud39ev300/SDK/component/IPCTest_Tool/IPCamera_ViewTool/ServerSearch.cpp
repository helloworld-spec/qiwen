...
extern BOOL g_finish_find_flag;
...
extern UINT g_finish_idex ;
extern char g_test_pass_flag[UPDATE_MAX_NUM];
	...
	for (i = 0; i < buf_size; i++)
	{
		if (buf[i] == 0x40)
		{
			...
			if (idex == 0) //（获取通道名）获取UID
			{
				...
				//if (!g_finish_flag)
				//{
				//	memset(g_test_config.m_current_config[m_ip_address_idex].Current_IP_UID, 0, MAC_ADDRESS_LEN);
			...
			else if(idex == 1) //ip
			{
				...
				if (!g_finish_flag)
				{
					memset(g_test_config.m_current_config[m_ip_address_idex].Current_IP_address_buffer, 0, IP_ADDRE_LEN);
			...
			else if (idex == 4)//（DNS1）
			{
				...
				//memset(g_test_config.m_current_config[m_ip_address_idex].Current_IP_MAC, 0, MAC_ADDRESS_LEN);
			...
			else if (idex == 5)//（DNS2）
			{
				...
				//memset(g_test_config.m_current_config[m_ip_address_idex].Current_IP_version, 0, MAC_ADDRESS_LEN);
			...
			else if (idex == 7)//设备名
			{
				...
				{
					UTF8StrToUnicode(temp_buf, strlen(temp_buf), buffer, MAX_PATH);
					memset(g_test_config.m_current_config[m_ip_address_idex].Current_IP_diver_name, 0, MAX_PATH);
			...
			else if (idex == 9)//MAC
			{
				...
				if (g_finish_flag)
				{
					memset(g_test_config.m_finish_update_config[g_finish_idex].Current_IP_MAC, 0, MAC_ADDRESS_LEN);
					_tcsncpy(g_test_config.m_finish_update_config[g_finish_idex].Current_IP_MAC,  A2T(temp_buf), len);
					g_finish_idex++;
				}
				else
				{
					...
					memset(g_test_config.m_before_update_config[m_ip_address_idex].Current_IP_MAC, 0, MAC_ADDRESS_LEN);
					_tcsncpy(g_test_config.m_before_update_config[m_ip_address_idex].Current_IP_MAC,  A2T(temp_buf), len);
...
int CServerSearch::reset_IPaddr()
{
#if 1
	if (!g_finish_flag && m_ip_address_idex > 0)
	{
		...
		else
		{
			...
			for (i = 0; i < src_ip_address_idex; i++)
			{
				for (j = 0; j < m_ip_address_idex; j++)
				{
					...
					{
						memcpy(&m_temp_config[last_idex], &g_test_config.m_last_config[i], sizeof(T_CURRENT_IP_CONFIG));
						last_idex++;
						break;
					}
				}
...
{
	UINT i =0,j=0, len = 0, n =0, idex;
	CServerSearch search;

	g_finish_idex = 0;

	for (idex = 0; idex < MAX_PATH; idex++)
	{
		memset(g_test_config.m_finish_update_config[i].Current_IP_MAC, 0, MAC_ADDRESS_LEN);
	}
	...
	if (g_finish_flag && g_finish_idex > 0)
	{
		//判断与查找前记录的MAC地址是否相同
		for (i = 0; i < g_finish_idex; i++)
		{

			for (j = 0; j < m_ip_address_idex; j++)
			{
				if(g_test_pass_flag[j] != 1)
				{
					len = _tcslen(g_test_config.m_finish_update_config[i].Current_IP_MAC);
					if (len > 0 && (len == _tcslen(g_test_config.m_before_update_config[j].Current_IP_MAC)))
					{
						for(n = 0; n < len; n++)
						{
							if (g_test_config.m_finish_update_config[i].Current_IP_MAC[n] != g_test_config.m_before_update_config[j].Current_IP_MAC[n])
							{
								break;
							}
						}
						if(n == len)
						{
							g_test_pass_flag[j] = 1;
							//memset(g_test_config.m_finish_update_config[i].Current_IP_MAC, 0, MAC_ADDRESS_LEN*sizeof(TCHAR));
							break;
						}
					}
				}
