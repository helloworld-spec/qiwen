...
void CRecordPlayDlg::OnBnClickedButtonGetFiles()
{
	...
	if (m_pClient) {
		int iRet = AfxMessageBox(L"当前正在回放，执行重获取操作会强制停止回放，继续吗？(Y/N)", MB_YESNO);
		if (iRet == IDNO) return;
	}
	...
	for (; iter != m_ftpDownloadThread.end(); ++iter) {
		...
		if (((*iter)->GetDownloadLen() < (*iter)->GetFtpFileLen()) && ((*iter)->IsStart())) {
			int iRet = AfxMessageBox(L"当前还有下载任务未完成，是需要强制停止下载？(Y/N)", MB_YESNO);
			if (iRet == IDNO) return;
			else break;
		}
	}
	...
	if (m_pIServer->SendGetFiles() < 0) {
		AfxMessageBox(L"发送获取录像文件的命令失败！可能已经和服务器断开连接，请退回预览主界面重新与此服务器进行连接。");
		return;
	}
}
...
void CRecordPlayDlg::OnNMDblclkList1(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	*pResult = 0;
	...
	m_bIsPlay = TRUE;
	GetDlgItem(IDC_BUTTON_PLAY)->SetWindowText(L"暂停");
	m_dCurrentDuration = 0.0;
	m_dLastSendDuration = 0.0;
}
...
void CRecordPlayDlg::OnBnClickedButtonPlay()
{
	...
	if (m_bIsPlay) {
		...
		m_pClient->PausePlay();
		m_bIsPlay = FALSE;
		GetDlgItem(IDC_BUTTON_PLAY)->SetWindowText(L"播放");
	}else {
...
void CRecordPlayDlg::OnClose()
{
	...
	for (; iter != m_ftpDownloadThread.end(); ++iter) {
		...
		if (((*iter)->GetDownloadLen() < (*iter)->GetFtpFileLen()) && ((*iter)->IsStart())) {
			int iRet = AfxMessageBox(L"当前还有下载任务未完成，是需要强制退出吗？(Y/N)", MB_YESNO);
			if (iRet == IDNO) return;
			else break;
		}
	}
...
void CRecordPlayDlg::OnBnClickedButtonDownload()
{
	...
	if (m_nCurrentSelect < 0) {
		AfxMessageBox(L"请在文件列表中选者一个文件进行下载！");
		return;
	}

	if (m_strDownloadDir.GetLength() == 0) {
		AfxMessageBox(L"无法创建下载文件夹！请确保D盘下是否有AnykaIPC目录。不进行下载操作");
		return;	
	}
	...
	if (iter != m_ftpDownloadThread.end()) {
		if ((*iter)->GetLastErrorCode() < 0) {
			...
			AfxMessageBox(L"当前文件正在下载中");
			return;
		}else if (((*iter)->GetDownloadLen() == (*iter)->GetFtpFileLen()) && ((*iter)->GetDownloadLen())) {
			int iRet = AfxMessageBox(L"当前文件已经下载完成，重新下载吗？(Y/N)", MB_YESNO);
			if (iRet == IDNO) return;
			else {
				(*iter)->Stop();
				delete *iter;
	...
	if (iter == m_ftpDownloadThread.end()) {
		if (m_ftpDownloadThread.size() >= MAX_DOWNLOAD_THREAD) {
			AfxMessageBox(L"当前正在下载的线程已满，请等待下载完成后再进行新的下载！");
			return;
		}
	...
	if (iRet < 0) {
		...
		strError.Format(L"下载文件%s错误:%s", strFileName, strErrMsg.c_str());
	...
	if (nIDEvent == 1) {
		...
		for (; iter != m_ftpDownloadThread.end(); ) {
			...
			if (nErrorCode < 0) {
				...
				strErrMsg.Format(L"下载文件错误:%s", A2W(strError.c_str()));
