...
HRESULT XA2Player::Initialize(const WAVEFORMATEX* format, int index, bool isMusicPlayback)
{
	if (NULL == m_xaudio2 && m_ds == NULL) {
		AfxMessageBox(L"音频无法播放，此问题的产生是由于您pc上的DX版本过低，或声卡驱动不支持DX的新特性!", 0, 0);
		return E_FAIL;
	}
	...
	if (m_ds) { //使用的是DirectSound
		...
		if (!hWnd) {
			AfxMessageBox(L"音频无法播放，获取不到协调窗口给DirectSound使用!", 0, 0);
			return E_FAIL;
		}

		if (!((CStreamAudio*)m_ds)->Open(hWnd, (CAudioStreamHandler*)this, format)) {
			AfxMessageBox(L"音频无法播放，无法打开DirectSound!", 0, 0);
			return E_FAIL;
		}
