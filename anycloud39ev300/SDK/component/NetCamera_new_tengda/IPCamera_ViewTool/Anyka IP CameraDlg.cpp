...
#define  TOOL_VERSOIN _T("生产测试工具_v2.0.10.03")
...
BOOL CAnykaIPCameraDlg::Anyka_Test_check_info(void)
{
	...
	while (1)
	{
		...
		{
			g_test_pass_flag = 0;
			AfxMessageBox(_T("超时(10s)没有返回确认命令"));
			return FALSE;
		}
...
void CAnykaIPCameraDlg::OnBnClickedButtonStart()
{
	...
	m_RightDlg.GetDlgItem(IDC_BUTTON_PASS)->SetWindowText(_T("通过(Y/y)"));//可用 
...
void CAnykaIPCameraDlg::OnBnClickedButtonWriteUid()
{
	...
	if(IDOK == m_Burn_UIdDlg.DoModal() && m_Burn_UIdDlg.UID_PASS_flag)
	{
		...
		frmLogUidFile.WriteLogFile(0,"UID烧录成功,UID:%s\r\n", T2A(burnUid));
	}
...
BOOL CAnykaIPCameraDlg::case_monitor()
{
	...
	if (str.IsEmpty())
	{
		...
		//m_RightDlg.GetDlgItem(IDC_BUTTON_FAILED)->EnableWindow(TRUE);//可用
		g_send_commad = 0;
		str.Format(_T("监听测试, 时长为空，请填写测试需要的时长！"));
		AfxMessageBox(str, MB_OK); 
		return FALSE;
	}
	...
	while (1)
	{
		...
		if (time2 - time1 > 10000)
		{
			...
			AfxMessageBox(_T("获取文件超时退出"), MB_OK);  
			g_send_commad = 0;
			return FALSE;
		}
	...
	if(0xFFFFFFFF == faConfig)
	{
		...
		AfxMessageBox(_T("没有获取小机上的录音文件 "), MB_OK); 
		g_send_commad = 0;
		return FALSE;
	}

	faConfig &= ~FILE_ATTRIBUTE_HIDDEN;  //如果文件是系统，那么设非系统
	...
	if (!play_pcm_test(test_pcm_name))
	{
		...
		AfxMessageBox(_T("播放pcm文件失败"), MB_OK);
		g_send_commad = 0;
		return FALSE;
	}
...
void CAnykaIPCameraDlg::case_interphone()
{
	...
	if (m_pFtpConnection == NULL || !m_pFtpConnection->PutFile(strSourceName,
		strDestName, FTP_TRANSFER_TYPE_BINARY, 1))   
	{
		AfxMessageBox(_T("对讲测试 PutFile fail"), MB_OK);  
		download_dev_file_flag = FALSE;
		g_send_commad = 0;
		return;
	}
...
void CAnykaIPCameraDlg::case_head()
{
	...
	if (m_pFtpConnection == NULL || !m_pFtpConnection->PutFile(strSourceName,
		strDestName, FTP_TRANSFER_TYPE_BINARY, 1))   
	{
		AfxMessageBox(_T("云台测试 PutFile fail"), MB_OK);  
		download_dev_file_flag = FALSE;
		g_send_commad = 0;
		return;
	}
...
void CAnykaIPCameraDlg::case_sd()
{
	...
	if (!Send_cmd(TEST_COMMAND, 1, name_buf, NULL))
	{
		...
		AfxMessageBox(_T("sd测试 Send_cmd fail "), MB_OK); 
		g_send_commad = 0;
		return ;
	}
	start_test = FALSE;
	...
	m_RightDlg.GetDlgItem(IDC_STATIC_TEST_POINT1)->SetWindowText(_T("sd卡测试成功"));
	m_RightDlg.GetDlgItem(IDC_STATIC_TEST_POINT1)->ShowWindow(SW_SHOW);
	start_test = TRUE;
	m_RightDlg.GetDlgItem(IDC_BUTTON_PASS)->EnableWindow(TRUE);//可用 
	m_RightDlg.GetDlgItem(IDC_BUTTON_FAILED)->EnableWindow(TRUE);//可用
...
BOOL CAnykaIPCameraDlg::Check_Uid()
{
	...
	for(i=0;i<len/36;i++)
	{
		if(buff[i*36]==serial[0] && buff[i*36+2]==serial[2] && buff[i*36+4]==serial[4] && buff[i*36+6]==serial[6] && 
		 ...
		 {
			 AfxMessageBox(_T("UID已烧录过了,请检查！"),MB_OK);
			 fp.Close();
			 return FALSE;
		 }
	} 
	...
	while(read_len > 0)
	{
		...
		{
			CloseHandle(hFile);
			MessageBox(_T("UID已烧录过了,请检查！"),MB_OK);
			return FALSE;
		}
...
BOOL CAnykaIPCameraDlg::case_uid()
{
	...
	if (!Anyka_Test_check_info())
	{
		...
		//frmLogUidFile.WriteLogFile(0,"UID烧录失败,UID:%s\r\n", Uid);
		return FALSE;
	}
...
void  CAnykaIPCameraDlg::ChangeTestItem(int case_idex)
{
	...
	case CASE_UID:
		if (g_test_config.config_uid_enable)
		{
			...
			m_RightDlg.GetDlgItem(IDC_BUTTON_PASS)->SetWindowText(_T("烧录UID"));//可用 
			m_RightDlg.GetDlgItem(IDC_BUTTON_FAILED)->SetWindowText(_T("不烧录UID"));//可用
	...
	case CASE_MAC:
		if (g_test_config.config_lan_mac_enable)
		{
			...
			m_RightDlg.GetDlgItem(IDC_BUTTON_PASS)->SetWindowText(_T("烧录MAC"));//可用 
			m_RightDlg.GetDlgItem(IDC_BUTTON_FAILED)->SetWindowText(_T("不烧录MAC"));//可用 
	...
	else if (case_idex == CASE_UID)
	{
		if(g_test_config.config_uid_enable)
		{
			if (pass_flag)
			{
				...
				/*CString str,struid;
				str.Format(_T("UID烧录成功,UID:%s\r\n"),g_test_config.m_uid_number);
				struid.Format(_T("UID烧录成功,UID:%s\r\n"),burnUid);*/
				USES_CONVERSION;
				frmLogfile.WriteLogFile(0,"UID烧录成功,UID:%s\r\n", T2A(burnUid));
				frmLogUidFile.WriteLogFile(0,"UID烧录成功,UID:%s\r\n", T2A(burnUid));
			...
			{
				g_test_config.config_uid_test_pass = FALSE;
				frmLogfile.WriteLogFile(0,"不烧录UID\r\n");
				frmLogUidFile.WriteLogFile(0,"不烧录UID\r\n");
			}
		}
	}
