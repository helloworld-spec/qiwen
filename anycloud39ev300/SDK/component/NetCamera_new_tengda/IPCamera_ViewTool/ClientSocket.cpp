...
int g_hSocket_heat= INVALID_SOCKET;
...
BOOL CClientSocket::Heat_Socket_Create(UINT idex)  
{
	...
	// m_hSocket是类内Socket对象，创建一个基于TCP/IP的Socket变量，并将值赋给该变量； 
	if(g_hSocket_heat != INVALID_SOCKET)
	{
		closesocket(g_hSocket_heat);
		g_hSocket_heat = INVALID_SOCKET;

	}

	if ((g_hSocket_heat = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP)) == INVALID_SOCKET) 
	{ 
		return FALSE; 
	} 
...
{ 
	if (closesocket(g_hSocket_heat) == SOCKET_ERROR) 	
	{  
		return FALSE; 
	} 
	g_hSocket_heat = INVALID_SOCKET;
...
int CClientSocket::Heat_Socket_Connect(char *lIPAddress, unsigned int iPort, UINT idex)//定义连接函数； 
{
	...
	if(connect(g_hSocket_heat, (SOCKADDR*)&m_sockaddr, sizeof(m_sockaddr)) == SOCKET_ERROR ) 	
	{  
		return FALSE; 
...
{  
	...
	if( send(g_hSocket_heat, strData, iLen, 0) == SOCKET_ERROR ) 
	{  
		return FALSE; 
	}  
...
{  
	...
	ret = recv(g_hSocket_heat, strData, iLen, 0); 
	return ret; 
}  