...
VOID  CLogUidFile::SetFileName(TCHAR *name)
{
    if ( (name[1] == ':') && (name[2] == '\\') )   
    {
...
DWORD CLogUidFile::WriteLogFile(UCHAR wFlag, const char *fmt, ...)
{
    va_list ap;
    char    pbuf[300];
    UINT    buf_size;
...
BOOL CLogUidFile::GetCurPCPath(PTCHAR curPCPath, int buf_len)
{
    UINT len;
    CString sPath;
...
BOOL  CLogUidFile::CheckFileSize(DWORD file_max_size)
{
