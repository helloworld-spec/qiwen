...
struct dana_airlink_handler {
    ...
    struct dana_mutex_handler *dana_mutex; //为什么非要加*
};
...
dana_airlink_handler_t *dana_airlink_create(const char *if_name)
{
    ...
    dana_airlink->dana_mutex = dana_mutex_create();
    if (NULL == dana_airlink->dana_mutex) {
      dbg("dana_airlink_create dana_mutex_create failed\n");
...
{  
    dana_mutex_lock(dana_airlink->dana_mutex);   
    int      ret=0;
 
    dana_airlink->fd = socket(SL_AF_RF, SOCK_RAW, dana_airlink->work_channel);
    Report("HITHIT0: %d\n", dana_airlink->fd);
    //dana_airlink->fd = socket(SL_AF_RF, SOCK_RAW, 0); 
    if (0 > dana_airlink->fd) {
        dana_mutex_unlock(dana_airlink->dana_mutex);
        return -1; 
    }
    ...
    ret = sl_SetSockOpt(dana_airlink->fd ,SL_SOL_SOCKET,SL_SO_RCVTIMEO, &timeval, \
                                  sizeof(timeval));
    if(ret < 0){
      dana_mutex_unlock(dana_airlink->dana_mutex);
...
int32_t dana_airlink_recv(dana_airlink_handler_t *dana_airlink, char *recv_buf, int32_t size)
{
  dana_mutex_lock(dana_airlink->dana_mutex);
  Report("HITHIT1: %d\n", dana_airlink->fd);
  int32_t ret = recv(dana_airlink->fd, recv_buf, size, 0);
  dana_mutex_unlock(dana_airlink->dana_mutex);
  return ret;
}
...
bool dana_airlink_setchannel(dana_airlink_handler_t *dana_airlink, int32_t channel)
{
  //dbg("set success %d\r\n",channel);
    ...
    int ret = sl_Close(dana_airlink->fd);
    int fd = sl_Socket(SL_AF_RF, SL_SOCK_RAW, channel);
    if (fd < 0) {
