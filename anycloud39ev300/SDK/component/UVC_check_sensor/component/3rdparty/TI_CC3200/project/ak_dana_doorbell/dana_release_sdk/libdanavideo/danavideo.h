...
typedef enum _dana_video_pushmsg_msg_type_e {
    ...
    DANA_VIDEO_PUSHMSG_MSG_TYPE_DOOR_BELL       = 15,   // 门铃被触发
...
bool lib_danavideo_util_pushmsg_v2(const uint32_t ch_no, const uint32_t alarm_level, const uint32_t msg_type, const char *msg_title, const char *msg_body, const int64_t cur_time, const uint32_t att_flag, const char *att_path, const char *att_type, const uint32_t record_flag, const int64_t start_time, const uint32_t time_len, const uint32_t save_site, const char *save_path, const char *sensor_id);
