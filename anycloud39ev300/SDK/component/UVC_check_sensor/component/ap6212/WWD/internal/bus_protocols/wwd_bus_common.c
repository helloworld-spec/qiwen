...
static wwd_result_t download_resource( wwd_resource_t resource, uint32_t address )
{
	...
	WPRINT_WWD_ERROR(("download_resource size = %d\n", image_size));
	image_out = malloc(1024);
	if(image_out == NULL)
	{
		WPRINT_WWD_ERROR(("malloc error\n"));
	}
    ...
    for ( transfer_progress = 0; transfer_progress < image_size; transfer_progress += transfer_size, address += transfer_size, image += transfer_size )
    {
		...
		WPRINT_WWD_ERROR(("transfer size = %d\n", transfer_size));
		...
		//wmj+ to check read image 
		#if 1
		int i;
		
		wwd_bus_transfer_bytes(BUS_READ, BACKPLANE_FUNCTION, address & BACKPLANE_ADDRESS_MASK, 1024, image_out);
