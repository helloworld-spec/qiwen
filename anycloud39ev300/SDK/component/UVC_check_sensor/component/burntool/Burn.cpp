...
extern T_DOWNLOAD_NAND g_download_nand[MAX_DOWNLOAD_MTD];//nand闪存的结构体

extern UINT g_download_udisk_count;// 下载u盘的文件个数
extern T_DOWNLOAD_UDISK g_download_udisk[MAX_DOWNLOAD_FILES];
...
BOOL Burn_check_size(UINT nID, HWND hWnd, UINT data_size, char *fileName, char *is_noexit)
{
	UINT j = 0;
	...
	*is_noexit = 0;
	...
	{
		*is_noexit = 1;
		PostMessage(hWnd, ON_BURNFLASH_MESSAGE, WPARAM(nID+100), LPARAM(MESSAGE_GETPARTITION_NAME_FAIL));
		return FALSE;
	}
	
	if (theConfig.partition_data[j].Size == 0)
	{
		PostMessage(hWnd, ON_BURNFLASH_MESSAGE, WPARAM(nID+100), LPARAM(MESSAGE_GET_PARTITION_SIZE_FAIL));
		return FALSE;
	}
					 ...
					 PTCHAR maclowbuf, PTCHAR mactempbuf, BOOL *readmacflag, UINT  *macmode)
{
	...
	if (theConfig.chip_type == CHIP_10XX_T)
	{
		return TRUE;
	}
	...
	if(0)//theConfig.burn_mode != E_CONFIG_SFLASH)//非spi烧录的升级都不支持mac和序列号
	{
	...
	if(theConfig.macaddr_flag) 
	{
		...
		else
		{
			*macmode = ASA_MODE_OPEN;
			...
			if ((BT_ReadASAFile(nID, "MAC", buf, &maclen)  == BT_SUCCESS) && (maclen != 0))
			{
				*readmacflag = TRUE;
	...
	if(theConfig.sequenceaddr_flag && nID == 1) 
	{
		...
		else
		{
			*Serialmode = FALSE;
			//if不是强制写入
			burnFile.WriteLogFile(LOG_LINE_TIME,  "++read asa file++\r\n" );
			if ((BT_ReadASAFile(nID, "SEQ", buf, &Seriallen)  == BT_SUCCESS) && (Seriallen != 0))
			{
				*readSerialflag = TRUE;
...
//写环境变量到安全区
BOOL Burn_Write_NEV(UINT nID, HWND hWnd)
{
	CHAR buf[1024] = {0};
	BYTE tbuf[1024];
	UINT  macmode = ASA_MODE_CREATE;
	...
	if(!theConfig.bUpdate && (theConfig.chip_type == CHIP_39XX_H2 || theConfig.chip_type == CHIP_39XX_H240))
	{
		char is_noexit = 0;
		...
		PostMessage(hWnd, ON_BURNFLASH_MESSAGE, WPARAM(nID+100), LPARAM(MESSAGE_WRITE_ENV_START));
		
		if (!Burn_check_size( nID, hWnd, len, "ENV", &is_noexit))
		{
			if (is_noexit == 1 && theConfig.planform_tpye == E_ROST_PLANFORM)
			{
				return TRUE;
			} 
			else
			{
				PostMessage(hWnd, ON_BURNFLASH_MESSAGE, WPARAM(nID+100), LPARAM(MESSAGE_WRITE_ENV_FAIL));
				return FALSE;
			}
		}
		
		//写mac地址到安全区内
		if (BT_WriteASAFile(nID, "ENV", tbuf, len+4, macmode) != BT_SUCCESS)
		{
			PostMessage(hWnd, ON_BURNFLASH_MESSAGE, WPARAM(nID+100), LPARAM(MESSAGE_WRITE_ENV_FAIL));
			return FALSE;
		}
					   ...
					   BOOL readmacflag, UINT  macmode)
{
	...
	if(theConfig.macaddr_flag) 
	{
		...
		if (!Burn_check_size( nID, hWnd, ptr[0] + 4, "MAC", &is_noexit))
		{
		...
		//写mac地址到安全区内
		if (BT_WriteASAFile(nID, "MAC", tbuf, ptr[0] + 4, macmode) != BT_SUCCESS)
		{
	...
	else if(!theConfig.bUpdate && (theConfig.chip_type == CHIP_39XX_H2 || theConfig.chip_type == CHIP_39XX_H240))
	{
		char is_noexit = 0;
						  ...
						  BOOL readSerialflag, UINT  Serialmode)
{
	...
	if(theConfig.sequenceaddr_flag && nID == 1) 
	{
		...
		if (!Burn_check_size( nID, hWnd, ptr[0] + 4, "SEQ", &is_noexit))
		{
			PostMessage(hWnd, ON_BURNFLASH_MESSAGE, WPARAM(nID+100), LPARAM(MESSAGE_WRITE_SERIAL_ADDR_ASA_FAIL));
			return FALSE;
		}
		
		if (BT_WriteASAFile(nID, "SEQ", tbuf, ptr[0]+4, Serialmode) != BT_SUCCESS)
		{
			//Burn_Fail(seriallowbuf, maclowbuf, nID, readSerialflag, readmacflag);
			burnFile.WriteLogFile(LOG_LINE_TIME, "->fail!\r\n");
			PostMessage(hWnd, ON_BURNFLASH_MESSAGE, WPARAM(nID+100), LPARAM(MESSAGE_WRITE_SERIAL_ADDR_ASA_FAIL));
...
BOOL Burn_upload_spinand_Data(UINT nID, HWND hWnd, PTCHAR file_name, T_NAND_PHY_INFO *NandPhyInfo)
{
	//回读整一个spi的数据
	if (g_bUpload_spialldata)
	{
		...
		if (!Burn_Get_spiAlldata(nID, hWnd, file_name,  NandPhyInfo->blk_num*NandPhyInfo->page_per_blk*NandPhyInfo->page_size))
		{
...
UINT Burn_Get_Filelenght(UINT nID, HWND hWnd, TCHAR *pstrPath)
{
	UINT lenght = 0;
...
BOOL Burn_Set_spinand_Boot_block_size(UINT nID, HWND hWnd, TCHAR *pstrPath, T_NAND_PHY_INFO *NandPhyInfo, UINT *boot_bnum)
{
	UINT dwSize = 0;
	UINT boot_blcok_size = 0;
	UINT dwSize_temp = 0;
	UINT page_size = NandPhyInfo->page_size;
	UINT page_per_block = NandPhyInfo->page_per_blk;
	
	dwSize = Burn_Get_Filelenght(nID, hWnd, pstrPath);
	if (dwSize == 0)
	{
		return AK_FALSE;
	}
	...
	//页对齐(文件大小+文件大小10%)
	dwSize_temp = (dwSize/10 + page_size - 1)/page_size;
	boot_blcok_size = (dwSize + page_size - 1)/page_size + dwSize_temp;
	
	//块对齐
	boot_blcok_size = (boot_blcok_size + page_per_block - 1)/page_per_block;
	
	
	*boot_bnum = boot_blcok_size;
	if(BT_FAIL == BT_Set_Ex_Parameter(nID, boot_blcok_size))
	{
		return AK_FALSE;
	}
...
BOOL Burn_Set_Boot_block_size(UINT nID, HWND hWnd, TCHAR *pstrPath, T_SFLASH_PHY_INFO *sFlashPhyInfo, UINT *boot_bnum)
{
	UINT dwSize = 0;
	UINT boot_blcok_size = 0;
	UINT dwSize_temp = 0;
...
BOOL Burn_Download_spi_img(UINT nID, HWND hWnd, char *burn_spi_img_flag)
{
	UINT i = 0;
	T_DOWNLOAD_SPI_IMG pDownload_spiImg;

	if (theConfig.download_spi_img_count > 1)
	{
		PostMessage(hWnd, ON_BURNFLASH_MESSAGE, WPARAM(nID+100), LPARAM(MESSAGE_SPIFLASH_IMG_NUM_ERROR));
		return FALSE;
	}
	
	PostMessage(hWnd, ON_BURNFLASH_MESSAGE, WPARAM(nID+100), LPARAM(MESSAGE_DOWNLOAD_SPIFLASH_IMG_START));

	for(i = 0; i < theConfig.download_spi_img_count; i++)
	{
		pDownload_spiImg.bCompare = theConfig.download_spi_img_data[i].bCompare;
		_tcscpy(pDownload_spiImg.pc_path , theApp.ConvertAbsolutePath(theConfig.download_spi_img_data[i].pc_path));
		
		if (BT_Download_spi_img(nID, &pDownload_spiImg) != BT_SUCCESS)
		{
			PostMessage(hWnd, ON_BURNFLASH_MESSAGE, WPARAM(nID+100), LPARAM(MESSAGE_DOWNLOAD_SPIFLASH_IMG_FAIL));
			return FALSE;
		}
...
BOOL Burn_Erase_Partition(UINT nID, HWND hWnd, T_SFLASH_PHY_INFO_TRANSC *sFlashPhyInfo, UINT boot_block_num)
{
	UINT i = 0, j = 0;
	UINT block = 0;
	UINT page = 0;
	UINT block_64K = 65536;
	UINT one_k_size = 1024;
	UINT pageperblock = sFlashPhyInfo->erase_size/sFlashPhyInfo->page_size;

	USES_CONVERSION;

	if (theConfig.burn_mode == MODE_NEWBURN && theConfig.erase_partition_size_flag)
	{
		PostMessage(hWnd, ON_BURNFLASH_MESSAGE, WPARAM(nID+100), LPARAM(MESSAGE_START_ERASE_PARTITON));
		...
		for (i = 0; i < theConfig.partition_count; i++)
		{
			...
			if (theConfig.partition_data[i].type == FHA_BIN_TYPE)
			{
				UINT src_len = 0;
				UINT dst_len = 0;
				src_len = _tcslen(theConfig.partition_data[i].Disk_Name);
				for(j = 0; j < g_download_nand_count; j++)
				{
					dst_len = _tcslen(g_download_nand[j].file_name);
					if (src_len == dst_len)
					{
						if (_tcsnccmp(theConfig.partition_data[i].Disk_Name, g_download_nand[j].file_name, dst_len) == 0)
						{
							break;
						}
					}
				...
				if (j == g_download_nand_count)
				{
					PostMessage(hWnd, ON_BURNFLASH_MESSAGE, WPARAM(nID+100), LPARAM(MESSAGE_PARTITION_NAME_ERROR));
					return FALSE;
				}
			}
			...
			//统计分区的大小,以获得最后一块
			page = (theConfig.partition_data[i].Size*one_k_size + sFlashPhyInfo->page_size - 1)/sFlashPhyInfo->page_size;
			page = (page + pageperblock - 1)/pageperblock;
			block += page;
		}

		page = block*pageperblock;
		//计算boot和分区表占的页数
		boot_block_num = (boot_block_num+2) * sFlashPhyInfo->erase_size/sFlashPhyInfo->page_size;
 
		//以64K对齐
		block = (page + boot_block_num + block_64K/sFlashPhyInfo->page_size - 1)/(block_64K/sFlashPhyInfo->page_size);
		
		if(block > (sFlashPhyInfo->total_size/block_64K))
		{
			PostMessage(hWnd, ON_BURNFLASH_MESSAGE, WPARAM(nID+100), LPARAM(MESSAGE_ERASE_BLOCK_NUM_ERROR));
			return AK_FALSE;
		}
		
		for(i = 0; i < block; i++)
		{
			if(BT_Erase_Partition_Size(nID,  i,  i+1)  != BT_SUCCESS)
			{
				PostMessage(hWnd, ON_BURNFLASH_MESSAGE, WPARAM(nID+100), LPARAM(MESSAGE_ERASE_PARTITON_ERROR));
				return AK_FALSE;
			}
		}
	}
...
BOOL Burn_check_fs_type(TCHAR *name, char *fs_type)
{	
	UINT len_src = 0;
	UINT len_dst = 0;
	UINT i = 0;
...
BOOL Burn_Creat_Partition(UINT nID, HWND hWnd)
{
	T_PARTITION_INFO *partInfo = NULL;
	UINT i = 0, j = 0;
	char fs_type = 0;
	UINT namelen = 0;
	...
	partInfo = new T_PARTITION_INFO[theConfig.partition_count];
	if (partInfo == NULL)
	{
	...
	memset(partInfo, 0, theConfig.partition_count * sizeof(T_PARTITION_INFO));
	...
	for (i = 0; i < theConfig.partition_count; i++)
	{
		...
		{
			if (!Burn_check_fs_type(theConfig.partition_data[i].Disk_Name, &fs_type))
			{
				PostMessage(hWnd, ON_BURNFLASH_MESSAGE, WPARAM(nID+100), LPARAM(MESSAGE_CHECK_FS_TYPE_ERROR));
				return FALSE;
			}
		...
		else if (theConfig.partition_data[i].type == FHA_BIN_TYPE)
		{
			...
			partInfo[i].check = g_download_nand[j].bCompare;
			partInfo[i].ld_addr = g_download_nand[j].ld_addr;
			partInfo[i].backup = g_download_nand[j].bBackup;
		}
	}

	//创建分区的功能由produce执行
    if (BT_CreatePartion(nID, partInfo, theConfig.partition_count) != BT_SUCCESS)
	{
		PostMessage(hWnd, ON_BURNFLASH_MESSAGE, WPARAM(nID+100), LPARAM(MESSAGE_CREATE_PARTITION_FAIL));
		return FALSE;
	}
...
//判断序列号是否有效
BOOL Blueaddr_isunuse(char *buf, int len)
{
...
//从安全区读取序列号
BOOL Burn_GetBlueAddrInfo(UINT nID, HWND hWnd, PTCHAR file_name, BOOL *readSerialflag, UINT  *Serialmode, TCHAR *blue_addr_temp, char *need_write_flag)
{
	BOOL serialaddrflag = TRUE;
	UINT Seriallen = MAX_BT_ADDR_LEN + 4;
	TCHAR tempbuf_serial[MAX_BT_ADDR_LEN+1] = {0};
	char blue_addr_empty_flag = 0;
	TCHAR tempbuf[MAX_BT_ADDR_LEN+1] = {0};
	int ret = 0;
    //TCHAR SerialBuf[MAX_MAC_SEQU_ADDR_COUNT+1] = {0};
	...
	*need_write_flag = TRUE;
	if(theConfig.BT_addr_flag) 
	{
		BYTE buf[MAX_BT_ADDR_LEN*2+1] = {0};
		CLogFile  burnFile(file_name);
		
		//判断是否强制烧录序列号
		if (theConfig.fore_write_BT_addr)
		{
			*Serialmode = TRUE;
			//获取序列号地址
			PostMessage(hWnd, ON_BURNFLASH_MESSAGE, WPARAM(nID+100), LPARAM(MESSAGE_GET_BT_ADDR_START));

			//只限制一台烧录
			//读取配置文件，如果配置文件不存在的话，那么就只用开始的

			WaitForSingleObject(g_handle,INFINITE);
			if (!theConfig.No_need_add_flag)
			{
				theConfig.Blue_Address_add(theConfig.blue_cur_addr, tempbuf);
				_tcscpy(theConfig.blue_cur_addr, tempbuf);
			}
			theConfig.No_need_add_flag = FALSE;
			
			_tcscpy(blue_addr_temp, theConfig.blue_cur_addr);
			ReleaseSemaphore(g_handle,1,NULL);

			//判断一下蓝牙地址是否超过结束地址
			if (_tcsnccmp(blue_addr_temp, theConfig.blue_end_addr, MAX_BT_ADDR_LEN) > 0)
			{
				PostMessage(hWnd, ON_BURNFLASH_MESSAGE, WPARAM(nID+100), LPARAM(MESSAGE_GET_BT_ADDR_MORE_END));
				return FALSE;
			}
		}
		else
		{
			*Serialmode = FALSE;
			//if不是强制写入
			burnFile.WriteLogFile(LOG_LINE_TIME,  "++read asa file++\r\n" );
			if ((BT_ReadASAFile(nID, "BTADDR", buf, &Seriallen) == BT_SUCCESS) && (Seriallen != 0))
			{
				*readSerialflag = TRUE;

				if (Seriallen != MAX_BT_ADDR_LEN)
				{
					PostMessage(hWnd, ON_BURNFLASH_MESSAGE, WPARAM(nID+100), LPARAM(MESSAGE_GET_BT_ADDR_LEN_ERROR));
					return FALSE;
				}
				
				//判断读出来的序列号地址是否有效
				PostMessage(hWnd, ON_BURNFLASH_MESSAGE, WPARAM(nID+100), LPARAM(MESSAGE_GET_BT_ADDR_START));
				if (!Blueaddr_isunuse((char *)buf, Seriallen))
				{
					burnFile.WriteLogFile(0, "read serial ADDR is error from asafile!\r\n");
					PostMessage(hWnd, ON_BURNFLASH_MESSAGE, WPARAM(nID+100), LPARAM(MESSAGE_GET_BT_ADDR_ERROR));
					return FALSE;
				}
				
				memset(tempbuf_serial, 0, MAX_BT_ADDR_LEN+1);
				theConfig.lower_to_upper(A2T((char *)buf), tempbuf_serial);
				//_tcscpy(theConfig.blue_cur_addr, tempbuf_serial);
				_tcscpy(blue_addr_temp, tempbuf_serial);
			}
			else
			{
				*Serialmode = TRUE;
		...
		//显示当前通道的序列号地址
		_tcscpy(theConfig.m_blue_show_addr[nID-1], blue_addr_temp);
		//_tcscpy(serialtempbuf, SerialBuf);
		PostMessage(hWnd, ON_BURNFLASH_MESSAGE, WPARAM(nID+100), LPARAM(MESSAGE_BURN_BLUE_ADDR_SHOW));
	}
...
//写mac地址到安全区
BOOL Burn_Write_BlueAddr(UINT nID, HWND hWnd, TCHAR *blue_addr_temp)
{
	...
	if (theConfig.BT_addr_flag)
	{
		...
		TCHAR buf[MAX_BT_ADDR_LEN+1] = {0};
		BYTE tbuf[MAX_BT_ADDR_LEN*2+1];
		UINT *ptr = (UINT *)tbuf; 
		
		//_tcscpy(buf, theConfig.blue_cur_addr);
		_tcscpy(buf, blue_addr_temp);
		ptr[0] = wcslen(buf);
		memcpy(&tbuf[4], T2A(buf), ptr[0]);
		
		PostMessage(hWnd, ON_BURNFLASH_MESSAGE, WPARAM(nID+100), LPARAM(MESSAGE_WRITE_BT_ADDR_START));
		//写blue地址到安全区内
		if (BT_WriteASAFile(nID, "BTADDR", tbuf, ptr[0] + 4, 1) != BT_SUCCESS)
		{
			PostMessage(hWnd, ON_BURNFLASH_MESSAGE, WPARAM(nID+100), LPARAM(MESSAGE_WRITE_BT_ADDR_FAIL));
			return FALSE;
		}
...
//spi的烧录
BOOL burn_spiflash(UINT nID, HWND hWnd, T_MODE_CONTROL ModeCtrl,  PTCHAR file_name, T_SFLASH_PHY_INFO_TRANSC *sFlashPhyInfo)
{
	...
	UINT value = 0, j = 0;
	UINT file_len = 0;
	char burn_spi_img_flag = 0;
	BOOL readblueflag_spi = FALSE;//写标志
	TCHAR blue_addr_temp[MAX_BT_ADDR_LEN+1] = {0};
	char need_write_flag = TRUE;//写标志
	UINT boot_block_num = 0;

	USES_CONVERSION;

	UINT sflashID, sflashChipCnt;
	PostMessage(hWnd, ON_BURNFLASH_MESSAGE, WPARAM(nID+100), LPARAM(MESSAGE_GET_CHIP_PARA));
	if (0)//theConfig.chip_type == CHIP_10XX_T)
	{
		if (g_bUploadbinMode || g_bUpload_spialldata)
		{
			value = 1;
		}
		if (BT_Setvalue_forupload_orspimake(nID,  value) != BT_SUCCESS)
		{
	...
	if (i == theConfig.spiflash_parameter_count)
	{
		PostMessage(hWnd, ON_BURNFLASH_MESSAGE, WPARAM(nID+100), LPARAM(MESSAGE_SET_CHIP_PARA_FAIL));
		return FALSE;
	}
	...
	//传boot区的块值下去
	if (!Burn_Set_Boot_block_size(nID, hWnd, theApp.ConvertAbsolutePath(theConfig.path_nandboot), sFlashPhyInfo, &boot_block_num))
	{
		PostMessage(hWnd, ON_BURNFLASH_MESSAGE, WPARAM(nID+100), LPARAM(IDS_BURN_SET_EX_PARAM_FAIL));
		return FALSE;
	}
	...
	if (ModeCtrl.burn_mode == MODE_NEWBURN && g_bEraseMode)
	{
	...
	//如果有镜像文件，那么就只下载镜像文件
	if(Burn_Download_spi_img(nID, hWnd, &burn_spi_img_flag) == BT_FAIL)
	{
		return FALSE;
	}
	else
	...
	{
		//读取蓝牙地址
		memset(blue_addr_temp, 0, MAX_BT_ADDR_LEN+1);
		if (!Burn_GetBlueAddrInfo(nID, hWnd, file_name, &readblueflag_spi, &Serialmode_spi, blue_addr_temp, &need_write_flag))
		{
			Burn_Fail(seriallowbuf_spi, maclowbuf_spi, nID, readSerialflag_spi, readmacflag_spi);
			return FALSE;
		}
	...
	//
	if(!Burn_Erase_Partition(nID, hWnd, sFlashPhyInfo, boot_block_num))
	{
		Burn_Fail(seriallowbuf_spi, maclowbuf_spi, nID, readSerialflag_spi, readmacflag_spi);
		return FALSE;
	}
	...
	//创建partition
	if (!Burn_Creat_Partition(nID, hWnd))
	{
		Burn_Fail(seriallowbuf_spi, maclowbuf_spi, nID, readSerialflag_spi, readmacflag_spi);
		return FALSE;
	}
	...
	for(i = 0; i < g_download_nand_count; i++)
	{
		...
		for (j = 0; j < theConfig.partition_count; j++)
		{
			...
			if(part_name_len > DOWNLOAD_BIN_FILENAME_SIZE || down_name_len > DOWNLOAD_BIN_FILENAME_SIZE)
			{
				Burn_Fail(seriallowbuf_spi, maclowbuf_spi, nID, readSerialflag_spi, readmacflag_spi);
				PostMessage(hWnd, ON_BURNFLASH_MESSAGE, WPARAM(nID+100), LPARAM(MESSAGE_GETPARTITION_NAME_lEN_ERROR));
				return FALSE;	
			}
		...
		file_len = Burn_Get_Filelenght(nID, hWnd, download_bin.pc_path);
	...
	if (theConfig.chip_type == CHIP_10XX_T) 
	{
		if(!Burn_Write_BlueAddr(nID, hWnd, blue_addr_temp))
		{
			theConfig.blue_burn_result = 0;
			//记录到文档上
			if (theConfig.BT_addr_flag && !readblueflag_spi)
			{
				WaitForSingleObject(g_handle,INFINITE);
				theConfig.Write_blue_addr_log(BLUE_LOG_FILE_NAME, blue_addr_temp);
				ReleaseSemaphore(g_handle,1,NULL);
			}
		...
		theConfig.blue_burn_result = 1;
	}


	//下载文件系统镜像
	if (TRANSC_MEDIUM_SPI_EMMC != ModeCtrl.eMedium)
	{
		...
		for(i = 0; i < theConfig.download_mtd_count; i++)
		{
			...
			if (src_len == 0)
			{
				PostMessage(hWnd, ON_BURNFLASH_MESSAGE, WPARAM(nID+100), LPARAM(MESSAGE_GET_PARTITION_FAIL));
				return FALSE;
			}
			...
			if (part_size == 0 || part_size > sFlashPhyInfo->total_size)
			{
				PostMessage(hWnd, ON_BURNFLASH_MESSAGE, WPARAM(nID+100), LPARAM(MESSAGE_GET_PARTITION_SIZE_FAIL));
				return FALSE;
			}
	...
	{
		if(theConfig.bUpdate && 0xFFFFFFFF == GetFileAttributes(theApp.ConvertAbsolutePath(theConfig.path_nandboot)))
		{
			//当升级时，不需要升级boot，去掉就可以了
		}
		else
		{
...
BOOL Download_Channel_Addranddata_ToRam(UINT nID, HWND hWnd, TCHAR *file_name)
{
	...
	*/
	if(theConfig.chip_type == CHIP_39XX_H2 || theConfig.chip_type == CHIP_39XX_H240)//ak39芯片
	{
		burnFile.WriteLogFile(0,  "channel ID=0x80100000\\n");
		channel_addr = 0x80100000;
	}
	...
	if (theConfig.cmdLine.bCmdLine)
	{
		...
		if(theConfig.chip_type == CHIP_37XX || theConfig.chip_type == CHIP_37XX_L || theConfig.chip_type == CHIP_37XX_C)//ak37芯片
		{
...
{
	if (0)//CHIP_980X == theConfig.chip_type) 
	{
		CLogFile  burnFile(file_name);
...
BOOL Burn_SetRamParam(UINT nID, HWND hWnd, PTCHAR file_name)
{
	...
	//	 && (CHIP_10XXC != theConfig.chip_type) && (CHIP_10XX_T != theConfig.chip_type))
	if (CHIP_10XX_T != theConfig.chip_type)
	{
		CLogFile  burnFile(file_name);
...
{
	if (0)//(theConfig.planform_tpye == E_ROST_PLANFORM) && ((CHIP_37XX == theConfig.chip_type)  || (CHIP_37XX_L == theConfig.chip_type) || theConfig.chip_type == CHIP_37XX_C))
	{
		CLogFile  burnFile(file_name);
...
{
    //#ifdef SUPPORT_LINUX
	if (0)//theConfig.planform_tpye == E_LINUX_PLANFORM)
	{
		CLogFile  burnFile(file_name);
...
BOOL Burn_SwitchUSB11To20(UINT nID, HWND hWnd, PTCHAR file_name)
{
	...
	if (!g_bEraseMode)
	{
		//if (((theConfig.bUsb2) && (CHIP_1080L != theConfig.chip_type) 
		//	&& (CHIP_10XXC != theConfig.chip_type) && (CHIP_10XX_T != theConfig.chip_type)
		//	&& (CHIP_37XX_L != theConfig.chip_type) && (theConfig.chip_type != CHIP_37XX_C)
		//	&& (theConfig.chip_type != CHIP_39XX_H2) && (theConfig.chip_type != CHIP_39XX_H240))
		//	|| ((theConfig.bPLL_Frep_change) && ((CHIP_1080L == theConfig.chip_type) 
		//	|| (CHIP_10XXC == theConfig.chip_type) || (CHIP_37XX_L == theConfig.chip_type) 
		//	|| (CHIP_37XX_C == theConfig.chip_type) || (CHIP_10XX_T == theConfig.chip_type)
		//	|| (CHIP_39XX_H2 == theConfig.chip_type) || (theConfig.chip_type == CHIP_39XX_H240))))
		
		if ((theConfig.bUsb2) 
			&& (CHIP_10XX_T != theConfig.chip_type)
			&& (theConfig.chip_type != CHIP_39XX_H2) 
			&& (theConfig.chip_type != CHIP_39XX_H240))
		{
			CLogFile  burnFile(file_name);
...
BOOL Burn_SetBurnMode(UINT nID, HWND hWnd, PTCHAR file_name, T_MODE_CONTROL ModeCtrl)
{
	...
	if (ModeCtrl.burn_mode == MODE_NEWBURN)
	{
		ModeCtrl.erase_partition_size = theConfig.erase_partition_size_flag;
		ModeCtrl.mo_need_erase = theConfig.no_need_erase_flag;
	}
	else
	{
		ModeCtrl.erase_partition_size = 0;
		ModeCtrl.mo_need_erase = 0;
	}
...
BOOL Burn_GetFlashIDAndSetNandParam(UINT nID, HWND hWnd, PTCHAR file_name, 
					 UINT *sector_per_page, UINT *SectorSize, T_NAND_PHY_INFO *NandPhyInfo, UINT *flash_ChipCnt, UINT *boot_block_num)
{
	...
	if (E_CONFIG_NAND == theConfig.burn_mode || E_CONFIG_SPI_NAND == theConfig.burn_mode)
	{
		...
		*flash_ChipCnt = flashChipCnt; //(低位是chip_cnt, 高位highID)
		...
		//获取BOOT文件的大小，并设置下去
		if (!Burn_Set_spinand_Boot_block_size(nID, hWnd, theApp.ConvertAbsolutePath(theConfig.path_nandboot), NandPhyInfo, boot_block_num))
		{
			PostMessage(hWnd, ON_BURNFLASH_MESSAGE, WPARAM(nID+100), LPARAM(IDS_BURN_SET_EX_PARAM_FAIL));
			return FALSE;
		}
		...
		//获取到的nandID传到produce上
		if (BT_SetNandParam(nID, NandPhyInfo) == BT_FAIL)
		{
			burnFile.WriteLogFile(LOG_LINE_TIME, "->set nand para fail\r\n" );
			PostMessage(hWnd, ON_BURNFLASH_MESSAGE, WPARAM(nID+100), LPARAM(MESSAGE_SET_NAND_PARA_FAIL));
			return FALSE;
...
BOOL Burn_CreatePartion(UINT nID, HWND hWnd, PTCHAR file_name, T_PARTION_INFO *partInfo)
{
    ...
    */
    ...
    if (1)//BT_CreatePartion(nID, partInfo, theConfig.format_count, theConfig.fs_res_blocks) != BT_SUCCESS)
	{
        //Burn_Fail(seriallowbuf, maclowbuf, nID, readSerialflag, readmacflag);
		burnFile.WriteLogFile(LOG_LINE_TIME, "->fail!\r\n" );
		PostMessage(hWnd, ON_BURNFLASH_MESSAGE, WPARAM(nID+100), LPARAM(MESSAGE_CREATE_PARTITION_FAIL));
...
BOOL Burn_OnlineMakingImage(UINT nID, HWND hWnd, PTCHAR file_name, UINT sector_per_page, UINT SectorSize, UINT StartID, UINT DriverCnt, UCHAR MediumType)
{
	...
	//在线制作镜像
	if (0)//eConfig.planform_tpye == E_ROST_PLANFORM))
	{
			...
			//|| (CHIP_1080L == theConfig.chip_type) || (CHIP_10XXC == theConfig.chip_type))
		{
			...
			if (0)//CHIP_1080L == theConfig.chip_type || CHIP_10XXC == theConfig.chip_type)
			{
...
//下载镜像文件
BOOL Burn_Download_fsImg(UINT nID, HWND hWnd, PTCHAR file_name, T_NAND_PHY_INFO *NandPhyInfo)
{
	UINT pagesize = NandPhyInfo->page_size;
	T_DOWNLOAD_IMG download_img;
	UINT part_size = 0;
	UINT i = 0, j = 0;
	...
	for(i = 0; i < theConfig.download_mtd_count; i++)
	{
		...
		if (part_size == 0 || part_size > NandPhyInfo->blk_num*NandPhyInfo->page_per_blk*NandPhyInfo->page_size)
		{
			PostMessage(hWnd, ON_BURNFLASH_MESSAGE, WPARAM(nID+100), LPARAM(MESSAGE_GET_PARTITION_SIZE_FAIL));
			return FALSE;
		}
					  ...
					  T_NAND_PHY_INFO NandPhyInfo, T_PARTION_INFO *partInfo)
{
	...
	if (theConfig.burn_mode == E_CONFIG_SD || theConfig.burn_mode == E_CONFIG_SPI_SD )
	{
		pagesize = SectorSize;
...
BOOL Burn_ReSetDev(UINT nID, HWND hWnd, PTCHAR file_name)
{
	...
	//	|| CHIP_10XXC == theConfig.chip_type || CHIP_10XX_T == theConfig.chip_type))
	if(0)//CHIP_10XX_T == theConfig.chip_type)
	{
		if (m_worknum != 0)
		{
			//等待m_worknum == 0时才所有重启
			if (!theConfig.bAuto_Burnflag || !theConfig.bUDisk_Massboot)
			{
...
{
	if (0)//(theConfig.planform_tpye == E_ROST_PLANFORM) && ((CHIP_1080L == theConfig.chip_type) || (CHIP_10XXC == theConfig.chip_type)))
	{
		CAKFS cAK;
		if (E_CONFIG_NAND == theConfig.burn_mode || E_CONFIG_SPI_NAND == theConfig.burn_mode)
		{
		...
		else if (E_CONFIG_SD == theConfig.burn_mode || theConfig.burn_mode == E_CONFIG_SPI_SD)
		{
...
BOOL BurnDebugMode(UINT nID, HWND hWnd, CString path)
{
	...
	for(i = 0; i < theConfig.partition_count; i++)
	{
		...
		memcpy(uploadbin.file_name, T2A(theConfig.partition_data[i].Disk_Name), DOWNLOAD_BIN_FILENAME_SIZE);
		
        strTmp = path + _T("//UploadBin//");
		strTmp += uploadbin.file_name;
		strTmp += _T(".bin");
...
//	burnFile->WriteLogFile(LOG_LINE_TIME, "Burn_GetRegValue fail\n");
	...
	if (theConfig.chip_type == CHIP_10XX_T)
	{
		UINT metafixAddr = 0x00400000, metafixValue;
		...
		if(0x20130300 == metafixValue)
		{
			theConfig.chip_type = CHIP_10XX_T;  //10T
		}
		else
		{
			return FALSE;
	...
	if(theConfig.chip_type == CHIP_39XX_H2 || theConfig.chip_type == CHIP_39XX_H240)
	{
		...
		if (0x20140100 == metafixValue)
		{
			theConfig.chip_type = CHIP_39XX_H2;
		}
		else if (0x20150200 == metafixValue)
		{
			theConfig.chip_type = CHIP_39XX_H240;
		}
		else
...
//	char *timeline;
	...
	*/
	if (!theConfig.bAuto_Burnflag && theConfig.bUDiskUpdate && m_budisk_getUSBnum && g_udisk_burnnum > 0)
	{
		WaitForSingleObject(g_handle,INFINITE);
    ...
    if (0)//!Burn_SetGPIO(nID, hWnd, file_name))
	{
	...
	//由于使用的produce是根据MODE_DEBUG值进行读取spi文件的信息,
	...
	if (TRANSC_MEDIUM_SPIFLASH == ModeCtrl.eMedium || TRANSC_MEDIUM_SPI_EMMC == ModeCtrl.eMedium)
	{
		//spi 烧录
		if (burn_spiflash(nID, hWnd, ModeCtrl, file_name, &sFlashPhyInfo))
		{
			if (g_bUploadbinMode)
			{
				goto UPDATA_BINMODE;//bin回读
			}
			else if (g_bEraseMode)
			{
				//低格完成
				theConfig.burn_totalnum = 0;
				goto BURN_SUCCESS;
			...
			{
				if (ModeCtrl.eMedium == TRANSC_MEDIUM_SPI_EMMC)
				{
					goto BURN_SD;
				} 
				else
		...
		else
		{
			...
			if (theConfig.chip_type == CHIP_10XX_T && !g_bUploadbinMode)
			{
				BT_ResetDevice(nID);
			}
	...
	//
    if (!Burn_GetFlashIDAndSetNandParam(nID, hWnd, file_name, &sector_per_page, &SectorSize, &NandPhyInfo, &flash_ChipCnt, &boot_block_num))
	{
	...
	//回读spinand的镜像数据
	if (!Burn_upload_spinand_Data(nID, hWnd, file_name, &NandPhyInfo))
	{
...
UPDATA_BINMODE:
	...
	if (0)//fNand_Get_BadBlockBuf(nID, NandPhyInfo.blk_num, flash_ChipCnt, &ModeCtrl)  != BT_SUCCESS)
	{
...
SET_RESV_AREA:
	...
	//设置非文件系系统保留区大小，
	//并对此区进行擦
	if (0)//!Burn_SetResvAreaSize(nID, hWnd, file_name))
	{
...
BURN_SD:
	...
	if (0)//(ModeCtrl.burn_mode == MODE_UPDATE) && (theConfig.planform_tpye == E_ROST_PLANFORM) 
		//&& ((CHIP_1080L == theConfig.chip_type) || (CHIP_10XXC == theConfig.chip_type)))
	{
		goto BURN_BOOT;
	}
	...
	if (0)//(theConfig.planform_tpye == E_ROST_PLANFORM) && ((CHIP_1080L == theConfig.chip_type) || (CHIP_10XXC == theConfig.chip_type)))
	{
		...
		else if(TRANSC_MEDIUM_EMMC == ModeCtrl.eMedium || TRANSC_MEDIUM_SPI_EMMC == ModeCtrl.eMedium)
		{
			T_EMMC_INFO medium;
	...
	//burnFile.WriteLogFile(LOG_LINE_TIME, "Burn_CreateDiskVolume\r\n" );
	//创建卷标
	...
	//burnFile.WriteLogFile(LOG_LINE_TIME, "Burn_unmountdisk_Wait\r\n" );
	...
	//burnFile.WriteLogFile(LOG_LINE_TIME, "Burn_OnlineMakingImage\r\n" );
	...
	//burnFile.WriteLogFile(LOG_LINE_TIME, "Burn_DownloadImg\r\n" );
	//下开镜像文件
...
*/
	...
	//下载镜像文件
	if (!Burn_Download_fsImg(nID, hWnd, file_name, &NandPhyInfo))
	{
		Burn_Fail(seriallowbuf, maclowbuf, nID, readSerialflag, readmacflag);
		return FALSE;
	}
...
BURN_SUCCESS:
	...
	//在烧录完所有通道后，进行统一重启
	if (0)//!Burn_ReSetDev(nID, hWnd, file_name))
	{
...
T_BOOL fNand_Get_BadBlockBuf(UINT nID, T_U32 BlockNum, T_U32 nChipCnt, T_MODE_CONTROL *ModeCtrl)
{
	if (0)//(theConfig.planform_tpye == E_ROST_PLANFORM) 
		//&& ((CHIP_1080L == theConfig.chip_type) || (CHIP_10XXC == theConfig.chip_type)) 
