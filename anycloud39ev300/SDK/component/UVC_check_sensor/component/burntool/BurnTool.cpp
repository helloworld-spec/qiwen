...
//	theConfig.read_config_addr(CONFIG_ADDR_FILE_NAME, sequencecurrentlow, (TCHAR *)&theConfig.g_sequence_current_low, theConfig.sequence_current_low);
	...
	if (m_config_file_cnt > 0)
	{
		...
		{
			if (0 == _tcsncmp(strLang, m_config_file[i], 256))
			{
				m_config_index = i;
				break;
			}
	...
	if (theConfig.Read_blue_cur_addr(BLUE_LOG_FILE_NAME, &blue_addr_empty_flag) == 0 && blue_addr_empty_flag == 0)
	{
		//log文件不存在，或没有记录的开始地址
		theConfig.No_need_add_flag = TRUE;
	}
...
UINT CBurnToolApp::GetConfigFiles()
{
    ...
    while (!fFinish)
    {
        ...
        {
            _tcsncpy(m_config_file[m_config_file_cnt], FindFileData.cFileName,MAX_PATH);
            m_config_file_cnt++;
        }
