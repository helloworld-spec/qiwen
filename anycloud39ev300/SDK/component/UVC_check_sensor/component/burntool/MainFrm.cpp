...
T_DOWNLOAD_NAND g_download_nand[MAX_DOWNLOAD_NAND] = {0};
UINT g_download_udisk_count = 0;
T_DOWNLOAD_UDISK g_download_udisk[MAX_DOWNLOAD_FILES] = {0};
UINT g_download_mtd_count = 0;
T_DOWNLOAD_MTD g_download_mtd[MAX_DOWNLOAD_MTD] = {0};
UINT m_worknum = 0;
...
BEGIN_MESSAGE_MAP(CMainFrame, CFrameWnd)
	...
	ON_BN_CLICKED(IDC_CHECK_NO_FIXED, OnCheckNoFixed)
	ON_BN_CLICKED(IDC_CHECK_UDISK_MASSBOOT, OnCheckUdiskMassboot)
	...
	ON_BN_CLICKED(IDC_CHECK_ERASE_FILE_SIZE, OnCheckEraseFileSize)
	ON_BN_CLICKED(IDC_CHECK_NO_NEED_ERASE, OnCheckNoNeedErase)
	//}}AFX_MSG_MAP
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	m_DlgBar.SetDlgItemText(IDC_CHECK_NO_FIXED,theApp.GetString(IDS_NO_FIXED_CHANNELS));
	m_DlgBar.SetDlgItemText(IDC_CHECK_UDISK_MASSBOOT,theApp.GetString(IDS_UDISK_BURN_MASSBOOT));

	m_DlgBar.SetDlgItemText(IDC_CHECK_ERASE_FILE_SIZE,theApp.GetString(IDS_ERASE_FILE_SIZE));
	m_DlgBar.SetDlgItemText(IDC_CHECK_NO_NEED_ERASE,theApp.GetString(IDS_NO_NEED_ERASE));
...
*/
	...
	//	|| theConfig.chip_type == CHIP_10XX_T))
	if (TRUE == theConfig.bUDiskUpdate && theConfig.chip_type == CHIP_10XX_T)
	{
		...
		theConfig.bAuto_Burnflag = 0;

		((CButton *)m_DlgBar.GetDlgItem(IDC_CHECK_NO_FIXED))->EnableWindow(FALSE);
		((CButton *)m_DlgBar.GetDlgItem(IDC_CHECK_UDISK_MASSBOOT))->EnableWindow(FALSE);
		theConfig.bchannels_no_fixed = FALSE;
		theConfig.bUDisk_Massboot = FALSE;
		((CButton *)m_DlgBar.GetDlgItem(IDC_CHECK_NO_FIXED))->SetCheck(theConfig.bchannels_no_fixed);
		((CButton *)m_DlgBar.GetDlgItem(IDC_CHECK_UDISK_MASSBOOT))->SetCheck(theConfig.bUDisk_Massboot);
	} 
	else
	{
		((CButton *)m_DlgBar.GetDlgItem(IDC_CHECK_AUTO_BURN))->EnableWindow(TRUE);
		((CButton *)m_DlgBar.GetDlgItem(IDC_CHECK_AUTO_BURN))->SetCheck(theConfig.bAuto_Burnflag);
		
		((CButton *)m_DlgBar.GetDlgItem(IDC_CHECK_NO_FIXED))->EnableWindow(TRUE);
		((CButton *)m_DlgBar.GetDlgItem(IDC_CHECK_UDISK_MASSBOOT))->EnableWindow(TRUE);
		
		((CButton *)m_DlgBar.GetDlgItem(IDC_CHECK_NO_FIXED))->SetCheck(theConfig.bchannels_no_fixed);
		((CButton *)m_DlgBar.GetDlgItem(IDC_CHECK_UDISK_MASSBOOT))->SetCheck(theConfig.bUDisk_Massboot);
	}

	((CButton *)m_DlgBar.GetDlgItem(IDC_CHECK_ERASE_FILE_SIZE))->SetCheck(theConfig.erase_partition_size_flag);
	((CButton *)m_DlgBar.GetDlgItem(IDC_CHECK_NO_NEED_ERASE))->SetCheck(theConfig.no_need_erase_flag);
    
	check = ((CButton *)m_DlgBar.GetDlgItem(IDC_CHECK_ERASE_FILE_SIZE))->GetCheck();
	if (check)
	{
		theConfig.erase_partition_size_flag = TRUE;
		((CButton *)m_DlgBar.GetDlgItem(IDC_CHECK_NO_NEED_ERASE))->EnableWindow(FALSE);
		((CButton *)m_DlgBar.GetDlgItem(IDC_CHECK_NO_NEED_ERASE))->SetCheck(FALSE);
		theConfig.no_need_erase_flag = FALSE;
	} 
	else
	{
		theConfig.erase_partition_size_flag = FALSE;
		((CButton *)m_DlgBar.GetDlgItem(IDC_CHECK_NO_NEED_ERASE))->EnableWindow(TRUE);
	}
	// TODO: Add your control notification handler code here
	check = ((CButton *)m_DlgBar.GetDlgItem(IDC_CHECK_NO_NEED_ERASE))->GetCheck();
	if (check)
	{
		theConfig.no_need_erase_flag = TRUE;
		((CButton *)m_DlgBar.GetDlgItem(IDC_CHECK_ERASE_FILE_SIZE))->EnableWindow(FALSE);
		((CButton *)m_DlgBar.GetDlgItem(IDC_CHECK_ERASE_FILE_SIZE))->SetCheck(FALSE);
		theConfig.erase_partition_size_flag = FALSE;
	} 
	else
	{
		theConfig.no_need_erase_flag = FALSE;
		((CButton *)m_DlgBar.GetDlgItem(IDC_CHECK_ERASE_FILE_SIZE))->EnableWindow(TRUE);
	}



	//if (CHIP_10X6 == theConfig.chip_type || CHIP_10XXC == theConfig.chip_type 
	//	|| CHIP_10XX_T == theConfig.chip_type || CHIP_1080L == theConfig.chip_type)
	/*
...
void CMainFrame::ScanUDisk()
{
	...
	if (theConfig.bUDisk_Massboot && theConfig.bchannels_no_fixed && theConfig.bAuto_Burnflag)
	{
		frmLogfile.WriteLogFile(0,"wait udisk massboot\r\n");
		Sleep(1000);
	}
		...
		if(diskName[i] >= 'A' && diskName[i] <= 'Z')
		{
           ...
           frmLogfile.WriteLogFile(0,"BT_AttachUSB vol = %d diskName = %c m_udisk_state %d\n",vol, diskName[i], m_udisk_state[vol]);
			if(UDISK_NULL == m_udisk_state[vol])
			{
				...
				if (theConfig.bchannels_no_fixed)
				{
					BT_Is_Bind_channels_flag(FALSE);
				}
				else
				{
					BT_Is_Bind_channels_flag(TRUE);
				}
				send_CC_command = FALSE;
				index = BT_AttachUSB(strDisk, USB_MASS);
				if (index == 0xFFFFFFFF)
				{
					index = 0;
					if (theConfig.bUDisk_Massboot)
					{
						send_CC_command = TRUE;
					}
				}
				...
				if (attach_flag != 1)
				{
					...
					else if ((send_CC_command) ||(check == TRUE && m_worknum == 0 && theConfig.bUDiskUpdate)) // if (theConfig.bUDiskUpdate && m_worknum == 0)
					{
						//当勾选自动烧录，以及是刚开始的
						PostMessage(ON_BURNFLASH_MESSAGE, diskName[i], LPARAM(MESSAGE_SEND_CMD_ANYKA));
					}
	...
	else
	{
		...
		//{
			for (i = 0; i < theConfig.format_count; i++)
			{
				...
				parttion_length += theConfig.format_data[i].Size;
			}
...
LRESULT CMainFrame::On_Message(WPARAM wp, LPARAM lp)
{
	...
	case MESSAGE_SET_STANDBY:
		...
		if ((check))// && (0 ==(theConfig.device_num - m_cur_link)))
		{
			...
			//if (falg == TRUE && theConfig.planform_tpye == E_LINUX_PLANFORM)
			//{
			//	OnTbtnStart();
			//}
			//else if (theConfig.planform_tpye == E_ROST_PLANFORM)
			//{
			OnTbtnStart();
			//}
		}
	...
	case MESSAGE_BURN_BLUE_ADDR_SHOW:
		//烧录完成后显示当前序列号
		OnBlueAddr(port, TRUE); 
		break;
	...
	case MESSAGE_GET_HIGHID_FAIL:
		...
		break;
	case MESSAGE_GET_BT_ADDR_START:
		pView->SetStatDesp(port, theApp.GetString(IDS_GET_BT_ADDR_START));
		break;
	case MESSAGE_WRITE_BT_ADDR_START:
		pView->SetStatDesp(port, theApp.GetString(IDS_WRITE_BT_ADDR_START));
		break;
	case MESSAGE_GET_BT_ADDR_MORE_END:
		terminate_mission(port, theApp.GetString(IDS_GET_BT_ADDR_MORE_END));
		break;
	case MESSAGE_GET_BT_ADDR_LEN_ERROR:
		terminate_mission(port, theApp.GetString(IDS_GET_BT_ADDR_LEN_ERROR));
		break;
	case MESSAGE_GET_BT_ADDR_ERROR:
		terminate_mission(port, theApp.GetString(IDS_GET_BT_ADDR_ERROR));
		break;
	case MESSAGE_WRITE_BT_ADDR_FAIL:
		terminate_mission(port, theApp.GetString(IDS_WRITE_BT_ADDR_FAIL));
		break;
	...
	case MESSAGE_SET_CHIP_PARA_FAIL:
		...
		break;
	case MESSAGE_NOT_CHIP_PARA:
		terminate_mission(port, theApp.GetString(IDS_BURN_SET_NAND_PARAM_FAIL));
		break;
	case MESSAGE_SET_EX_PARA_FAIL:
		terminate_mission(port, theApp.GetString(IDS_BURN_SET_NAND_PARAM_FAIL));
		break;
	...
	case MESSAGE_GET_PARTITION_FAIL:
		terminate_mission(port, theApp.GetString(IDS_GET_PARTITION_FAIL));
		break;

	case MESSAGE_GET_PARTITION_SIZE_FAIL:
		terminate_mission(port, theApp.GetString(IDS_GET_PARTITION_SIZ_FAIL));
		break;
	...
	case MESSAGE_WRITE_ENV_FAIL:
		terminate_mission(port, theApp.GetString(IDS_WRITE_ENV_FAIL));
		break;
		
	case MESSAGE_WRITE_ENV_START:
		pView->SetStatDesp(port, theApp.GetString(IDS_WRITE_ENV_START));
		break;
	...
	case MESSAGE_BURN_SERIAL_ADDR_FAIL:
		...
		break;
	case MESSAGE_DOWNLOAD_SPIFLASH_IMG_FAIL:
		terminate_mission(port, theApp.GetString(IDS_DOWNLOAD_SPIFLASH_IMG_FAIL));
		break;
	case MESSAGE_SPIFLASH_IMG_NUM_ERROR:
		terminate_mission(port, theApp.GetString(IDS_SPIFLASH_IMG_NUM_ERROR));
		break;

	case MESSAGE_DOWNLOAD_SPIFLASH_IMG_START:
		pView->SetStatDesp(port, theApp.GetString(IDS_DOWNLOAD_SPIFLASH_IMG_START));
		break;
	case MESSAGE_DOWNLOAD_SPIFLASH_IMG_FINISH:
		pView->SetStatDesp(port, theApp.GetString(IDS_DOWNLOAD_SPIFLASH_IMG_FINISH));
		break;
    ...
    case MESSAGE_SET_RESV_AREA_FAIL:
		...
		break;
	case MESSAGE_GETPARTITION_NAME_lEN_ERROR:
		terminate_mission(port, theApp.GetString(IDS_GETPARTITION_NAME_LEN_ERROR));
		break;
	case MESSAGE_GETPARTITION_NAME_FAIL:
		terminate_mission(port, theApp.GetString(IDS_GETPARTITION_NAME_FAIL));
		break;
	case MESSAGE_START_ERASE_PARTITON: //开始擦分区的块
		pView->SetStatDesp(port, theApp.GetString(IDS_START_ERASE_PARTITON));
		break;  
	case MESSAGE_ERASE_BLOCK_NUM_ERROR://擦分区的块失败
		terminate_mission(port, theApp.GetString(IDS_ERASE_BLOCK_NUM_ERROR));
		break;
	case MESSAGE_ERASE_PARTITON_ERROR://擦分区的块失败
		terminate_mission(port, theApp.GetString(IDS_ERASE_PARTITON_ERROR));
		break;
    case MESSAGE_CREATE_PARTITION_START:
		pView->SetStatDesp(port, theApp.GetString(IDS_CREATE_PARTITION_START));
		break;      
	case MESSAGE_CHECK_FS_TYPE_ERROR:
		terminate_mission(port, theApp.GetString(IDS_CHECK_FS_TYPE_FAIL));
		break;
    case MESSAGE_CREATE_PARTITION_FAIL:
		terminate_mission(port, theApp.GetString(IDS_CREATE_PARTITION_FAIL));
		break;
	case MESSAGE_PARTITION_NAME_ERROR:
		terminate_mission(port, theApp.GetString(IDS_PARTITION_NAME_ERROR));
		break;
	...
	case MESSAGE_START_IMAGE_CREATE:
		...
		if(g_img_stat == E_IMG_INIT)
		{
			...
			if (1)//theConfig.planform_tpye == E_ROST_PLANFORM && ((theConfig.chip_type == CHIP_1080L) || (theConfig.chip_type == CHIP_10XXC) || (theConfig.chip_type == CHIP_10XX_T)))
			{
...
void CMainFrame::OnBlueAddr(int port, BOOL falg) 
{
	UINT tempsequence = 0;
	TCHAR buf[MAX_MAC_SEQU_ADDR_COUNT+1] = {0};
	...
	_tcsncpy(buf, theConfig.m_blue_show_addr[port], MAX_MAC_SEQU_ADDR_COUNT);		
...
void CMainFrame::OnAutoBurn() 
{
	...
	if (check)
	{
		theConfig.bAuto_Burnflag = TRUE;
	} 
	else
	{
		theConfig.bAuto_Burnflag = FALSE;
	}
}
...
void CMainFrame::OnCheckNoFixed() 
{
	// TODO: Add your control notification handler code here
	BOOL check = FALSE;
	check = ((CButton *)m_DlgBar.GetDlgItem(IDC_CHECK_NO_FIXED))->GetCheck();
	if (check)
	{
		BT_Is_Bind_channels_flag(FALSE);
		theConfig.bchannels_no_fixed = TRUE;
	} 
	else
	{
		BT_Is_Bind_channels_flag(TRUE);
...
void CMainFrame::OnCheckUdiskMassboot() 
{
	// TODO: Add your control notification handler code here
	BOOL check = FALSE;
	check = ((CButton *)m_DlgBar.GetDlgItem(IDC_CHECK_UDISK_MASSBOOT))->GetCheck();
	if (check)
	{
		theConfig.bUDisk_Massboot = TRUE;
	} 
	else
	{
		theConfig.bUDisk_Massboot = FALSE;
...
void CMainFrame::OnCheckEraseFileSize() 
{
	// TODO: Add your control notification handler code here
	BOOL check = FALSE;
	check = ((CButton *)m_DlgBar.GetDlgItem(IDC_CHECK_ERASE_FILE_SIZE))->GetCheck();
...
void CMainFrame::OnCheckNoNeedErase() 
{
	// TODO: Add your control notification handler code here
	BOOL check = FALSE;
	check = ((CButton *)m_DlgBar.GetDlgItem(IDC_CHECK_NO_NEED_ERASE))->GetCheck();
