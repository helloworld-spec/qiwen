...
void CPageDownload::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_LIST_DOWNLOAD_SPI_IMG, m_download_spi_img_list);
	//}}AFX_DATA_MAP
}
...
BOOL CPageDownload::get_config_data(CConfig &config)
{
	...
	for(i = 0; i < count; i++)
	{
		...
		// get download SPI IMG data
	count = m_download_spi_img_list.GetItemCount();//获取下载镜像文件的个数
	config.download_spi_img_count = count;
	config.download_spi_img_data = new T_DOWNLOAD_SPIFLASH_IMG[count];//分配内存
	for(i = 0; i < count; i++)
	{
		str = m_download_spi_img_list.GetItemText(i, 1);//是否比较
		config.download_spi_img_data[i].bCompare = (theApp.GetString(IDS_DOWNLOAD_YES) == str) ? TRUE : FALSE;
		
		m_download_spi_img_list.GetItemText(i, 2, config.download_spi_img_data[i].pc_path, MAX_PATH);
	}
...
*/
	...
	//set download SPI IMG list
	for(i = 0; i < config.download_spi_img_count; i++)
	{
		str.Format(_T("%d"), i+1);
		m_download_spi_img_list.InsertItem(i, str);
		
		if(config.download_spi_img_data[i].bCompare)//比较
		{
			m_download_spi_img_list.SetItemText(i, 1, theApp.GetString(IDS_DOWNLOAD_YES));
		}
		else
		{
			m_download_spi_img_list.SetItemText(i, 1, theApp.GetString(IDS_DOWNLOAD_NO));
		}
		
		m_download_spi_img_list.SetItemText(i, 2, config.download_spi_img_data[i].pc_path);//pc
	}
...
BOOL CPageDownload::OnInitDialog() 
{
	...
	ListView_SetExtendedListViewStyle(m_download_spi_img_list.m_hWnd, LVS_EX_GRIDLINES | LVS_EX_FULLROWSELECT);
	...
	str = theApp.GetString(IDS_DOWNLOAD_NUMBER);
	m_download_spi_img_list.InsertColumn(0, str, LVCFMT_LEFT, 40);
	
	str = theApp.GetString(IDS_DOWNLOAD_COMPARE);
	m_download_spi_img_list.InsertColumn(1, str, LVCFMT_LEFT, 60);
	
	str.Format(_T("%s|%s|"), theApp.GetString(IDS_DOWNLOAD_YES), theApp.GetString(IDS_DOWNLOAD_NO));
    m_download_spi_img_list.SetSubItemPopItem(1, T_POP_CMBBOX, str);
	
	str = theApp.GetString(IDS_DOWNLOAD_PC_PATH);
	m_download_spi_img_list.InsertColumn(2, str, LVCFMT_LEFT, 300);
	str.Format(_T("%s|"), theApp.GetString(IDS_DOWNLOAD_FILE));
    m_download_spi_img_list.SetSubItemPopItem(2, T_POP_MENU, str);
...
void CPageDownload::SetupDisplay()
{
	...
	str = theApp.GetString(IDS_DOWNLOAD_SPI_IMG);
	GetDlgItem(IDC_STATIC_DOWNLOAD_SPI_IMG)->SetWindowText(str);
}
