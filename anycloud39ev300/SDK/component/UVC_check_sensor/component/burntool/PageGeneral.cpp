...
int ctl_array[45] = 
{
	IDC_STATIC_k1,
	IDC_CHECK_MAC_ADDR,
	IDC_CHECK_FORCE_WRITE_MAC_ADDR,
	IDC_MACADDR_RESET,
	IDC_STATIC_MAC_START_NAME,
	IDC_EDIT_MAC_START_ADDR_HIGH,
	IDC_STATIC_MAC_START_ADDR,
	IDC_EDIT_MAC_START_ADDR_LOW,
	IDC_STATIC_MAC_END_NAME,
	IDC_EDIT_MAC_END_ADDR_HIGH,
	IDC_STATIC_MAC_END_ADDR,
	IDC_EDIT_MAC_END_ADDR_LOW,
	IDC_STATIC_k2,
	IDC_CHECK_SEQUENCE_ADDR,
	IDC_CHECK_FORCE_WRITE_SEQUENCE_ADDR,
	IDC_STATIC_SERIAL_START_NAME,
	IDC_EDIT_SEQUENCE_START_ADDR_HIGH,
	IDC_STATIC_BT,
	IDC_CHECK_BT_ADDR,
	IDC_CHECK_FORCE_WRITE_BT_ADDR,
	IDC_STATIC_START_BT_ADDR,
	IDC_EDIT_BT_START_ADDR,
	IDC_STATIC_END_BT_ADDR,
	IDC_EDIT_BT_END_ADDR,
	IDC_STATIC_a1,
	IDC_STATIC_a2,
	IDC_EDIT_PROD_RUN_ADDR,
	IDC_STATIC_PATH_PRODUCER,
	IDC_EDIT_PATH_PRODUCER,
	IDC_BTN_BROWSE_PRODUCER,
	IDC_STATIC_PATH_NANDBOOT,
	IDC_EDIT_PATH_NANDBOOT,
	IDC_BTN_BROWSE_NANDBOOT,
	IDC_STATIC_a3,
	IDC_EVENT_TIME, 
	IDC_STATIC_PID,
	IDC_EDIT_PID,
...
BEGIN_MESSAGE_MAP(CPageGeneral, CPropertyPage)
	...
	ON_BN_CLICKED(IDC_CHECK_FORCE_WRITE_BT_ADDR, OnCheckForceWriteBtAddr)
	ON_EN_CHANGE(IDC_EDIT_BT_START_ADDR, OnChangeEditBtStartAddr)
	ON_EN_CHANGE(IDC_EDIT_BT_END_ADDR, OnChangeEditBtEndAddr)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
*/
	{	
		...
		*/
		if (theConfig.chip_type == CHIP_39XX_H2 || theConfig.chip_type == CHIP_39XX_H240)
		{
			...
			((CButton *)GetDlgItem(IDC_STATIC_BT))->ShowWindow(FALSE);
			((CButton *)GetDlgItem(IDC_CHECK_BT_ADDR))->ShowWindow(FALSE);
			((CButton *)GetDlgItem(IDC_CHECK_FORCE_WRITE_BT_ADDR))->ShowWindow(FALSE);
			GetDlgItem(IDC_STATIC_START_BT_ADDR)->ShowWindow(FALSE);
			GetDlgItem(IDC_STATIC_END_BT_ADDR)->ShowWindow(FALSE);
			GetDlgItem(IDC_EDIT_BT_START_ADDR)->ShowWindow(FALSE);
			GetDlgItem(IDC_EDIT_BT_END_ADDR)->ShowWindow(FALSE);
		...
		{		
			...
			((CButton *)GetDlgItem(IDC_STATIC_BT))->ShowWindow(TRUE);
			((CButton *)GetDlgItem(IDC_CHECK_BT_ADDR))->ShowWindow(TRUE);
			((CButton *)GetDlgItem(IDC_CHECK_BT_ADDR))->EnableWindow(TRUE);
			((CButton *)GetDlgItem(IDC_CHECK_FORCE_WRITE_BT_ADDR))->ShowWindow(TRUE);
			GetDlgItem(IDC_STATIC_START_BT_ADDR)->ShowWindow(TRUE);
			GetDlgItem(IDC_STATIC_END_BT_ADDR)->ShowWindow(TRUE);
			GetDlgItem(IDC_EDIT_BT_START_ADDR)->ShowWindow(TRUE);
			GetDlgItem(IDC_EDIT_BT_END_ADDR)->ShowWindow(TRUE);
	...
	{
		check = ((CButton *)GetDlgItem(IDC_CHECK_BT_ADDR))->GetCheck();	
		if (check)
		{
			GetDlgItem(IDC_EDIT_BT_START_ADDR)->EnableWindow(TRUE);//序列号
			GetDlgItem(IDC_EDIT_BT_END_ADDR)->EnableWindow(TRUE);//
			GetDlgItem(IDC_CHECK_FORCE_WRITE_BT_ADDR)->EnableWindow(TRUE);//
		}
		else
		{	
			GetDlgItem(IDC_EDIT_BT_START_ADDR)->EnableWindow(FALSE);//
			GetDlgItem(IDC_EDIT_BT_END_ADDR)->EnableWindow(FALSE);//
			GetDlgItem(IDC_CHECK_FORCE_WRITE_BT_ADDR)->EnableWindow(FALSE);//
		}
	}
}
...
//	_tcsncpy(config.path_bios, str, MAX_PATH);
    ...
    if (check)
    {
		theConfig.BT_addr_flag = TRUE;
		check = ((CButton *)GetDlgItem(IDC_CHECK_FORCE_WRITE_BT_ADDR))->GetCheck();	
		if (check)
		{
			theConfig.fore_write_BT_addr = TRUE;//强制写
		}
		else
		{
			theConfig.fore_write_BT_addr = FALSE;//非
		}
    }
    else
    {
		theConfig.BT_addr_flag = FALSE;//非
		theConfig.fore_write_BT_addr = FALSE;//非
    }
	...
	GetDlgItemText(IDC_EDIT_BT_START_ADDR, str);
	_tcsncpy(surBuf, str, MAX_BT_ADDR_LEN);
	theConfig.lower_to_upper(surBuf, dstBuf);
	_tcsncpy(config.blue_start_addr, dstBuf, MAX_BT_ADDR_LEN);
	
	GetDlgItemText(IDC_EDIT_BT_END_ADDR, str);
	_tcsncpy(surBuf, str, MAX_BT_ADDR_LEN);
	theConfig.lower_to_upper(surBuf, dstBuf);
	_tcsncpy(config.blue_end_addr, dstBuf, MAX_BT_ADDR_LEN);
	
	if (_tcscmp (config.blue_start_addr, config.blue_cur_addr) >= 0)
	{
		_tcsncpy(config.blue_cur_addr, config.blue_start_addr, MAX_BT_ADDR_LEN);//当前低位
	}
...
*/
	...
	//蓝牙地址
	((CButton *)GetDlgItem(IDC_CHECK_BT_ADDR))->SetCheck(theConfig.BT_addr_flag);
	if (theConfig.BT_addr_flag)
	{
		GetDlgItem(IDC_CHECK_FORCE_WRITE_BT_ADDR)->EnableWindow(TRUE);
		((CButton *)GetDlgItem(IDC_CHECK_FORCE_WRITE_BT_ADDR))->SetCheck(theConfig.fore_write_BT_addr);
	}
	else
	{
		GetDlgItem(IDC_CHECK_FORCE_WRITE_BT_ADDR)->EnableWindow(FALSE);
		((CButton *)GetDlgItem(IDC_CHECK_FORCE_WRITE_BT_ADDR))->SetCheck(FALSE);
		theConfig.fore_write_BT_addr = FALSE;
	}
	GetDlgItem(IDC_EDIT_BT_START_ADDR)->EnableWindow(theConfig.BT_addr_flag);//
	GetDlgItem(IDC_EDIT_BT_END_ADDR)->EnableWindow(theConfig.BT_addr_flag);//
	
	str.Format(_T("%s"), theConfig.blue_start_addr);
	GetDlgItem(IDC_EDIT_BT_START_ADDR)->SetWindowText(str);
	str.Format(_T("%s"), theConfig.blue_end_addr);
	GetDlgItem(IDC_EDIT_BT_END_ADDR)->SetWindowText(str);
...
void CPageGeneral::SetupDisplay()
{
	...
	//蓝牙地址
	str = theApp.GetString(IDS_GENERAL_BT_ADDR);
	GetDlgItem(IDC_CHECK_BT_ADDR)->SetWindowText(str);
	str = theApp.GetString(IDS_GENERAL_FORCE_WRITE_BT_ADDR);
	GetDlgItem(IDC_CHECK_FORCE_WRITE_BT_ADDR)->SetWindowText(str);
	str = theApp.GetString(IDS_GENERAL_BT_START_ADDR);
	GetDlgItem(IDC_STATIC_START_BT_ADDR)->SetWindowText(str);
	str = theApp.GetString(IDS_GENERAL_BT_END_ADDR);
	GetDlgItem(IDC_STATIC_END_BT_ADDR)->SetWindowText(str);
...
void CPageGeneral::OnCheckBtAddr() 
{
	// TODO: Add your control notification handler code here
	BOOL check = FALSE;
	CString str;
    ...
    {
		GetDlgItem(IDC_CHECK_FORCE_WRITE_BT_ADDR)->EnableWindow(TRUE);
        GetDlgItem(IDC_EDIT_BT_START_ADDR)->EnableWindow(TRUE);
		GetDlgItem(IDC_EDIT_BT_END_ADDR)->EnableWindow(TRUE);
    }
    else
    {
...
void CPageGeneral::OnCheckForceWriteBtAddr() 
{
	// TODO: Add your control notification handler code here
	
}

BOOL CPageGeneral::BT_iserror(UINT nID, UINT len, CString str, TCHAR *pBuf)
{
	UINT i = 0;
	UINT s = 0;
	BOOL falg = TRUE;
...
void CPageGeneral::OnChangeEditBtStartAddr() 
{
	// TODO: If this is a RICHEDIT control, the control will not
	// send this notification unless you override the CPropertyPage::OnInitDialog()
	// function and call CRichEditCtrl().SetEventMask()
	...
	{		
		nLen = str.GetLength();
		if (nLen > MAX_BT_ADDR_LEN)
		{
			AfxMessageBox(_T("blue addr len more than 12"), MB_OK);
			return;
		}
		
		if (!BT_iserror( IDC_EDIT_BT_START_ADDR, nLen, str,  theConfig.blue_start_addr))
		{
			falg = FALSE;
		}
		...
		if(falg == TRUE)
		{
			...
			else
			{
				...
				str = str.Left(nLen-1);
				GetDlgItem(IDC_EDIT_BT_START_ADDR)->SetWindowText(str);
				((CEdit*)GetDlgItem(IDC_EDIT_BT_START_ADDR))->SetSel( nLen-1, nLen-1, FALSE );
			}
		}
	}
}

void CPageGeneral::OnChangeEditBtEndAddr() 
{
	// TODO: If this is a RICHEDIT control, the control will not
	// send this notification unless you override the CPropertyPage::OnInitDialog()
	// function and call CRichEditCtrl().SetEventMask()
	...
	{		
		...
		if (!BT_iserror( IDC_EDIT_BT_END_ADDR, nLen, str,  theConfig.blue_end_addr))
		{
			falg = FALSE;
		}
		...
		if(falg == TRUE)
		{
			...
			else
			{
				...
				str = str.Left(nLen-1);
				GetDlgItem(IDC_EDIT_BT_END_ADDR)->SetWindowText(str);
				((CEdit*)GetDlgItem(IDC_EDIT_BT_END_ADDR))->SetSel( nLen-1, nLen-1, FALSE );
			}
		}
	}
}
