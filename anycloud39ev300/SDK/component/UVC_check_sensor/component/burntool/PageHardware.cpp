...
void CPageHardware::DoDataExchange(CDataExchange* pDX)
{
	...
	DDV_MinMaxUInt(pDX, m_feq, 100, 440);
	//}}AFX_DATA_MAP
}
...
BEGIN_MESSAGE_MAP(CPageHardware, CPropertyPage)
	...
	ON_BN_CLICKED(IDC_RADIO_SPI_SD, OnRadioSpiSd)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
{
	IDC_STATIC_SD_RAM,        //1
	IDC_RADIO_SDRAM_MODE,     //2
	IDC_RADIO_DDR2_16_MODE,   //3
	IDC_RADIO_DDR2_32_MODE,   //4

	IDC_RADIO_MCP_MODE,       //5
	IDC_RADIO_DDR_MODE,       //6
	IDC_RADIO_DDR_16_MOBILE,  //7
	IDC_RADIO_DDR_MODE32,     //8
	IDC_RADIO_DDR_32_MOBILE,  //9

	IDC_STATIC_DDR_Freq,      //10
	IDC_EDIT_FREQ,            //11
	IDC_STATIC_Mhz,           //12
	IDC_STATIC_RAM_SIZE,      //13
	IDC_RAM_SIZE_LIST,        //14
	IDC_STATIC_RAM_ROW,       //15
	IDC_RAM_ROW_LIST,         //16
	IDC_BUTTON_IMPORT_RAM_CONFIG,//17
	IDC_STATIC_RAM_BANK,         //18
	IDC_RAM_BANK_LIST,           //19
	IDC_STATIC_RAM_COLUMN,       //20
	IDC_RAM_COLUMN_LIST,         //21
	IDC_BUTTON_EXPORT_RAM_CONFIG,//22

	IDC_STATIC_BURN_MODE,        //23
	IDC_RADIO_SFLASH,            //24
	IDC_RADIO_SPI_NANDFLASH,     //25
	IDC_RADIO_SPI_SD,            //26
	IDC_RADIO_SD,                //27
	IDC_RADIO_DEBUG,             //28
	IDC_RADIO_JTAG,              //29
	...
	IDC_RADIO_NANDFLASH,         //32
	IDC_UPDATESELF,              //33
	IDC_CHECK_UDISK_UPDATE,      //34

	IDC_STATIC_BURNED,           //35
	IDC_BURNED_NONE,             //36
	IDC_BURNED_GPIO,             //37
	IDC_GPIO_PIN_NUM,            //38
	IDC_BURNED_RTC_WAKUP,        //39
	IDC_BURNED_RESET,            //40
	IDC_BURNED_LVH,              //41
	IDC_BURNED_PULLUP,           //42
	IDC_GPIO_PULLUP_EN,          //43
	IDC_BURNED_RTC_LVH,          //44
	IDC_BURNED_POWER_OFF,        //45
	IDC_BURNED_LVL,              //46
	IDC_BURNED_PULLDOWN,         //47
	IDC_GPIO_PULLDOWN_EN,        //48
	IDC_BURNED_RTC_LVL,          //49
     
	IDC_GROUP_GPIO,               //50
	IDC_STATIC_CHIP_SELECT,       //51
	IDC_COMBO_CHIP_SEL,           //52
	IDC_STATIC_CE2_GPIO,          //53
	IDC_EDIT_GPIO_CE2,            //54
	IDC_STATIC_CE3_GPIO,          //55
	IDC_EDIT_GPIO_CE3,            //56
	IDC_CHECK_GPIO_PIN_SELECT,    //57
	IDC_EDIT_GPIO_PIN             //58
};
...
BOOL CPageHardware::get_config_data(CConfig &config)
{
	...
	if(str == _T("AK_39XX"))//CHIP_39XX_H2
	{
		config.chip_type = CHIP_39XX_H240;
		config.bUboot = true;
	}
	else if(str == _T("AK_10XX"))//CHIP_10XX_T
	{
		config.chip_type = CHIP_10XX_T;
		config.bUboot = true;
	}
	else
	{
	...
	if (((CButton *)GetDlgItem(IDC_RADIO_SFLASH))->GetCheck())
	{
		theConfig.burn_mode = E_CONFIG_SFLASH;//spi模式
	}
	else if (((CButton *)GetDlgItem(IDC_RADIO_SPI_NANDFLASH))->GetCheck())
	{
		theConfig.burn_mode = E_CONFIG_SPI_NAND;//spi模式
	}
	else if (((CButton *)GetDlgItem(IDC_RADIO_SD))->GetCheck())
	{
		theConfig.burn_mode = E_CONFIG_SD;//spi模式
	}
	else if (((CButton *)GetDlgItem(IDC_RADIO_SPI_SD))->GetCheck())
	{
		theConfig.burn_mode = E_CONFIG_SPI_SD;//spi模式
	}
	else if (((CButton *)GetDlgItem(IDC_RADIO_JTAG))->GetCheck())
	{
		theConfig.burn_mode = E_CONFIG_JTAG;//spi模式
	}
	else if (((CButton *)GetDlgItem(IDC_RADIO_DEBUG))->GetCheck())
	{
		theConfig.burn_mode = E_CONFIG_DEBUG;//spi模式
	}
	else
	{
		theConfig.burn_mode = E_CONFIG_SFLASH;//spi模式
...
BOOL CPageHardware::set_config_item(CConfig &config)
{
	...
	else if(RAM_TYPE_DDR16 == config.ram_param.type)
	{
		...
		if (config.chip_type == CHIP_39XX_H2
			|| config.chip_type == CHIP_39XX_H240)//CHIP_980X
		{
	...
	else if(RAM_TYPE_DDR32 == config.ram_param.type)//RAM_TYPE_DDR32
	{
		...
		GetDlgItem(IDC_STATIC_DDR_Freq)->ShowWindow(TRUE);
		GetDlgItem(IDC_STATIC_Mhz)->ShowWindow(TRUE);
		if (config.chip_type == CHIP_39XX_H2 || config.chip_type == CHIP_39XX_H240)//CHIP_980X
		{
	...
	else if(RAM_TYPE_DDR2_32 == config.ram_param.type)//RAM_TYPE_DDR2_32
	{
		...
		GetDlgItem(IDC_STATIC_DDR_Freq)->ShowWindow(TRUE);
		GetDlgItem(IDC_STATIC_Mhz)->ShowWindow(TRUE);
		if (config.chip_type == CHIP_39XX_H2 || config.chip_type == CHIP_39XX_H240)
		{
			GetDlgItem(IDC_BUTTON_IMPORT_RAM_CONFIG)->ShowWindow(TRUE);
			GetDlgItem(IDC_BUTTON_EXPORT_RAM_CONFIG)->ShowWindow(TRUE);
	...
	if (theConfig.burn_mode == E_CONFIG_SFLASH)
	{
		((CButton *)GetDlgItem(IDC_RADIO_SFLASH))->SetCheck(1);
	}
	else if (theConfig.burn_mode == E_CONFIG_SPI_NAND)
	{
		((CButton *)GetDlgItem(IDC_RADIO_SPI_NANDFLASH))->SetCheck(1);
	}
	else if (theConfig.burn_mode == E_CONFIG_SD)
	{
		((CButton *)GetDlgItem(IDC_RADIO_SD))->SetCheck(1);
	}
	else if (theConfig.burn_mode == E_CONFIG_SPI_SD)
	{
		((CButton *)GetDlgItem(IDC_RADIO_SPI_SD))->SetCheck(1);
	}
	else if (theConfig.burn_mode == E_CONFIG_JTAG)
	{
		((CButton *)GetDlgItem(IDC_RADIO_JTAG))->SetCheck(1);
	}
	else if (theConfig.burn_mode == E_CONFIG_DEBUG)
	{
		((CButton *)GetDlgItem(IDC_RADIO_DEBUG))->SetCheck(1);
	}
	else
	{
		((CButton *)GetDlgItem(IDC_RADIO_SFLASH))->SetCheck(1);
...
//	{
	...
	//	|| config.chip_type == CHIP_11XX || config.chip_type == CHIP_10XX_T)
	if (config.chip_type == CHIP_10XX_T)
	{
...
void CPageHardware::OnSelchangeChipTypeList() 
{
	...
	*/
	if (str == _T("AK_10XX"))//CHIP_10XX_T
	{
		theConfig.chip_type = CHIP_10XX_T;
	}
	else if (str == _T("AK_39XX"))//CHIP_39XX_H2
	{
...
void CPageHardware::select_change_chip_type(CString strChipType)
{
	...
	if (strChipType == _T("AK_10XX"))
	{
		...
		if ((rect_Flag.top + 59) < rect.top)
		{
			for(int j = 22; j < 49; j++)
			{
				...
				pWnd->GetWindowRect(&rect);
				ScreenToClient(&rect);
				pWnd->SetWindowPos( NULL,rect.left,rect.top - 120,0,0,SWP_NOZORDER | SWP_NOSIZE ); //移动位置
			}
		}
		...
		//当是10或11芯片时，自升级可用
		//GetDlgItem(h_ctl_array[33])->ShowWindow(TRUE);
		//GetDlgItem(h_ctl_array[32])->ShowWindow(FALSE);
	...
	else
	{
		...
		pWnd->GetWindowRect(&rect);
		//当是10或11芯片时，自升级不可用
		//GetDlgItem(h_ctl_array[33])->ShowWindow(FALSE);
		//theConfig.bUpdateself = 0;
		//GetDlgItem(h_ctl_array[32])->ShowWindow(TRUE);
...
void CPageHardware::OnRadioSpiSd() 
{
	// TODO: Add your control notification handler code here
	BOOL check = FALSE;
	...
	// TODO: Add your control notification handler code here
	check = ((CButton *)GetDlgItem(IDC_RADIO_SPI_SD))->GetCheck();
	if (check)
	{
		theConfig.burn_mode = E_CONFIG_SPI_SD;//spi模式
	}
