...
BOOL CPageSpiflash::get_config_data(CConfig &config)
{
	...
	for(int i = 0; i < count; i++)
	{
		...
		//{
			config.spiflash_parameter[i].reserved1 = config.hex2int(T2A(str));
		//}
		//else
		//{
...
BOOL CPageSpiflash::set_config_item(CConfig &config)
{
	...
	{		
		...
		//if (theConfig.planform_tpye == E_ROST_PLANFORM)
		//{
			str.Format(_T("0x%02x"), config.spiflash_parameter[i].reserved1);//disk erase size
...
BOOL CPageSpiflash::OnInitDialog() 
{
	...
	//if (theConfig.planform_tpye == E_ROST_PLANFORM)
	//{
		m_spiflash_param_list.InsertColumn(10, _T("ex_flag"), LVCFMT_LEFT, 90);//reserved1  以K对齐 
	//}
	//else
	//{
