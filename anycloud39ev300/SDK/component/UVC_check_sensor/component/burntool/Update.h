...
{
	BYTE head_info[8];			//文件头信息，固定为"anyka_V2"
	BYTE usb_mode;
	BYTE burn_mode;
	BYTE chip_type;
	BYTE planform_tpye;
	UINT partition_count;		//format信息个数
    UINT partition_offset;		//format信息偏移量
	UINT spinand_count;			//nand信息个数
    UINT spinand_offset;			//nand信息偏移量
	...
	UINT fs_img_count;			//img文件个数
    UINT fs_img_offset;	        //img信息偏移量
	UINT bin_file_count;		//bin文件个数
    UINT bin_file_offset;		//bin信息偏移量
	UINT spi_img_file_count;		//spi镜像文件个数
    UINT spi_img_file_offset;		//spi镜像信息偏移量
...
typedef struct tagFileInfo
{
	...
	CHAR file_name[8];			//文件名（存储在设备上文件名）
	UINT check_sum;//
}T_FILE_INFO;
