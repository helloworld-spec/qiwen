...
static UINT reg_addr_ddr2_sunh2[] =
{
	0x08000004,
	0x66668888,
	0x21000078,
...
static UINT reg_value_ddr2_sunh2[] =
{
	0x011253dc,
	0x000000c8,
	0x00144020,
...
static UINT reg_addr_ddr2_sunh240[] =
{
	0x08000004,
	0x66668888,
	0x21000078,
...
static UINT reg_value_ddr2_sunh240[] =
{
	0x011253dc,
	0x000000c8,
	0x00009041,
	...
	0x000000c8,
	0x00000001,
	0x0007a120,//linux都是500ms, RTOS是200ms
	0x88888888,
		
};

//RTOS与linux不同之处是DDR2 配置完后的等待时间用于Calibration（自校准），linux都是500ms, RTOS是200ms
static UINT reg_addr_ddr2_sunh240_rtos[] =
{
	0x08000004,
	0x66668888,
	0x21000078,
...
static UINT reg_value_ddr2_sunh240_rtos[] =
{
	0x011253dc,
	0x000000c8,
	0x00009041,
	...
	0x000000c8,
	0x00000001,
	0x00030d40,//linux都是500ms, RTOS是200ms
	0x88888888,
...
BOOL get_UI_data(UINT *data)
{
	...
	for(i=0; i < stRamData.uRegCount; ++i)
	{
		...
		*/
		//if((theConfig.chip_type == CHIP_39XX || theConfig.chip_type == CHIP_39XX_H2 || theConfig.chip_type == CHIP_39XX_H240) && 0x21000000 == stRamData.ram[i].reg_addr)  //JUST FOR AK98
		if((theConfig.chip_type == CHIP_39XX_H2 || theConfig.chip_type == CHIP_39XX_H240) && 0x21000000 == stRamData.ram[i].reg_addr)  //JUST FOR AK98
		{
			*data = stRamData.ram[i].reg_value;
			return TRUE;
		}
...
UINT config_ram_param(T_RAM_REG *ram)
{
	...
	if (CHIP_39XX_H2 == theConfig.chip_type || theConfig.chip_type == CHIP_39XX_H240)
	{
	...
	*/
	if (CHIP_39XX_H2 == theConfig.chip_type || theConfig.chip_type == CHIP_39XX_H240)
    {
		 ...
		 else if(RAM_TYPE_DDR2_16 == ram_type || RAM_TYPE_DDR2_32 == ram_type) 
		 {
			 if (0 == regCnt)
			 {
				 if (CHIP_39XX_H2 == theConfig.chip_type)
				 {
					 regCnt = sizeof(reg_value_ddr2_sunh2) / sizeof(UINT);
					 
					 for(i=0; i<regCnt; i++)
					 {
						 ram[i].reg_addr = reg_addr_ddr2_sunh2[i];
						 ram[i].reg_value = reg_value_ddr2_sunh2[i];

						 //界面频率
						 if (ram[i].reg_addr == 0x08000004)
						 {
							 T_U32 ret = 0;
							 ret = ram[i].reg_value;
							 ret = ret & ~(0x3FFF);
							 ...
							 ret = ret | (1 << 9);
							 ret = ret | (1 << 8);
							 ret = ret | (theConfig.m_freq/2);  //OD = 1, N = 3
							 ram[i].reg_value = ret;
						 }
					 }
				 }
				 else if (theConfig.chip_type == CHIP_39XX_H240)
				 {
					 regCnt = sizeof(reg_value_ddr2_sunh240) / sizeof(UINT);
					 
					 for(i=0; i<regCnt; i++)
					 {
						 if (theConfig.planform_tpye == E_LINUX_PLANFORM)
						 {
							 ram[i].reg_addr = reg_addr_ddr2_sunh240[i];
							 ram[i].reg_value = reg_value_ddr2_sunh240[i];
						 } 
						 else
						 {
							 ram[i].reg_addr = reg_addr_ddr2_sunh240_rtos[i];
						     ram[i].reg_value = reg_value_ddr2_sunh240_rtos[i];
						 }
