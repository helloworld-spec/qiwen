...
#define    DOWNLOAD_BIN_FILENAME_SIZE	    (6)
/*
typedef enum
{
    MODE_NEWBURN = 1,
...
typedef struct
{
    UINT data_length;        //镜像文件大小       
    UINT partition_size;     //分区大小(单位字节)
    char  r_w_flag;           //only read or write
    char  bCheck;
    char  rev1;
    char  rev2; 
    char  name[8];            //分区名
...
typedef struct
{
	CHAR  name[DOWNLOAD_BIN_FILENAME_SIZE];	//盘符名
	UINT  Size;                             //分区大小
    UINT  ld_addr;	                        //动行地址
    CHAR  type;				                //分区属性类型：data , bin, fs
    CHAR  r_w_flag;			                //	r_w or onlyread
	CHAR  hidden_flag;			            //	hidden_flag or not
    CHAR  backup;				            //是否备份
	CHAR  check;                            //比较
	CHAR  mtd_idex;                         //mtd的idex
	CHAR  fs_type;                          //文件系统的类型
	CHAR  rev1;                             //保留
    UINT  rev2;                             //保留
}T_PARTITION_INFO;
 ...
 * @retval BT_SUCCESS or BT_FAIL
 */
TRANSC_API UINT   BT_CreatePartion(UINT nID, T_PARTITION_INFO *PartionInfo, UINT nNumPartion);
...
TRANSC_API UINT   BT_Is_Bind_channels_flag(BOOL bind_channel_flag);
 ...
 * @retval BT_SUCCESS or BT_FAIL
 */
TRANSC_API UINT   BT_SetReg(HANDLE hHandle,  UINT regAddr, UINT regValue);

TRANSC_API UINT   BT_Erase_Partition_Size(UINT nID, UINT start_block, UINT end_block);

TRANSC_API UINT BT_hold_up_usb(char *pstrUSBName, char *HubPort);

TRANSC_API UINT   BT_Setvalue_forupload_orspimake(UINT nID, UINT value);

TRANSC_API UINT   BT_Set_Ex_Parameter(UINT nID, UINT boot_blcok_size);

TRANSC_API UINT  BT_Download_spi_img(UINT nID, T_DOWNLOAD_SPI_IMG *pDownload_spiImg);
//{{AFX_INSERT_LOCATION}}
