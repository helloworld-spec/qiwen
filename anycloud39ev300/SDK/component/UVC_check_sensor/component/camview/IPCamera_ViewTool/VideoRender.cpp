...
extern PICYUVDATA *g_pre_pstYuvData;
...
int CVideoRender::ThreadRender()
{
	...
	if (g_stop_play_flag)
	{
		if (g_pre_pstYuvData != NULL)
		{
			m_dePic.clear();
			pYuvData = g_pre_pstYuvData;
	...
	else
	{
		...
		if (g_pre_pstYuvData != NULL && g_heigth > 0)
		{
			if (g_pre_pstYuvData->pYData != NULL)
			{
				memcpy(g_pre_pstYuvData->pYData, pYuvData->pYData, pYuvData->nYlineSize * g_heigth);
			}
			g_pre_pstYuvData->nYlineSize = pYuvData->nYlineSize;

			if (g_pre_pstYuvData->pUData != NULL)
			{
				memcpy(g_pre_pstYuvData->pUData, pYuvData->pUData, pYuvData->nUlineSize * g_heigth >> 1);
			}
			g_pre_pstYuvData->nUlineSize = pYuvData->nUlineSize;

			if (g_pre_pstYuvData->pVData != NULL)
			{
				memcpy(g_pre_pstYuvData->pVData, pYuvData->pVData, pYuvData->nVlineSize * g_heigth >> 1);
			}
			g_pre_pstYuvData->nVlineSize = pYuvData->nVlineSize;
			g_pre_pstYuvData->sPresentationTime = pYuvData->sPresentationTime;
		}
	}
	LOCAL_ALOCK_END;
