...
#define ISP_DRV_LIB_VER		"ak_isp_drv_lib V0.2.15"
/***************************************************************************
...
/*typedef enum awb_stat_param_type {
...
typedef struct  ak_isp_awb_algo {
    ...
    short calc_b_gain;
    short calc_g_gain;
    int  current_stat_param_index ; //两个取值，一个为正常情况下参数，一个为非
                                    //正常情况的下的参数
    int  status_used ;
}AK_ISP_AWB_ALGO;
...
typedef struct ak_isp_ae_info
{
    T_S32  a_gain;                     	//模拟增益的值
    T_S32  d_gain;                    	//数字增益的值
    T_S32  isp_d_gain;                	//isp数字增益的值
    T_S32  exp_time;                	//曝光时间的值
