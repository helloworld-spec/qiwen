...
static unsigned long s_sdmmc_clok      = 0;
...
static void set_data_reg(unsigned long len,unsigned long blk_size,unsigned char bus_mode,unsigned char dir);
static bool sd_rx_data_cpu(unsigned char buf[], unsigned long len);
static bool sd_tx_data_cpu(unsigned char buf[], unsigned long len);
static bool sd_tx_data_cpu(unsigned char buf[], unsigned long len);
static void set_sdmmc_if(void);
static bool sd_abort_trans(unsigned char func_nbr);
...
static void set_sdmmc_if(void)
{
    ...
    s_L2Select  = ADDR_MMC_SD;
    s_PinSelect = ePIN_AS_MMCSD;
}
 ...
 * @return void
 */
void set_interface(T_eCARD_INTERFACE cif)
{
    if(INTERFACE_NOT_SD != cif)
    {
        ...
        {
            //set_sdio_if(); 
	        akprintf(C3, M_DRVSYS, "SD set interface ERR\n");
 ...
 * @return void
 */
void set_clock(unsigned long clk, unsigned long asic, bool pwr_save)
{
...
void sd_cfg_buf(unsigned char buf_mode)
{
 ...
 */
bool send_cmd( unsigned char cmd_index, unsigned char resp ,unsigned long arg)
{
 ...
 * @return void
 */
void get_long_resp(unsigned long resp[])
{
    unsigned long resp_value=0;
 ...
 */
bool sd_trans_data_dma(unsigned long ram_addr,unsigned long size,unsigned char dir)
{
    ...
    //set data reg
    set_data_reg(size, g_pSdDevice->ulDataBlockLen, g_pSdDevice->enmBusMode, dir);
 ...
 * @retval  false: transfer failed
 */
bool sd_trans_data_cpu(unsigned long ram_addr,unsigned long size,unsigned char dir)
{  
    bool ret = true;
    
	//set data reg
	if(size < g_pSdDevice->ulDataBlockLen)  //zoutx add 判断blocklen是否大于size
	{
		set_data_reg(size, size, g_pSdDevice->enmBusMode, dir); //zoutx: 如果大于则blocklen调成和size一样大		 
	}
	else
	{
    	set_data_reg(size, g_pSdDevice->ulDataBlockLen, g_pSdDevice->enmBusMode, dir);
    ...
    if (SD_DATA_CTL_TO_HOST == dir)
    {    
        if(!sd_rx_data_cpu((unsigned char*)ram_addr, size))
        {
            ret = false;
        }
    }
    ...
    {
        if(!sd_tx_data_cpu((unsigned char*)ram_addr, size))
        {
            ret = false;
        }
    }
...
*/
bool sd_read_multi_byte(unsigned long arg,unsigned char data[],unsigned long size)
{
    ...
    //config data register
	 if( size < g_pSdDevice->ulFunBlockLen[func])  //zoutx: 如果大于则blocklen调成和size一样大		 
	 {
	 	set_data_reg(size, size,g_pSdDevice->enmBusMode, SD_DATA_CTL_TO_HOST);
	 }
	 else
	 {
	 	set_data_reg(size, g_pSdDevice->ulFunBlockLen[func],g_pSdDevice->enmBusMode, SD_DATA_CTL_TO_HOST);
	 }
    //send cmd 53
...
*/
bool sd_write_multi_byte(unsigned long arg,unsigned char data[],unsigned long size)
{
    unsigned char func=0;
    func = (arg>>CMD53_FUN_NUM)&0x7;
	 ...
	 //config data register	 
 	 if( size < g_pSdDevice->ulFunBlockLen[func])  //zoutx add 判断blocklen是否大于size
 	 {
 	 	set_data_reg(size, size, g_pSdDevice->enmBusMode, SD_DATA_CTL_TO_CARD);  //zoutx: 如果大于则blocklen调成和size一样大		 
 	 }
	 else
	 {
		 set_data_reg(size, g_pSdDevice->ulFunBlockLen[func],g_pSdDevice->enmBusMode, SD_DATA_CTL_TO_CARD);		 
	 }		 
...
* @retval  false: read failed
*/
bool sd_read_multi_block(unsigned long arg,unsigned char data[],unsigned long size)
{
    unsigned char func=0;
    func = (arg>>CMD53_FUN_NUM)&0x7;
	 //config INNER BUFFER
...
/**
  ...
  */
bool sd_read_multi_block_L2(unsigned long arg,unsigned char data[],unsigned long size)
{
	unsigned char func=0;
	...
	g_pSdDevice->ulDataBlockLen = g_pSdDevice->ulFunBlockLen[func];
	  
	//zoutx : 其实这里并没有用到DMA， 只是用到L2模式而已
	if( sd_trans_data_dma((unsigned long)data, size, SD_DATA_CTL_TO_HOST) == false)
	{
		akprintf(C3, M_DRVSYS, "sd_trans_data_dma failed.\n");
		return  false; 
	}
...
* @retval  false: write failed
*/
bool sd_write_multi_block(unsigned long arg,unsigned char data[],unsigned long size)
{
    //send cmd 53
    unsigned char func=0;
    func = (arg>>CMD53_FUN_NUM)&0x7;
 ...
 /**
   ...
   * @retval  AK_FALSE: read failed
   */
bool sd_write_multi_block_L2(unsigned long arg,unsigned char data[],unsigned long size)
{
	 //send cmd 53
	 unsigned char func=0;
	 func = (arg>>CMD53_FUN_NUM)&0x7;
	 ...
	 if (!send_cmd( SD_CMD(53), SD_SHORT_RESPONSE, arg))
	 {
 	 ...
 	 g_pSdDevice->ulDataBlockLen = g_pSdDevice->ulFunBlockLen[func];
  	 ...
  	 //zoutx : 其实这里并没有用到DMA， 只是用到L2模式而已
	 if( sd_trans_data_dma((unsigned long)data, size, SD_DATA_CTL_TO_CARD) == false)
	 {
		akprintf(C3, M_DRVSYS, "sd_trans_data_dma failed.\n");
		return  false; 
	 }
