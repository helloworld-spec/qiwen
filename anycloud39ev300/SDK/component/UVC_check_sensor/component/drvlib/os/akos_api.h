...
typedef signed long               T_hTask;
typedef signed long               T_hQueue;
typedef signed long               T_hSemaphore;
typedef signed long               T_hHisr;
typedef unsigned char                T_OPTION;
typedef signed long               T_hMailbox;
typedef signed long               T_hEventGroup;
typedef signed long               T_hTimer;
...
/*@brief:此服务删除一个先前定义的任务。参数Task确定需要删除的任务。在这个任务上挂起的任务恢复时返回适当的错误状态。
  ...
  */
signed long AK_Delete_Task(T_hTask task);
...
/*
  ...
  *@retval AK_INVALID_TASK 表示任务句柄非法  
  */
signed long AK_Suspend_Task(T_hTask task);
...
/************************************************************************/
signed long AK_Resume_Task(T_hTask task);
 ...
 *@retval AK_INVALID_TASK 表示任务句柄无效 */                               
/************************************************************************/
signed long AK_Terminate_Task(T_hTask task);
...
/*                                                                      */
/************************************************************************/
signed long AK_Reset_Task(T_hTask task, unsigned long argc, void *argv);
...
/*                              挂起均在这个状态）                      */
/************************************************************************/
signed long AK_Task_Status(T_hTask task);
...
/************************************************************************/
signed long AK_Delete_Queue(T_hQueue queue);
...
/************************************************************************/
signed long AK_Send_To_Queue(T_hQueue queue, void *message, unsigned long size, unsigned long suspend);
...
/*注意事项  如果在中断等非任务线程中调用此接口，挂起参数设置不是AK_NO_SUSPEND，则不论队列是否为满，都会返回错误 AK_INVALID_SUSPEND*/
/************************************************************************/
signed long AK_Send_To_Front_of_Queue(T_hQueue queue, void *message, unsigned long size, unsigned long suspend);
...
返回值说明  
    ...
    AK_INVALID_POINTER  表示消息指针为NULL
...
/************************************************************************/
signed long AK_Broadcast_To_Queue(T_hQueue queue, void *message, unsigned long size, unsigned long suspend);
...
/*  retval  AK_SUCCESS          表示服务成功
    ...
    retval  AK_QUEUE_DELETE     在任务挂起期间队列被删除                */
/************************************************************************/
signed long AK_Receive_From_Queue(T_hQueue queue, void *message, unsigned long size,
...
/* @param:
    ...
    Suspend [in]如果队列已经包含了一个消息，指定是否挂起调用任务
            ...
            时间间隔值（1 ～ 4294967293）调用任务挂起直到一个队列消息放置，
                                        或者直到指定数量的时钟节拍到时。
    Function[in]进行比较的回调函数。预定比较结果：如果比较结果相同返回true，如果比较结果不同返回false。*/
...
/*注意事项  如果在中断等非任务线程中调用此接口，挂起参数设置不是AK_NO_SUSPEND，
            则不论队列是否为满，都会返回错误 AK_INVALID_SUSPEND*/
/************************************************************************/
signed long AK_Send_Unique_To_Queue(T_hQueue queue, void *message, unsigned long size, 
...
/* @brief:  此项服务放置消息到指定的队列顶端。
   ...
   @param
    ...
    Function[in]进行比较的回调函数。
                预定比较结果：如果比较结果相同返回true，如果比较结果不同返回false。*/
...
/*注意事项  如果在中断等非任务线程中调用此接口，挂起参数设置不是AK_NO_SUSPEND，
            则不论队列是否为满，都会返回错误 AK_INVALID_SUSPEND*/
/************************************************************************/
signed long AK_Send_Unique_To_Front_of_Queue(T_hQueue queue_ptr, void *message, 
...
/*返回值说明    
    ...
    AK_INVALID_QUEUE    表示队列无效*/
/************************************************************************/
signed long AK_Reset_Queue(T_hQueue queue);

// Mailbox API

T_hMailbox AK_Create_Mailbox(T_OPTION suspend_type);

signed long AK_Delete_Mailbox(T_hMailbox mailbox);

signed long AK_Broadcast_To_Mailbox(T_hMailbox mailbox, unsigned long *message, T_OPTION suspend_type);

signed long AK_Receive_From_Mailbox(T_hMailbox mailbox, unsigned long *message, T_OPTION suspend_type);

signed long AK_Send_To_Mailbox(T_hMailbox mailbox, unsigned long *message, T_OPTION suspend_type);
...
/************************************************************************/
signed long AK_Delete_Semaphore(T_hSemaphore semaphore);
...
/************************************************************************/
signed long AK_Obtain_Semaphore(T_hSemaphore semaphore, unsigned long suspend);
...
/************************************************************************/
signed long AK_Release_Semaphore(T_hSemaphore semaphore);
...
/*返回值说明
    ...
    AK_INVALID_SEMAPHORE    表示信号量句柄非法                          */
/************************************************************************/
signed long AK_Reset_Semaphore(T_hSemaphore semaphore, unsigned long initial_count);

//EventGroup API
T_hEventGroup AK_Create_Event_Group(void);

signed long AK_Delete_Event_Group(T_hEventGroup eventgroup);

signed long AK_Retrieve_Events(T_hEventGroup eventgroup, unsigned long requested_events, 
                        T_OPTION operation, unsigned long *retrieved_events, unsigned long suspend);

signed long AK_Set_Events(T_hEventGroup eventgroup, unsigned long event_flags, T_OPTION operation);
...
signed long AK_Control_Timer(T_hTimer timer, T_OPTION enable);

signed long AK_Delete_Timer(T_hTimer timer);
...
/************************************************************************/
signed long AK_Activate_HISR(T_hHisr hisr);
...
/*返回值说明
    ...
    AK_INVALID_HISR 表示HISR句柄非法                                    */
/************************************************************************/
signed long AK_Delete_HISR(T_hHisr hisr);
...
/************************************************************************/
signed long AK_Register_LISR(signed long vector, void (*list_entry)(signed long), 
                            void (**old_lisr)(signed long));
...
参数说明
    ...
    new     [in]新的中断服务子程序
                                                                     */
/************************************************************************/
void *AK_Setup_Vector(signed long vector, void *new_vector);
...
*******************************************************************************************************/
 ...
 {
     signed long             error_type;
     signed char              tc_name[8+4];             //task or HISR 's name
...
#define AK_Feed_Watchdog(x) AK_Feed_WatchdogEx(x, (char *)(__FILE__),(unsigned long)(__LINE__))
signed long AK_Feed_WatchdogEx(unsigned long  food, char * filename, unsigned long line);
