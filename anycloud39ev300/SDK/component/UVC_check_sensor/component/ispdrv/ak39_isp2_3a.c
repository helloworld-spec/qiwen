...
#define CONTROL_ZONE2	64
#define CONTROL_ZONE3	96
#define CONTROL_ZONE4	128
#define CONTROL_STEP1	8
#define CONTROL_STEP2	16
#define CONTROL_STEP3	24
#define CONTROL_STEP4	32
#define CONTROL_STEP5	64
#define CONTROL_STEP_MAX	64
...
//获取最新的isp统计结果，即现在isp正在处理的buffer的前一时刻用的buffer
...
static int _calc_ccm_step(void)
{
	...
	int ccid[3][3] = {{256,0,0},{0,256,0},{0,0,256}};
	int saturation = isp->ccm_info.saturation;
	...
	ccm_target.cc_enable = isp->ccm_info.ccm_target.cc_enable;
	for(i=0; i<3; i++)
		for(j=0; j<3; j++)
			ccm_target.ccm[i][j] = (isp->ccm_info.ccm_target.ccm[i][j]*saturation+ccid[i][j]*(256-saturation))/256;
		
	for(i=0; i<3; i++)
		for(j=0; j<3; j++)
		{
			delta = ccm_target.ccm[i][j]- isp->ccm_info.ccm_current.ccm[i][j];
			if(delta!=0)
				change_flag = 1;
			if(delta>0)
			{
	...
	if(isp->ccm_info.ccm_current.cc_enable !=ccm_target.cc_enable)
	{
		change_flag = 1;
		isp->ccm_info.ccm_current.cc_enable = ccm_target.cc_enable;
	}
...
{
	int  delta_lut_a,delta_lut_b,delta_lut_s;
	int  step = 0;
	int i;
	int hue_change_flag = 0;
	...
	for(i = 0; i < 64;i++)
	{
		delta_lut_a = isp->hue_info.hue_target.hue_lut_a[i]- isp->hue_info.hue_current.hue_lut_a[i];
		if(delta_lut_a>0)
		{
			delta_lut_a= (delta_lut_a+step-1)/step;
		}
		if(delta_lut_a<0)
		{
			delta_lut_a= (delta_lut_a-step+1)/step;
		}
		isp->hue_info.hue_current.hue_lut_a[i] += delta_lut_a;
		

		delta_lut_b = isp->hue_info.hue_target.hue_lut_b[i]- isp->hue_info.hue_current.hue_lut_b[i];
		if(delta_lut_b>0)
		{
			delta_lut_b= (delta_lut_b+step-1)/step;
		}
		if(delta_lut_b<0)
		{
			delta_lut_b= (delta_lut_b-step+1)/step;
		}
		isp->hue_info.hue_current.hue_lut_b[i] += delta_lut_b;
		

		delta_lut_s = (T_S16)isp->hue_info.hue_target.hue_lut_s[i]- isp->hue_info.hue_current.hue_lut_s[i];
		if(delta_lut_s>0)
		{
			delta_lut_s= (delta_lut_s+step-1)/step;
		}
		if(delta_lut_s<0)
		{
			delta_lut_s= (delta_lut_s-step+1)/step;
		}
		isp->hue_info.hue_current.hue_lut_s[i] += delta_lut_s;

		if(delta_lut_a!=0 || delta_lut_b !=0 || delta_lut_s!=0)
			hue_change_flag = 1;
	}
...
static  int _get_total_gain(void)
{
...
int ak_isp_awb_work(void)
{
	...
	//每一次中断isp->frame_cnt都会加1，即统计图像帧的数量。偶数次帧才进行相应的白平衡处理
	...
	else
	{
		...
		if(total_gain<=32*256)
		{
			isp->ccm_info.saturation = 256;	//sat = 1
		}
		else if(total_gain>=64*256)
		{
			isp->ccm_info.saturation = 0;
		}
		else
		{
			isp->ccm_info.saturation = 256 - (total_gain-32*256)/32;
		}
		if(_calc_ccm_step())
		{
    ...
    // color_index = _get_normal_color_temp_index();   //获取正常情况下的颜色指示标志 
    color_index = _get_abnormal_color_temp_index();    //获取计算出的awb的指示标志
	//是否统计到有用的白平衡结果
...
{   
	...
	if(isp->ae_para.OE_suppress_en)
		histo_arr = isp->yuv_hist_stat_para.y_hist;
	else
	...
	{        
		...
		{	        
			histo_sum[i] += ((j*(histo_arr[j])*isp->ae_para.hist_weight[i]));	       
			pixel_sum[i] += (histo_arr[j]*isp->ae_para.hist_weight[i]);	    
		}   
	}	 
	for(i=0;i<256;i++)	 
	{	 	
...
/*
  因为ISP TOOL环境的界定是根据曝光时间定义的，
  所以随环境变化的使用不同参数的函数调用会在曝光
 ak_isp_ae_work函数中调用
  */
int _isp_para_change_with_envi(enum  envi_flag envi_flag)
{
	...
	{
		_set_wdr_attr(p_wdr);	//立即生效
		isp->wdr_info.step = 0;
	}
	else
		isp->wdr_info.step = 16;	//逐渐过渡到target_wdr
...
int  _calc_aec_para(void)
{
	...
	else*/
	{
		...
		if(calc_lumi_average<high_lock+((high_lock*CONTROL_ZONE1)>>8)&&  calc_lumi_average>low_lock-((low_lock*CONTROL_ZONE1)>>8))
		{
			gain_step = CONTROL_STEP1;		
		}
		else if(calc_lumi_average<high_lock+((high_lock*CONTROL_ZONE2)>>8)&&  calc_lumi_average>low_lock-((low_lock*CONTROL_ZONE2)>>8))
		{
			gain_step = CONTROL_STEP2;		
		}
		else if(calc_lumi_average<high_lock+((high_lock*CONTROL_ZONE3)>>8)&&  calc_lumi_average>low_lock-((low_lock*CONTROL_ZONE3)>>8))
		{
			gain_step = CONTROL_STEP3;		
		}
		else if(calc_lumi_average<high_lock+((high_lock*CONTROL_ZONE4)>>8)&&  calc_lumi_average>low_lock-((low_lock*CONTROL_ZONE4)>>8))
		{
			gain_step = CONTROL_STEP4;
		}
		else
		{
			gain_step = CONTROL_STEP5;
		}
		...
		if(tmp_rang>0)
		{
			...
			if(calc_lumi_average<low_lock/2 || calc_lumi_average>high_lock*2)
			{
				if(gain_step<CONTROL_STEP_MAX)
					gain_step = CONTROL_STEP_MAX;		//very fast
			}
		}
	...
	{    
		...
		if(isp->ae_run_info_para.current_exp_time>isp->ae_para.exp_time_min)
		{
			...
			isp->ae_run_info_para.current_isp_d_gain = isp->ae_para.isp_d_gain_min;
			if(isp->ae_run_info_para.current_exp_time>exp_step)	//exp_time取整
			...
			if(isp->ae_algo.isp_d_gain_need_updata ==1&&isp->ae_algo.exp_time_need_updata  == 1)
				isp->ae_drop_count=DROP_FRAMES_MAX;
			return 0;
		}
	}
	else if(increase_flag ==1)        //弱爆光，增加
	{  
		//增加曝光时间
		//printk("dark envi \n");
		//printk("exp_time =%d\n",isp->ae_run_info_para.current_exp_time);    
...
int _get_iso(void)
{
	...
	if(isp->ae_para.envi_gain_range[0][0] !=0&&exp_time<isp->ae_para.envi_gain_range[0][0])
		iso = 0;
	else if(isp->ae_para.envi_gain_range[0][0] ==0&& all_gain<isp->ae_para.envi_gain_range[0][1]*ONE_X_GAIN)	//[0][1]表示曝光时间
		iso = 0;
...
int _set_blc_attr(AK_ISP_BLC *blc)
{
    ...
    isp_dbg("%s enter.\n", __func__);
    bl_r_a = (isp->ae_run_info_para.current_isp_d_gain*blc->bl_r_a)>>GAIN_SHIFT;
    if(bl_r_a>1023)
    	bl_r_a=1023;

    bl_gr_a = (isp->ae_run_info_para.current_isp_d_gain*blc->bl_gr_a)>>GAIN_SHIFT;
    if(bl_gr_a>1023)
    	bl_gr_a=1023;

    bl_gb_a = (isp->ae_run_info_para.current_isp_d_gain*blc->bl_gb_a)>>GAIN_SHIFT;
    if(bl_gb_a>1023)
    	bl_gb_a=1023;

    bl_b_a = (isp->ae_run_info_para.current_isp_d_gain*blc->bl_b_a)>>GAIN_SHIFT;
    if(bl_b_a>1023)
    	bl_b_a=1023;

    bl_r_offset = (isp->ae_run_info_para.current_isp_d_gain*blc->bl_r_offset)>>GAIN_SHIFT;
    bl_gr_offset = (isp->ae_run_info_para.current_isp_d_gain*blc->bl_gr_offset)>>GAIN_SHIFT;
    bl_gb_offset = (isp->ae_run_info_para.current_isp_d_gain*blc->bl_gb_offset)>>GAIN_SHIFT;
    bl_b_offset = (isp->ae_run_info_para.current_isp_d_gain*blc->bl_b_offset)>>GAIN_SHIFT;
...
int ak_isp_ae_work(void)
{
	...
	//isp->ae_run_info_para.current_calc_avg_compensation_lumi);
    //根据计算出来的平均值与目标值的差值计算步长
  	//_cmos_calc_step();
    //根据计算亮度和目标的关系计算相关参数
    _calc_aec_para();
	//_isp_change_ae_target();
    //根据步长计算出相关写给sensor的寄存器的值
	...
	{
		int skip_frame;
		skip_frame = _cmos_updata_exp_time(isp->ae_run_info_para.current_exp_time);
	...
	{
		int skip_frame;
		skip_frame = 	_cmos_updata_a_gain(isp->ae_run_info_para.current_a_gain);
		if(skip_frame>=0 && skip_frame>isp->ae_drop_count)
			isp->ae_drop_count = skip_frame;
	...
	{
		int skip_frame;
		skip_frame = _cmos_updata_d_gain(isp->ae_run_info_para.current_d_gain);
		if(skip_frame>=0 && skip_frame>isp->ae_drop_count)
			isp->ae_drop_count = skip_frame;
...
int _isp_ccm_change_with_envi(enum isp2_colortemp_mode color_index)
{
	...
	if(isp->ccm_para.cc_mode!=MODE_MANUAL)
	{
		...
		case ISP2_COLORTEMP_MODE_A:
			memcpy(&(isp->ccm_info.ccm_target), &(isp->ccm_para.ccm[ISP2_COLORTEMP_MODE_A]), sizeof(AK_ISP_CCM));
			break;
		case ISP2_COLORTEMP_MODE_TL84:
			memcpy(&(isp->ccm_info.ccm_target), &(isp->ccm_para.ccm[ISP2_COLORTEMP_MODE_TL84]), sizeof(AK_ISP_CCM));
			break;
		case ISP2_COLORTEMP_MODE_D50:
			memcpy(&(isp->ccm_info.ccm_target), &(isp->ccm_para.ccm[ISP2_COLORTEMP_MODE_D50]), sizeof(AK_ISP_CCM));
			break;
		case ISP2_COLORTEMP_MODE_D65:
			memcpy(&(isp->ccm_info.ccm_target), &(isp->ccm_para.ccm[ISP2_COLORTEMP_MODE_D65]), sizeof(AK_ISP_CCM));
			break;
		default:
			break;  
		}
	...
	{
		memcpy(&(isp->ccm_info.ccm_target), &(isp->ccm_para.manual_ccm), sizeof(AK_ISP_CCM));
	}
...
int _isp_hue_change_with_envi(enum isp2_colortemp_mode color_index)
{
	if(isp->hue_para.hue_mode!=MODE_MANUAL)
	{
		...
		{
		case ISP2_COLORTEMP_MODE_A:
			memcpy(&(isp->hue_info.hue_target), &(isp->hue_para.hue[ISP2_COLORTEMP_MODE_A]), sizeof(AK_ISP_HUE));
			break;
		case ISP2_COLORTEMP_MODE_TL84:
			memcpy(&(isp->hue_info.hue_target), &(isp->hue_para.hue[ISP2_COLORTEMP_MODE_TL84]), sizeof(AK_ISP_HUE));
			break;
		case ISP2_COLORTEMP_MODE_D50:
			memcpy(&(isp->hue_info.hue_target), &(isp->hue_para.hue[ISP2_COLORTEMP_MODE_D50]), sizeof(AK_ISP_HUE));
			break;
		case ISP2_COLORTEMP_MODE_D65:
			memcpy(&(isp->hue_info.hue_target), &(isp->hue_para.hue[ISP2_COLORTEMP_MODE_D65]), sizeof(AK_ISP_HUE));
			break;
		default:
			break;  
		}
	...
	{
		memcpy(&(isp->hue_info.hue_target), &(isp->hue_para.manual_hue), sizeof(AK_ISP_HUE));
	}
