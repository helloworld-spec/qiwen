...
apcmd_antcfg(int argc, char *argv[])
{
    ...
    if (argc) {
        if ((ISDIGIT(argv[0]) == 0) || (atoi(argv[0]) < 1) ||
            (atoi(argv[0]) > 3)) {
            printf
                ("ERR:Illegal ANTENNA parameter %s. Must be either '1', '2' or '3'.\n",
                 argv[0]);
            print_antcfg_usage();
            return UAP_FAILURE;
        }
        if (argc == 2) {
            if ((ISDIGIT(argv[1]) == 0) || (atoi(argv[1]) < 1) ||
                (atoi(argv[1]) > 3)) {
                printf
                    ("ERR:Illegal RX ANTENNA parameter %s. Must be either '1', '2' or '3'.\n",
                     argv[1]);
                print_antcfg_usage();
                return UAP_FAILURE;
            }
    ...
    while (config_get_line(line, MAX_CONFIG_LINE, config_file, &li, &pos)) {
        ...
        if (strcmp(args[0], "Enable11n") == 0) {
            ...
            if (enable_11n == 1) {
                ...
                if (0 == get_fw_info(&fw)) {
                    if ((fw.hw_dev_mcs_support & 0x0f) >= 2)
                        bss->supported_mcs_set[1] = DEFAULT_MCS_SET_1;
                }
            } else {
                /* disable mcs rate */
...
get_max_mgmt_ie(int *max_mgmt_ie, int flag)
{
    apcmdbuf_sys_configure *cmd_buf = NULL;
    t_u8 *buf = NULL;
    t_u16 cmd_len, buf_len;
    int ret = UAP_SUCCESS;
    tlvbuf_header *pcurrent_tlv = NULL;
    int tlv_buf_left, i;
    ...
    *max_mgmt_ie = 0;
    ...
    if (ret == UAP_SUCCESS) {
        ...
        if (cmd_buf->result == CMD_SUCCESS) {
            ...
            pcurrent_tlv =
                (tlvbuf_header *) (buf + sizeof(apcmdbuf_sys_configure));
            tlv_buf_left =
                cmd_buf->size - sizeof(apcmdbuf_sys_configure) +
                BUF_HEADER_SIZE;
            ...
            while (tlv_buf_left >= (int) sizeof(tlvbuf_header)) {
                ...
                case MRVL_MAX_MGMT_IE_TLV_ID:
                    ...
                    for (i = 0; i < max_mgmt_ie_tlv->count; i++) {
                        ...
                        *max_mgmt_ie += max_mgmt_ie_tlv->info[i].buf_count;
                        if (flag) {
                            printf("buf%d_size = %d\n", i,
                                   max_mgmt_ie_tlv->info[i].buf_size);
