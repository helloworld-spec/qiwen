...
const static chan_freq_power_t channel_freq_power_UN_BG[] = {
...
const static chan_freq_power_t channel_freq_power_UN_AJ[] = {
...
wlan_11d_region_2_code(pmlan_adapter pmadapter, t_u8 * region, OUT t_u8 * code)
{
    ...
    for (i = 0; i < size; i++) {
        if (!memcmp( region_code_mapping[i].region,
                              ...
                              parsed_region_chan_11d_t * parsed_region_chan)
{
    ...
    memset( domain_info, 0, sizeof(wlan_802_11d_domain_reg_t));
                  ...
                  t_u8 no_of_chan, t_u8 * chan)
{
    ...
    //reduce 5GHZ
	#ifdef NOT_BYPASS_5GHZ
        cfp = (chan_freq_power_t *)channel_freq_power_UN_AJ;
                         ...
                         IEEEtypes_SubbandSet_t * sub_band_list)
{
    ...
    memset( pdomain, 0, sizeof(wlan_802_11d_domain_reg_t));
    memcpy( pdomain->country_code, country_code, COUNTRY_CODE_LEN);
    pdomain->band = band;
    pdomain->no_of_sub_band = num_sub_band;
    memcpy( pdomain->sub_band, sub_band_list,
...
wlan_11d_chan_2_freq(pmlan_adapter pmadapter, t_u8 chan, t_u8 band)
{
    ...
    if (band & (BAND_A | BAND_AN)) {
		//reduce 5GHZ
		#ifdef NOT_BYPASS_5GHZ
        cf = (chan_freq_power_t *)channel_freq_power_UN_AJ;
...
wlan_11d_set_universaltable(mlan_private * pmpriv, t_u8 band)
{
    ...
    if (band & (BAND_A | BAND_AN)) {
        ...
        pmadapter->universal_channel[i].pcfp = (chan_freq_power_t *)channel_freq_power_UN_AJ;
