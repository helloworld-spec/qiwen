                         ...
                         RxReorderTbl * rx_reor_tbl_ptr)
{
    ...
    DBG_HEXDUMP(MDAT_D, "Reorder ptr", (t_u8*)rx_reor_tbl_ptr->rx_reorder_ptr,
                sizeof(char *) * rx_reor_tbl_ptr->win_size);
                              ...
                              int win_size, int seq_num)
{
     ...
     */
    if ((rx_reor_tbl_ptr = wlan_11n_get_rxreorder_tbl(priv, tid, ta)) != 0) {
        ...
        if (queuing_ra_based(priv)) {
            ...
            if (GET_BSS_ROLE(priv) == MLAN_BSS_ROLE_UAP) {
                if ((sta_ptr = wlan_get_station_entry(priv, ta)) != 0)
...
wlan_11n_get_rxreorder_tbl(mlan_private * priv, int tid, t_u8 * ta)
{
    ...
    while (rx_reor_tbl_ptr != (RxReorderTbl *) & priv->rx_reorder_tbl_ptr) {
        if ((!memcmp
             (rx_reor_tbl_ptr->ta, ta, MLAN_MAC_ADDR_LENGTH)) &&
                          ...
                          HostCmd_DS_COMMAND * cmd, void *pdata_buf)
{
    ...
    memcpy( padd_ba_rsp->peer_mac_addr,
                       ...
                       t_u8 * ta, t_u8 pkt_type, void *payload)
{
    ...
    if (
        (rx_reor_tbl_ptr =
         wlan_11n_get_rxreorder_tbl((mlan_private *) priv, tid, ta)) == 0) {
                             ...
                             t_u8 * peer_mac, t_u8 type, int initiator)
{
    ...
    if (cleanup_rx_reorder_tbl) {
        ...
        if ((ptxtbl = wlan_11n_get_txbastream_tbl(priv, tid, peer_mac)) == 0) {
...
wlan_cleanup_reorder_tbl(mlan_private * priv, t_u8 * ta)
{
    ...
    for (i = 0; i < MAX_NUM_TID; ++i) {
        if ((rx_reor_tbl_ptr = wlan_11n_get_rxreorder_tbl(priv, i, ta)) != 0) {
            wlan_11n_delete_rxreorder_tbl_entry(priv, rx_reor_tbl_ptr);
        }
    }
    LEAVE();
