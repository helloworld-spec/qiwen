...
wlan_init_priv(pmlan_private priv)
{
	...
	priv->assoc_rsp_buf = (t_u8*)mem_calloc(200, 1);
	if(!priv->assoc_rsp_buf)
		priv->max_assoc_rsp_size = 0;
	else
		priv->max_assoc_rsp_size = 200;
#else
    ...
    memset( &priv->uap_state_chan_cb, 0,
    ...
    memset( &priv->adhoc_last_start_ssid, 0,
	...
	priv->gen_ie_buf = (t_u8*)mem_calloc(32, 1);
	priv->max_gen_ie_buf_len = 32;
#else
...
wlan_init_adapter(pmlan_adapter pmadapter)
{
    ...
    {
    	pmadapter->delay_to_ps = DELAY_TO_PS_UNCHANGED;
    	pmadapter->enhanced_ps_mode = PS_MODE_UNCHANGED;
	pmadapter->init_auto_ds = MFALSE;	
	pmadapter->ps_mode = Wlan802_11PowerModeCAM;
    }
	...
	pmadapter->event_body = (t_u8 *)mem_calloc(128, 1);
	if(!pmadapter->event_body)
		pmadapter->event_body_len = 0;
	else
		pmadapter->event_body_len = 128;
#else
    ...
    memset( &pmadapter->sleep_params, 0,
           sizeof(pmadapter->sleep_params));
    memset( &pmadapter->sleep_period, 0,
    ...
    memcpy( pmadapter->country_code, MRVDRV_DEFAULT_COUNTRY_CODE,
           COUNTRY_CODE_LEN);
