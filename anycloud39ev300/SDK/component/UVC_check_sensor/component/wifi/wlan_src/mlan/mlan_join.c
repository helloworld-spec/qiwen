...
wlan_cmd_append_generic_ie(mlan_private * priv, t_u8 ** ppbuffer)
{
    ...
    if (priv->gen_ie_buf_len) {
        ...
        *ppbuffer += sizeof(ie_header);
        ...
        memcpy( *ppbuffer, priv->gen_ie_buf,
               priv->gen_ie_buf_len);
                              ...
                              OUT t_u32 * pout_rates_size)
{
    ...
    memcpy( pout_rates, pbss_desc->supported_rates,
                           ...
                           IN BSSDescriptor_t * pnew_bss_desc)
{
    ...
    memcpy( &new_tsf_base, pnew_bss_desc->time_stamp,
           sizeof(new_tsf_base));
                          ...
                          IN HostCmd_DS_COMMAND * cmd, IN t_void * pdata_buf)
{
    ...
    memcpy( pssid_tlv->ssid, pbss_desc->ssid.ssid,
    ...
    if (IS_SUPPORT_MULTI_BANDS(pmadapter)
        ...
        memset( pchan_tlv->chan_scan_param, 0x00,
    ...
    if (!pmpriv->wps.session_enable) {
        if ((pmpriv->sec_info.wpa_enabled || pmpriv->sec_info.wpa2_enabled)) {
			...
			if(pmpriv->max_wpa_ie_len < (prsn_ie_tlv->header.len + 2))
			{
				int ie_len = prsn_ie_tlv->header.len + 2;
				p_err("err ie len:%d, need:%d", pmpriv->max_wpa_ie_len, ie_len);

				if(pmpriv->wpa_ie)
					mem_free(pmpriv->wpa_ie);

				if((ie_len + 32) > 256)
					ie_len = 256;

				pmpriv->wpa_ie = (t_u8*)mem_calloc(ie_len + 32, 1);
				if(!pmpriv->wpa_ie)
				{
					pmpriv->max_wpa_ie_len = 0;
					ret = MLAN_STATUS_FAILURE;
                	goto done;
				}

				pmpriv->max_wpa_ie_len = ie_len + 32;
			}
...
#if WPA_IE_ALLOC_IN_USE
			if (prsn_ie_tlv->header.len <= (pmpriv->max_wpa_ie_len - 2))
            ...
            if (pbss_desc->pwpa_ie) {
                ...
                prsn_ie_tlv->header.len = prsn_ie_tlv->header.len & 0x00FF;
#if WPA_IE_ALLOC_IN_USE
				if (prsn_ie_tlv->header.len <= (pmpriv->max_wpa_ie_len)) {
                          ...
                          IN HostCmd_DS_COMMAND * resp, IN t_void * pioctl_buf)
{
	...
	pmpriv->assoc_rsp_size = MIN(resp->size - S_DS_GEN,
									 /*pmpriv->max_assoc_rsp_size*/MRVDRV_ASSOC_RSP_BUF_SIZE);

	if(pmpriv->max_assoc_rsp_size < pmpriv->assoc_rsp_size)
	{
		mem_free(pmpriv->assoc_rsp_buf);
		pmpriv->assoc_rsp_buf = (t_u8*)mem_calloc(pmpriv->assoc_rsp_size + 32, 1);
		if(!pmpriv->assoc_rsp_buf){
			pmpriv->max_assoc_rsp_size = 0;
			ret = MLAN_STATUS_FAILURE;
        	goto done;
		}
		pmpriv->max_assoc_rsp_size = pmpriv->assoc_rsp_size + 32;
	}
#else
	pmpriv->assoc_rsp_size = MIN(resp->size - S_DS_GEN,
...
#endif
    memcpy( pmpriv->assoc_rsp_buf, &resp->params,
...
#define AD_HOC_CAP_PRIVACY_ON   1
    ...
    memset( padhoc_start->DataRate, 0,
...
#if WPA_IE_ALLOC_IN_USE
		if(pmpriv->max_wpa_ie_len < sizeof(wpa_ie))
		{
			if(pmpriv->wpa_ie)
				mem_free(pmpriv->wpa_ie);

			pmpriv->wpa_ie = (t_u8*)mem_calloc(sizeof(wpa_ie) + 32, 1);
			if(!pmpriv->wpa_ie)
			{
				pmpriv->max_wpa_ie_len = 0;
				ret = MLAN_STATUS_FAILURE;
			...
			pmpriv->max_wpa_ie_len = sizeof(wpa_ie) + 32;
		}
#endif
...
#define USE_G_PROTECTION        0x02
    ...
    memcpy( &tmp_cap, &pbss_desc->cap_info,
    ...
    memcpy( padhoc_join->bss_descriptor.data_rates,
                 ...
                 IN t_void * pioctl_buf, IN mlan_802_11_ssid * padhoc_ssid)
{
    ...
    if (ret == MLAN_STATUS_SUCCESS)
        memcpy( &pmpriv->adhoc_last_start_ssid,
