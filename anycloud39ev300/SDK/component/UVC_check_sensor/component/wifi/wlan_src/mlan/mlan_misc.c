                              ...
                              IN custom_ie * ie_data, IN t_u16 idx)
{
    ...
    memcpy( del_ie, ie_data->ie_buffer,
    ...
    for (index = 0; index < pmadapter->max_mgmt_ie_index; index++) {
        ...
        while (tmp_len) {
            if (!memcmp( tmp_ie, del_ie, del_len)) {
                memcpy( ie, pmpriv->mgmt_ie[index].ie_buffer,
                       cnt);
                         ...
                         IN pmlan_ioctl_req pioctl_req)
{
    ...
    if (pioctl_req->action == MLAN_ACT_SET) {
        ...
        memcpy( pmadapter->dbg.last_cmd_id,
               info->param.debug_info.last_cmd_id,
               sizeof(pmadapter->dbg.last_cmd_id));
        memcpy( pmadapter->dbg.last_cmd_act,
        ...
        memcpy( pmadapter->dbg.last_event,
        ...
        memcpy( info->param.debug_info.last_cmd_id,
               pmadapter->dbg.last_cmd_id, sizeof(pmadapter->dbg.last_cmd_id));
        memcpy( info->param.debug_info.last_cmd_act,
        ...
        info->param.debug_info.last_cmd_resp_index =
            pmadapter->dbg.last_cmd_resp_index;
        memcpy( info->param.debug_info.last_event,
                       ...
                       t_u32 head_room, t_u32 malloc_flag)
{
    ...
    case MOAL_MALLOC_BUFFER:
        ...
        memset( pmbuf, 0, /*sizeof(mlan_buffer)*/buf_size);
        ...
        while (ioctl_len > 0) {
            ...
            if (MLAN_CUSTOM_IE_AUTO_IDX_MASK == index) {        /* Need to be
                ...
                if (ie_data->mgmt_subtype_mask == MLAN_CUSTOM_IE_DELETE_MASK &&
                    ...
                    memset( &pmpriv->mgmt_ie[index], 0,
                           sizeof(custom_ie));
                    memcpy( &pmpriv->mgmt_ie[index], ie_data,
                           sizeof(custom_ie));
                }
...
wlan_get_station_entry(mlan_private * priv, t_u8 * mac)
{
    ...
    while (sta_ptr != (sta_node *) & priv->sta_list) {
        if (!memcmp
            (sta_ptr->mac_addr, mac, MLAN_MAC_ADDR_LENGTH)) {
            LEAVE();
            return sta_ptr;
        }
...
wlan_add_station_entry(mlan_private * priv, t_u8 * mac)
{
    ...
    memset( sta_ptr, 0, sizeof(sta_node));
    memcpy( sta_ptr->mac_addr, mac, MLAN_MAC_ADDR_LENGTH);
                               ...
                               IN t_u8 * payload, IN t_u32 payload_len)
{
    ...
    if (unicast == MTRUE) {
        if (memcmp
            ( pieee_pkt_hdr->addr1, priv->curr_addr,
    ...
    memcpy( (t_u8 *) (pevent->event_buf + sizeof(pevent->event_id)),
           payload, payload_len);
                               ...
                               IN pmlan_ioctl_req pioctl_req)
{
    ...
    if (ds_rate->param.rate_cfg.is_rate_auto) {
        ...
        if (/*(rate_index >= MLAN_RATE_INDEX_HRDSSS0)*/1 &&
            (rate_index <= MLAN_RATE_INDEX_HRDSSS3)) {
            bitmap_rates[0] = 1 << rate_index;
            ret = MLAN_STATUS_SUCCESS;
