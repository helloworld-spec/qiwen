    ...
    while (ptmp_chan_list->chan_number) {
        ...
        while (tlv_idx < max_chan_per_scan && ptmp_chan_list->chan_number &&
            ...
            if (foundJPch14 == MTRUE) {
                ...
                pchan_tlv_out->header.len = 0;
                if (ptlv_temp) {
                    memcpy( pscan_cfg_out->tlv_buf, ptlv_temp,
                           tlv_buf_len);
            ...
            if ((pmadapter->region_code == COUNTRY_CODE_JP_40 ||
                ...
                memcpy( ptlv_temp, ptlv_pos, tlv_buf_len);
                ...
                if (foundJPch14) {
                    ...
                    prates_tlv->header.len =
                        wlan_copy_rates(prates_tlv->rates, 0, (void*)SupportedRates_B,
                    ...
                    memmove( ptlv_pos + new_ratetlv_len,
                                ...
                                IN t_u8 ** pbeacon_info, IN t_u32 * bytes_left)
{
    ...
    *bytes_left -= beacon_size;
    ...
    memcpy( pbss_entry->mac_address, pcurrent_ptr,
    ...
    memcpy( (void*)&fixed_ie.beacon_interval, (const void*)pcurrent_ptr, 2);
    pbss_entry->beacon_period = wlan_le16_to_cpu(fixed_ie.beacon_interval);
    pcurrent_ptr += 2;
    bytes_left_for_current_beacon -= 2;

    /* Capability information is 2 bytes long */
    memcpy( (void*)&fixed_ie.capabilities, (const void*)pcurrent_ptr, 2);
    ...
    memcpy( &pbss_entry->cap_info, pcap_info,
    ...
    while (bytes_left_for_current_beacon >= 2) {
        ...
        case SSID:
            ...
            if (!pbss_entry->ssid.ssid_len) {
                ...
                memcpy( pbss_entry->ssid.ssid, (pcurrent_ptr + 2),
                       element_len);
            }
        ...
        case SUPPORTED_RATES:
            ...
            memcpy( pbss_entry->data_rates, pcurrent_ptr + 2,
                   element_len);
            memcpy( pbss_entry->supported_rates, pcurrent_ptr + 2,
                   element_len);
        ...
        case FH_PARAM_SET:
            ...
            memcpy( &pbss_entry->phy_param_set.fh_param_set,
        ...
        case DS_PARAM_SET:
            ...
            memcpy( &pbss_entry->phy_param_set.ds_param_set,
        ...
        case CF_PARAM_SET:
            ...
            memcpy( &pbss_entry->ss_param_set.cf_param_set,
        ...
        case IBSS_PARAM_SET:
            ...
            memcpy( &pbss_entry->ss_param_set.ibss_param_set,
                                  ...
                                  IN BSSDescriptor_t * pnew_beacon)
{
    ...
    if (pmadapter->pscan_table[beacon_idx].pbeacon_buf) {
        ...
        if (new_bcn_size == old_bcn_size) {
            ...
            memcpy( pbcn_store, pnew_beacon->pbeacon_buf,
                   new_bcn_size);
        ...
        if ((pmadapter->pbcn_buf_end + pnew_beacon->beacon_buf_size +
            ...
            if ((ret == MLAN_STATUS_SUCCESS) && (tmp_buf)) {
                ...
                memcpy( tmp_buf, pmadapter->bcn_buf, bcn_size);
                /* Adjust beacon buffer pointers that are past the current */
                for (adj_idx = 0; adj_idx < num_of_ent; adj_idx++) {
                    bcn_offset =
                        pmadapter->pscan_table[adj_idx].pbeacon_buf -
...
wlan_flush_scan_table(IN pmlan_adapter pmadapter)
{
    ...
    memset( pmadapter->bcn_buf, 0, pmadapter->bcn_buf_size);
    pmadapter->pbcn_buf_end = pmadapter->bcn_buf;
                     ...
                     IN HostCmd_DS_COMMAND * pcmd, IN t_void * pdata_buf)
{
    ...
    memcpy( pscan_cmd->tlv_buffer, pscan_cfg->tlv_buf,
                         ...
                         IN HostCmd_DS_COMMAND * pcmd, IN t_void * pdata_buf)
{
    ...
    memcpy( pext_scan_cmd->tlv_buffer,
    ...
    for (idx = 0; idx < number_of_sets && bytes_left >
        ...
        if (wlan_interpret_bss_desc_with_ie(pmadapter,
            ...
            if (pscan_info_tlv) {
                ...
                memcpy( &tsf_val, (const void*)&pscan_info_tlv->tsf,
                       sizeof(tsf_val));
                tsf_val = wlan_le64_to_cpu(tsf_val);
                memcpy( &bss_new_entry->network_tsf,
                       &tsf_val, sizeof(bss_new_entry->network_tsf));
                        ...
                        IN t_void * pioctl_buf, IN mlan_802_11_ssid * preq_ssid)
{
    ...
    memcpy( pscan_cfg->ssid_list[0].ssid,
