                      ...
                      t_u32 upld_typ)
{
    ...
    case MLAN_TYPE_CMD:
        ...
        if (!pmadapter->curr_cmd) {
        	...
        	if(pmadapter->curr_cmd->pioctl_buf)
				PRINTM(MINFO, " in decode rx packet, curr_cmd ioctl %p\n", pmadapter->curr_cmd->pioctl_buf);
    ...
    if (f_do_rx_aggr) {
        ...
        for (pind = 0; pind < pmadapter->mpa_rx.pkt_cnt; pind++) {
            ...
            if ((pkt_type == MLAN_TYPE_DATA) &&
                ...
                memcpy( mbuf_deaggr->pbuf + mbuf_deaggr->data_offset,
                       curr_ptr, pkt_len);
                       ...
                       mlan_tx_param * tx_param)
{
    ...
    *(t_u16 *) & payload[2] = wlan_cpu_to_le16(type);
    ...
    if (ret != MLAN_STATUS_SUCCESS) {
        ...
        DBG_HEXDUMP(MIF_D, "SDIO Blk Wr1", pmbuf->pbuf + pmbuf->data_offset,
