...
********************************************************/
#ifdef STA_SUPPORT
const mlan_operations mlan_sta_ops = {
    /* init cmd handler */
...
#endif
#ifdef UAP_SUPPORT
const mlan_operations mlan_uap_ops = {
    /* init cmd handler */
...
/** mlan function table */
const mlan_operations *mlan_ops[] = {
#ifdef STA_SUPPORT
    &mlan_sta_ops,
#endif
#ifdef UAP_SUPPORT
        ...
        if (pmdevice->bss_attr[i].active == MTRUE) {
            ...
            for (j = 0; mlan_ops[j]; j++) {
                ...
                if (mlan_ops[j]->bss_role == GET_BSS_ROLE(pmadapter->priv[i])) {
                    memcpy( &pmadapter->priv[i]->ops, mlan_ops[j],
  ...
  process_start:
    do {
        ...
        if (!pmadapter->scan_processing && !pmadapter->data_sent &&
            ...
            //zoutx :  修改这个函数增加返回值判断,用于让这个函数跳出这个
            //mlan_main_process的循环回到上一层能够接收SDIO数据的地方， 从而将PAUSE解除
            if( wlan_wmm_process_tx(pmadapter) == MFALSE)
            {
            	break;
            }
//			printf("pmadapter->data_sent = %x\n", pmadapter->data_sent);
            if (pmadapter->hs_activated == MTRUE) {
                pmadapter->is_hs_configured = MFALSE;
                wlan_host_sleep_activated_event(wlan_get_priv
                                                (pmadapter, MLAN_BSS_ROLE_ANY),
