                            ...
                            IN HostCmd_DS_COMMAND * cmd, IN t_u16 cmd_action)
{
    ...
    if (cmd_action == HostCmd_ACT_GEN_SET) {
        memcpy( cmd->params.mac_addr.mac_addr,
                               ...
                               IN t_void * pdata_buf)
{
    ...
    /* Set AP MAC address */
    memcpy( pdeauth->mac_addr, (t_u8 *) pdata_buf,
                               ...
                               IN t_u16 cmd_action, IN t_void * pdata_buf)
{
    ...
    if (memcmp
        ((t_u8 *) & psk->bssid, zero_mac, sizeof(zero_mac))) {
        ...
        memcpy( pbssid_tlv->bssid, (t_u8 *) & psk->bssid,
               MLAN_MAC_ADDR_LENGTH);
    ...
    if (psk->psk_type == MLAN_PSK_PMK) {
        ...
        memcpy( ppmk_tlv->pmk, psk->psk.pmk.pmk,
               MLAN_MAX_KEY_LENGTH);
                      ...
                      IN t_u16 cmd_action, IN t_void * pdata_buf)
{
    ...
    if (pmgmt_ie_list->ds_mgmt_ie.ie_data_list && cust_ie->ie_data_list) {
        ...
        if (cust_ie->len)
            memcpy( pmgmt_ie_list->ds_mgmt_ie.ie_data_list,
