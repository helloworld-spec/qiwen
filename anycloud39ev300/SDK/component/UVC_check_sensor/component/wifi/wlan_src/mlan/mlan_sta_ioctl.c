                       ...
                       IN pmlan_ioctl_req pioctl_req)
{
    ...
    memcpy( &info->param.bss_info.capability_info,
    ...
    memset( info->param.bss_info.peer_supp_rates, 0,
           sizeof(info->param.bss_info.peer_supp_rates));
    memcpy( info->param.bss_info.peer_supp_rates,
                           ...
                           IN pmlan_ioctl_req pioctl_req)
{
    ...
    if (pioctl_req->action == MLAN_ACT_GET) {
        ...
        memcpy( &bss->param.mac_addr, pmpriv->curr_addr,
               MLAN_MAC_ADDR_LENGTH);
        ret = MLAN_STATUS_SUCCESS;
        goto exit;
    }

    memcpy( pmpriv->curr_addr, &bss->param.mac_addr,
           MLAN_MAC_ADDR_LENGTH);
    ...
    if (pmpriv->bss_mode == MLAN_BSS_MODE_INFRA) {
        if (!bss->param.ssid_bssid.idx ||
            ...
            if (bss->param.ssid_bssid.ssid.ssid_len) {
                if (memcmp
                    (&bss->param.ssid_bssid.bssid, zero_mac,
                     sizeof(zero_mac)))
...
wlan_query_passphrase(mlan_private * priv, pmlan_ioctl_req pioctl_req)
{
    ...
    memset( &sec_pp, 0, sizeof(mlan_ds_passphrase));
    ...
    if (ssid_bssid->ssid.ssid_len == 0) {
        ...
        if (i >= 0) {
            ...
            memcpy( &sec_pp.ssid, &pbss_desc->ssid,
                   sizeof(mlan_802_11_ssid));
        } else
            memcpy( &sec_pp.bssid, &ssid_bssid->bssid,
                   MLAN_MAC_ADDR_LENGTH);
    } else {
        memcpy( &sec_pp.ssid, &ssid_bssid->ssid,
               sizeof(mlan_802_11_ssid));
    }
                               ...
                               IN pmlan_ioctl_req pioctl_req)
{
    ...
    case 4:
        ...
        if ((mod_class = wlan_get_modulation_class(pmadapter, data[0])) == 0) {
...
wlan_set_wpa_ie_helper(mlan_private * priv, t_u8 * ie_data_ptr, t_u16 ie_len)
{
	...
	if (ie_len) {
		...
		if (ie_len > priv->max_wpa_ie_len) {
            p_err("ie len err:%d ,need:%d\n", priv->max_wpa_ie_len,ie_len);
            if(priv->wpa_ie)
				mem_free(priv->wpa_ie);

			if((ie_len + 32) > 256)
				ie_len = 256;

			priv->wpa_ie = (t_u8*)mem_calloc(ie_len + 32, 1);
			if(!priv->wpa_ie)
			{
				priv->max_wpa_ie_len = 0;
				return MLAN_STATUS_FAILURE;
			}

			priv->max_wpa_ie_len = ie_len + 32;
	    }
        ...
        memset( priv->wpa_ie, 0, priv->max_wpa_ie_len);
        priv->wpa_ie_len = 0;
...
wlan_set_wapi_ie(mlan_private * priv, t_u8 * ie_data_ptr, t_u16 ie_len)
{
    ENTER();
	 ...
	 if (ie_len) {
			if(ie_len > priv->max_wapi_ie_len)
			{
				if((ie_len + 32) > 256)
					ie_len = 256;
				
				p_err("ie_len err:%d, need:%d", priv->max_wapi_ie_len, ie_len);
				if(priv->wapi_ie)
					mem_free(priv->wapi_ie);
	
				priv->wapi_ie = (t_u8*)mem_calloc(ie_len + 32, 1);
	
				if(!priv->wapi_ie){
					priv->max_wapi_ie_len = 0;
					return MLAN_STATUS_FAILURE;
				}else
					priv->max_wapi_ie_len = ie_len + 32;
			}
			...
			memset( priv->wapi_ie, 0, priv->max_wapi_ie_len);
			priv->wapi_ie_len = (t_u8) ie_len;
                           ...
                           IN pmlan_ioctl_req pioctl_req)
{
    ...
    if (sec->param.encrypt_key.key_len == WPA_AES_KEY_LEN) {
        ...
        memcpy( pmpriv->aes_key.key_param_set.key,
               sec->param.encrypt_key.key_material,
    ...
    if (pmpriv->bss_mode == MLAN_BSS_MODE_IBSS &&
        ...
        if (memcmp
            (sec->param.encrypt_key.key_material, zero_key_material,
             WPA_AES_KEY_LEN)) {
    ...
    if (memcmp
        (sec->param.encrypt_key.mac_addr, broadcast_mac_addr,
         MLAN_MAC_ADDR_LENGTH))
        sec->param.encrypt_key.key_index = MLAN_KEY_INDEX_UNICAST;
                           ...
                           IN pmlan_ioctl_req pioctl_req)
{
    ...
    if (pioctl_req->action == MLAN_ACT_SET) {
        ...
        else if (sec->param.encrypt_key.key_len && (sec->param.encrypt_key.key_len != MAX_WEP_KEY_SIZE) && (sec->param.encrypt_key.key_len != MIN_WEP_KEY_SIZE))
    ...
    if (pioctl_req->action == MLAN_ACT_SET) {
        ...
        if (sec->param.passphrase.psk_type == MLAN_PSK_QUERY) {
            if (sec->param.passphrase.ssid.ssid_len == 0) {
                ...
                if (i >= 0) {
                    pbss_desc = &pmadapter->pscan_table[i];
                    memcpy( &sec->param.passphrase.ssid,
...
wlan_set_gen_ie_helper(mlan_private * priv, t_u8 * ie_data_ptr, t_u16 ie_len)
{
    ...
    if (!ie_len) {
        ...
        if (((pvendor_ie->element_id == WPA_IE)
             &&
             (!memcmp
              (pvendor_ie->oui, wpa_oui, sizeof(wpa_oui))))
            ...
            if ((pvendor_ie->element_id == WPS_IE) &&
                (priv->wps.session_enable == MFALSE) &&
                (!memcmp
                 (pvendor_ie->oui, wps_oui, sizeof(wps_oui)))) {
            /* 
			...
			if (pvendor_ie->len > 4) {
				...
				int ie_len = pvendor_ie->len + sizeof(IEEEtypes_VendorHeader_t);
				if(priv->wps.max_wps_ie_len < ie_len)
				{
					if(priv->wps.wps_ie)
						mem_free(priv->wps.wps_ie);
					
					priv->wps.wps_ie = (IEEEtypes_VendorSpecific_t*)mem_calloc(ie_len, 1);
					if(!priv->wps.wps_ie)
					{
						ret = MLAN_STATUS_FAILURE;
						priv->wps.max_wps_ie_len = 0;
						return ret;
					}
					
					priv->wps.max_wps_ie_len = ie_len;
				}
				
                memcpy( (t_u8 *) priv->wps.wps_ie, ie_data_ptr,
                       ie_len);
                //HEXDUMP("wps_ie", (t_u8 *) priv->wps.wps_ie,
                //        priv->wps.wps_ie.vend_hdr.len + 2);
            } else {
                /* Only wps oui exist, reset driver wps buffer */
                memset( (t_u8 *) priv->wps.wps_ie, 0x00,
                       priv->wps.max_wps_ie_len);
			...
			if (ie_len < (/*sizeof(priv->gen_ie_buf)*/MRVDRV_GENIE_BUF_SIZE - priv->gen_ie_buf_len)) {
				...
				if(priv->max_gen_ie_buf_len < (priv->gen_ie_buf_len + ie_len))
				{
					t_u8 *old_data;
					int new_len = priv->gen_ie_buf_len + ie_len + 32;
					p_err("max_gen_ie_buf_len too short:%d, need:%d", priv->max_gen_ie_buf_len, new_len);
					if(new_len > MRVDRV_GENIE_BUF_SIZE)
						new_len = MRVDRV_GENIE_BUF_SIZE;

					old_data = priv->gen_ie_buf;
					priv->gen_ie_buf = (t_u8*)mem_calloc(new_len, 1);
					if(!priv->gen_ie_buf)
					{
						ret = MLAN_STATUS_FAILURE;
						priv->gen_ie_buf = old_data;
						return ret;
					}
					memcpy(priv->gen_ie_buf, old_data, priv->gen_ie_buf_len);	
					mem_free(old_data);
					priv->max_gen_ie_buf_len = new_len;
                       ...
                       IN pmlan_ioctl_req pioctl_req, IN t_u32 ipv4_addr)
{
    ...
    if (len > ARP_FILTER_MAX_BUF_SIZE) {
        ...
        memcpy( pmadapter->arp_filter, buf, len);
                          ...
                          IN pmlan_ioctl_req pioctl_req, IN t_u32 ipv4_addr)
{
    ...
    memset( hostcmd, 0, sizeof(mlan_ds_misc_cmd));
    buf = hostcmd->cmd;
    ...
    if (!ipv4_addr) {
        ...
        memcpy( &filter[FLTR_BUF_IP_OFFSET], &ipv4_addr,
               sizeof(ipv4_addr));
                        ...
                        IN pmlan_ioctl_req pioctl_req)
{
    ...
    case MEF_CFG_HOSTCMD:
        ...
        filter = buf + buf_len;
        memcpy( filter, mef_cfg->param.cmd_buf.cmd,
                             ...
                             IN mlan_ioctl_req * pioctl_req)
{
    ...
    if (pioctl_req->action == MLAN_ACT_SET) {
        ...
        memcpy( pmadapter->country_code,
               country_code->country_code, COUNTRY_CODE_LEN);
    } else {
        /* GET operation */
        memcpy( country_code->country_code,
...
wlan_find_bss(mlan_private * pmpriv, pmlan_ioctl_req pioctl_req)
{
    ...
    if (memcmp
        (&bss->param.ssid_bssid.bssid, zero_mac, sizeof(zero_mac))) {
