...
//                                  SIZE_OF_DBG_STRUCT), prx_pd->rx_pkt_length);
    ...
    if ((memcmp( &prx_pkt->rfc1042_hdr,
        ...
        ((memcmp( &prx_pkt->rfc1042_hdr,
                 rfc1042_eth_hdr, sizeof(rfc1042_eth_hdr)) == 0) &&
         memcmp( (void*)&prx_pkt->rfc1042_hdr.snap_type,
                (void*)appletalk_aarp_type, sizeof(appletalk_aarp_type)) &&
         memcmp( (void*)&prx_pkt->rfc1042_hdr.snap_type,
                (void*)ipx_snap_type, sizeof(ipx_snap_type)))) {
        /* 
...
wlan_ops_sta_process_rx_packet(IN t_void * adapter, IN pmlan_buffer pmbuf)
{
    ...
    if (queuing_ra_based(priv)) {
        memcpy( ta, prx_pkt->eth803_hdr.src_addr,
               MLAN_MAC_ADDR_LENGTH);
    } else {
