...
wlan_process_tx_pause_event(pmlan_private priv, pmlan_buffer pevent)
{
    ...
    while (tlv_buf_left >= (int) sizeof(MrvlIEtypesHeader_t)) {
        ...
        if (tlv_type == TLV_TYPE_TX_PAUSE) {
            ...
            if ((sta_ptr = wlan_get_station_entry(priv, tx_pause_tlv->peermac)) != 0) {
                       ...
                       IN t_u16 cmd_action, IN pmlan_ioctl_req pioctl_buf)
{
    ...
    if (memcmp
        (zero_mac, &bss->param.bss_config.mac_addr,
    ...
    if ((bss->param.bss_config.protocol & PROTOCOL_WPA) ||
        ...
        if (bss->param.bss_config.wpa_cfg.length) {
            ...
            memcpy( tlv_passphrase->passphrase,
    ...
    if ((bss->param.bss_config.ht_cap_info)
        ...
        memcpy( tlv_htcap->ht_cap.supported_mcs_set,
    ...
    if (bss->param.bss_config.wmm_para.qos_info == 0x80 ||
        ...
        memcpy( tlv_wmm_parameter->wmm_para.ouitype,
                           ...
                           IN pmlan_ioctl_req pioctl_buf, IN t_void * pdata_buf)
{
        ...
        if (pdata_buf) {
            ...
            case TLV_TYPE_MGMT_IE:
                ...
                if (ie && cust_ie->ie_data_list) {
                    ...
                    memcpy( ie, cust_ie->ie_data_list,
                           cust_ie->len);
                }
                break;
            default:
    ...
    if (pioctl_buf->req_id == MLAN_IOCTL_BSS) {
        ...
        if ((misc->sub_command == MLAN_OID_MISC_GEN_IE) &&
            ...
            if (cmd_action == HostCmd_ACT_GEN_SET)
                memcpy( ie, misc->param.gen_ie.ie_data,
                       misc->param.gen_ie.len);
        }
                           ...
                           IN mlan_ioctl_req * pioctl_buf)
{
    ...
    while (tlv_buf_left >= sizeof(MrvlIEtypesHeader_t)) {
        ...
        if (tlv_buf_left < (tlv_len + sizeof(MrvlIEtypesHeader_t))) {
            PRINTM(MERROR,
                   "Error processing uAP sys config TLVs, bytes left < TLV length:%d, %d\n", resp->size, tlv_buf_left);
        ...
        case TLV_TYPE_UAP_MAC_ADDRESS:
            tlv_mac = (MrvlIEtypes_MacAddr_t *) tlv;
            memcpy( &bss->param.bss_config.mac_addr,
        ...
        case TLV_TYPE_SSID:
            ...
            memcpy( bss->param.bss_config.ssid.ssid,
        ...
        case TLV_TYPE_RATES:
            tlv_rates = (MrvlIEtypes_RatesParamSet_t *) tlv;
            memcpy( bss->param.bss_config.rates,
        ...
        case TLV_TYPE_UAP_PSK:
            ...
            memcpy( bss->param.bss_config.psk, tlv_psk->psk,
        ...
        case HT_CAPABILITY:
            ...
            memcpy( bss->param.bss_config.supported_mcs_set,
    ...
    if (pioctl_buf) {
        ...
        if (pioctl_buf->req_id == MLAN_IOCTL_MISC_CFG) {
            ...
            if ((pioctl_buf->action == MLAN_ACT_GET) &&
                ...
                memcpy( &misc->param.cust_ie, cust_ie,
        ...
        case TLV_TYPE_UAP_MAC_ADDRESS:
            memcpy( pmpriv->curr_addr, tlv->mac,
                   MLAN_MAC_ADDR_LENGTH);
            break;
        case TLV_TYPE_UAP_CHAN_BAND_CONFIG:
                      ...
                      IN mlan_ioctl_req * pioctl_buf)
{
    ...
    if (psnmp_mib->query_type == HostCmd_ACT_GEN_GET) {
        ...
        while (tlv_buf_left >= sizeof(MrvlIEtypes_snmp_oid_t)) {
            ...
            memcpy( &data, psnmp_oid, sizeof(t_u32));
            switch (tlv_type) {
                        ...
                        IN HostCmd_DS_COMMAND * cmd, IN t_void * pdata_buf)
{
    ...
    memcpy( pcmd_sta_deauth->mac, deauth->mac_addr,
           MLAN_MAC_ADDR_LENGTH);
                          ...
                          IN t_u16 cmd_oid, IN t_void * pdata_buf)
{
    ...
    if (pkey->key_len &&
        memcmp(pkey->mac_addr, bc_mac, sizeof(bc_mac)))
    ...
    if (pkey->is_wapi_key) {
        ...
        if (0 != memcmp(pkey->mac_addr, bc_mac, sizeof(bc_mac))) {     /* WAPI 
            ...
            if ((sta_ptr = wlan_add_station_entry(pmpriv, pkey->mac_addr)) != 0) {
        ...
        memcpy( &pkey_material->key_param_set.key[2],
        ...
        memcpy( tlv->mac, pkey->mac_addr, MLAN_MAC_ADDR_LENGTH);
        cmd->size =
    ...
    if (pkey->key_len == WPA_AES_KEY_LEN &&
        ...
        if (memcmp(pkey->mac_addr, bc_mac, sizeof(bc_mac)))    /* AES 
                                                                                   pairwise 
                                                                                   key: 
                                                                                   unicast 
                                                                                 */
        ...
        if (memcmp(pkey->mac_addr, bc_mac, sizeof(bc_mac)))    /* TKIP 
                                                                                   pairwise 
                                                                                   key: 
                                                                                   unicast 
                                                                                 */
    ...
    if (pkey_material->key_param_set.key_type_id) {
        ...
        memcpy( pkey_material->key_param_set.key,
               pkey->key_material, pkey->key_len);
        pkey_material->key_param_set.length =
        ...
        if (pkey_material->key_param_set.key_type_id ==
            ...
            memcpy( param->ipn, pkey->pn, SEQ_MAX_SIZE);
            memcpy( param->key, pkey->key_material,
                   pkey->key_len);
                      ...
                      IN mlan_ioctl_req * pioctl_buf)
{
    ...
    if (pioctl_buf) {
        ...
        for (i = 0; i < info->param.sta_list.sta_count; i++) {
            memcpy( info->param.sta_list.info[i].mac_address,
...
wlan_check_uap_capability(pmlan_private priv, pmlan_buffer pevent)
{
    ...
    while (tlv_buf_left >= (int) sizeof(MrvlIEtypesHeader_t)) {
        ...
        if (tlv_type == HT_CAPABILITY) {
            DBG_HEXDUMP(MCMD_D, "HT_CAP tlv", (t_u8*)tlv,
        ...
        if (tlv_type == VENDOR_SPECIFIC_221) {
            if (!memcmp
                ...
                DBG_HEXDUMP(MCMD_D, "wmm ie tlv", (t_u8*)tlv,
                            tlv_len + sizeof(MrvlIEtypesHeader_t));
                priv->wmm_enabled = MFALSE;
        ...
        if (tlv_type == TLV_TYPE_UAP_PKT_FWD_CTL) {
            DBG_HEXDUMP(MCMD_D, "pkt_fwd tlv", (t_u8*)tlv,
                        tlv_len + sizeof(MrvlIEtypesHeader_t));
    ...
    while (tlv_buf_left >= (int) sizeof(MrvlIEtypesHeader_t)) {
        ...
        if (tlv_type == TLV_TYPE_AP_WAPI_INFO) {
            ...
            DBG_HEXDUMP(MCMD_D, "Fw:multicast_PN", wapi_tlv->multicast_PN,
                        PN_SIZE);
            memcpy( (t_u8 *) tx_pn, wapi_tlv->multicast_PN,
                   PN_SIZE);
            for (i = 0; i < 4; i++)
                tx_pn[i] = mlan_ntohl(tx_pn[i]);
            memcpy( wapi_tlv->multicast_PN, (t_u8 *) tx_pn,
                   PN_SIZE);
        ...
        if (tlv_type == TLV_TYPE_UAP_MGMT_FRAME) {
            ...
            if ((mgmt_tlv->frame_control.type == 0) &&
                ...
                assoc_req_ie =
                    (t_u8 *) tlv + sizeof(MrvlIETypes_MgmtFrameSet_t) +
                    assoc_ie_len;
                memcpy( pevent->event_buf + pevent->event_len,
                       assoc_req_ie, ie_len);
...
wlan_ops_uap_process_event(IN t_void * priv)
{
    ...
    memset( &pevent->event_id, 0, sizeof(pevent->event_id));
    ...
    case EVENT_MICRO_AP_BSS_START:
        ...
        memcpy( pmpriv->curr_addr, pmadapter->event_body + 2,
               MLAN_MAC_ADDR_LENGTH);
    ...
    case EVENT_MICRO_AP_STA_ASSOC:
        ...
        memcpy( sta_addr, pmadapter->event_body + 2,
               MLAN_MAC_ADDR_LENGTH);
    ...
    case EVENT_RADAR_DETECTED:
        ...
        memset( event_buf, 0x00, MAX_EVENT_SIZE);
        pevent->bss_index = pmpriv->bss_index;
        pevent->event_id = MLAN_EVENT_ID_DRV_PASSTHRU;
