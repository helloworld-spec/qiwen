                         ...
                         IN pmlan_ioctl_req pioctl_req)
{
    ...
    for (i = 0; i < pmadapter->max_mgmt_ie_index; i++) {
        memset( &pmpriv->mgmt_ie[i], 0, sizeof(custom_ie));
    }
...
wlan_uap_set_wapi_ie(mlan_private * priv, pmlan_ioctl_req pioctl_req)
{
	...
	if (misc->param.gen_ie.len) {
		if(misc->param.gen_ie.len > priv->max_wapi_ie_len)
		{
			if((misc->param.gen_ie.len + 32) > 256)
							misc->param.gen_ie.len = 256;
						
			p_err("ie_len err:%d, need:%d", priv->max_wapi_ie_len, misc->param.gen_ie.len);
			if(priv->wapi_ie)
				mem_free(priv->wapi_ie);
			
			priv->wapi_ie = (t_u8*)mem_calloc(misc->param.gen_ie.len + 32, 1);
			
			if(!priv->wapi_ie){
				priv->max_wapi_ie_len = 0;
				return MLAN_STATUS_FAILURE;
			}else
				priv->max_wapi_ie_len = misc->param.gen_ie.len + 32;
		}
        ...
        memcpy( priv->wapi_ie, misc->param.gen_ie.ie_data,
...
#if WAPI_IE_ALLOC_IN_USE
			memset( pmpriv->wapi_ie, 0,
                   pmpriv->max_wapi_ie_len);
#else
            memset( pmpriv->wapi_ie, 0,
...
wlan_uap_get_bss_info(IN pmlan_adapter pmadapter, IN pmlan_ioctl_req pioctl_req)
{
    ...
    /* BSSID */
    memcpy( &info->param.bss_info.bssid, pmpriv->curr_addr,
           MLAN_MAC_ADDR_LENGTH);
...
wlan_uap_get_channel(IN pmlan_private pmpriv)
{
    ...
    ENTER();
    memset( &tlv_chan_band, 0, sizeof(tlv_chan_band));
...
wlan_ops_uap_ioctl(t_void * adapter, pmlan_ioctl_req pioctl_req)
{
    ...
    case MLAN_IOCTL_GET_INFO:
        ...
        if (pget_info->sub_command == MLAN_OID_GET_VER_EXT)
            ...
            memcpy( &pget_info->param.fw_info.mac_addr,
