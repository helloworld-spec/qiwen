...
wlan_ops_uap_process_txpd(IN t_void * priv, IN pmlan_buffer pmbuf)
{
    ...
    if (pmbuf->buf_type == MLAN_BUF_TYPE_RAW_DATA) {
        memcpy( &pkt_type, pmbuf->pbuf + pmbuf->data_offset,
               sizeof(pkt_type));
        memcpy( &tx_control,
    ...
    memset( plocal_tx_pd, 0, sizeof(UapTxPD));
    ...
    if (prx_pkt->eth803_hdr.dest_addr[0] & 0x01) {
        if (!(priv->pkt_fwd & PKT_FWD_INTRA_BCAST)) {
            ...
            if ((newbuf =
                ...
                /* copy the data */
                memcpy( (t_u8 *) newbuf->pbuf + newbuf->data_offset,
...
wlan_process_uap_rx_packet(IN mlan_private * priv, IN pmlan_buffer pmbuf)
{
    ...
    if (prx_pkt->eth803_hdr.dest_addr[0] & 0x01) {
		//wmj modi 20160907 do not need pkt forward  in our platform
		//if (!(priv->pkt_fwd & PKT_FWD_INTRA_BCAST)) {
		if ((priv->pkt_fwd & PKT_FWD_INTRA_BCAST)) {
            /* Multicast pkt */
            if ((newbuf =
