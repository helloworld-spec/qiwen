...
/** WMM information IE */
const static t_u8 wmm_info_ie[] = { WMM_IE, 0x07,
    0x00, 0x50, 0xf2, 0x02,
    0x00, 0x01, 0x00
};
 ...
 */
const static t_u8 wmm_aci_to_qidx_map[] = { WMM_AC_BE,
    WMM_AC_BK,
    WMM_AC_VI,
    WMM_AC_VO
};
...
const static mlan_wmm_ac_e tos_to_ac[] = { WMM_AC_BE,
    WMM_AC_BK,
    WMM_AC_BK,
    WMM_AC_BE,
    WMM_AC_VI,
...
{
//wmj+ do not all downgrade as wmm ACM is not set
#if 0
                           ...
                           t_u8 * srcAddr, t_u8 tid, t_u8 up, t_u8 status)
{
    ...
    memcpy( pOutBuf, (t_u8 *) "TSPEC:", 6);
    pOutBuf += 6;

    memcpy( pOutBuf, (t_u8 *) typeStr, wlan_strlen(typeStr));
    ...
    *pOutBuf++ = status + '0';
    ...
    memcpy( pOutBuf, (t_u8 *) "]:TID", 5);
    pOutBuf += 5;
    *pOutBuf++ = tid + '0';

    memcpy( pOutBuf, (t_u8 *) ":UP", 3);
    pOutBuf += 3;
    *pOutBuf++ = up + '0';
...
wlan_ralist_update(mlan_private * priv, t_u8 * old_ra, t_u8 * new_ra)
{
    ...
    for (tid = 0; tid < MAX_NUM_TID; ++tid) {
        ...
        if (ra_list) {
            ...
            memcpy( ra_list->ra, new_ra, MLAN_MAC_ADDR_LENGTH);
        }
    }
...
wlan_wmm_add_buf_txqueue(pmlan_adapter pmadapter, pmlan_buffer pmbuf)
{
    ...
    if (!queuing_ra_based(priv)) {
        ...
        memcpy( ra, pmbuf->pbuf + pmbuf->data_offset,
        ...
        if (ra[0] & 0x01)
            ...
            if ((sta_ptr = wlan_get_station_entry(priv, ra)) != 0) {
                if (!sta_ptr->is_wmm_enabled) {
                   //wmj- use the same tid as wmm enabled 
                                 ...
                                 IEEEtypes_HTCap_t * pHTCap)
{
    ...
    if ((priv->wmm_required
        ...
        memcpy( pwmm_tlv->wmm_ie, &wmm_info_ie[2],
               pwmm_tlv->header.len);
 ...
 *  @return        N/A
 */  //zoutx: 修改这个函数增加返回值判断
t_u8 wlan_wmm_process_tx(pmlan_adapter pmadapter)
{
    t_u8 ret_val = MTRUE;
    ENTER();
    ...
    do {
        ...
        {
            //zoutx :  在wlan_dequeue_tx_packet函数中返回MLAN_STATUS_FAILURE是因为产生了PAUSE
        	ret_val = MFALSE;
			printf("MMM...\n");  //zoutx: 出现MMM打印是代表严重的错误，实际测试中不应该出现MMM打印
            break;
        }
        /* Check if busy */
...
int wlan_get_ralist_pause_status(IN pmlan_adapter pmadapter, IN t_u8 *mac, OUT t_u8 *pause, OUT t_u16 *pkts_queued)
{
	pmlan_private priv_tmp;
    raListTbl *ra_list;
    mlan_bssprio_node *bssprio_node, *bssprio_head;
		...
		for ( j= pmadapter->priv_num - 1; j >= 0; --j) 
		{
			if (!(util_peek_list(pmadapter->pmoal_handle,
				...
				do {
					...
					for (i = 0; i < MAX_NUM_TID; ++i)
					{
							ra_list = wlan_wmm_get_ralist_node(priv_tmp, i, mac);
							if (ra_list->tx_pause) 
							{
								PRINTM(MINFO, "got paused sta ralist %p, ra [%02x:%02x:%02x:%02x:%02x:%02x], tx_pause %d, total_pkts:%d\n", 
										ra_list, ra_list->ra[0], ra_list->ra[1], ra_list->ra[2], 
										ra_list->ra[3], ra_list->ra[4], ra_list->ra[5], ra_list->tx_pause,ra_list->total_pkts);
								
								*pause = ra_list->tx_pause;
								*pkts_queued= ra_list->total_pkts;
...
/**
*This function return WiFi pause status if any sta paused return this station's status.
*param pmadapter   A pointer to mlan_adapter structure
*param pause:  pause status get from ralist , to be setted in this funtion, 0 or 1
*param pkts:   packts in ralist tx queue, to be setted in this function.
*
...
int wlan_get_pause_status(IN pmlan_adapter pmadapter, OUT t_u8 *pause, OUT t_u16 *pkts_queued)
{
	pmlan_private priv_tmp;
    raListTbl *ra_list, *ra_head;
    mlan_bssprio_node *bssprio_node, *bssprio_head;
    tid_tbl_t *tid_ptr;
    int i, j;
    int next_prio = 0;
	...
	ENTER();
	printf("enter wlan_get_pause_status\n");

	if(pmadapter)
	{
		printf("get pause status:\n");
		for ( j= pmadapter->priv_num - 1; j >= 0; --j) 
		{
			if (!(util_peek_list(pmadapter->pmoal_handle,
							 &pmadapter->bssprio_tbl[j].bssprio_head,
				...
				do {
					...
					{
						ra_list = (raListTbl *) util_peek_list(pmadapter->pmoal_handle,
                                               &priv_tmp->wmm.tid_tbl_ptr[i].ra_list, MNULL, MNULL);

						
						//ralist is a round linked list and the head is a list structure without ralist element.						
						if(ra_list && (ra_list != (raListTbl *) & priv_tmp->wmm.tid_tbl_ptr[i].ra_list))
						{
							do 
							{
								if(ra_list->tx_pause)
							...
							}while(ra_list && (ra_list != (raListTbl *) & priv_tmp->wmm.tid_tbl_ptr[i].ra_list));
						}
					}
