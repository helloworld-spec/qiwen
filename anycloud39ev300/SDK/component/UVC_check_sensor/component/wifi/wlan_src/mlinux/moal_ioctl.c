...
#define MEAS_REPORT_TIME 60*OS_TICK_RATE_HZ
...
const static region_code_mapping_t region_code_mapping[] = {
    {"US ", 0x10},              /* US FCC */
    {"CA ", 0x20},              /* IC Canada */
    {"SG ", 0x10},              /* Singapore */
    {"EU ", 0x30},              /* ETSI */
                         ...
                         t_u8 wait_option)
{
    ...
    case MOAL_IOCTL_WAIT:
        ...
        break;
    case MOAL_CMD_WAIT:
       ret = wait_event_timeout(priv->cmd_wait_q, 3000);
        break;
#ifdef CONFIG_PROC_FS
    case MOAL_PROC_WAIT:
    ...
    if (wait->condition == MFALSE) {
        ...
        PRINTM(MIOCTL,
               "IOCTL cancel: %p id=0x%x, sub_id=0x%x, wait_option=%d, action=%d, status=%d\n",
...
//    unsigned long flags;
    ...
    if ((req->action == MLAN_ACT_SET || req->action == 0) &&
        ...
        cac_left_jiffies = MEAS_REPORT_TIME -
            (os_ticks_get() - priv->phandle->meas_start_jiffies);
        ...
        if (sub_command == MLAN_OID_BSS_START) {
            ...
            if (priv->phandle->delay_bss_start == MFALSE) {
                PRINTM(MMSG, "Received BSS Start command during CAC period, "
                       "delay executing %ld seconds\n", cac_left_jiffies / OS_TICK_RATE_HZ);
...
/**
*wmj+ get tx_pending, let upper layer know how many tx packets pending in driver
*@param dev      A pointer to net_device structure
 *  @param req      A pointer to ifreq structure
 *  @return         0 --success, otherwise fail
*/
int
woal_get_tx_pending(struct net_device *dev, struct ifreq *req)
{
	int ret = 0;
	moal_private *priv = (moal_private *) netdev_priv(dev);
	u8 tx_pending;
	...
	ENTER();
	tx_pending = atomic_read(&priv->phandle->tx_pending);
	if (copy_to_user(req->ifr_ifru.ifru_data, &tx_pending, sizeof(u8))) {
		PRINTM(MERROR, "Copy to user failed!\n");
		ret = -EFAULT;
	}
...
/**
*wmj+ set max_tx_pending, set max packets can pending in driver.
*@param dev      A pointer to net_device structure
 *  @param req      A pointer to ifreq structure
 *  @return         0 --success, otherwise fail
*/
int
woal_set_max_tx_pending(struct net_device *dev, struct ifreq *req)
{
	int ret = 0;
	moal_private *priv = (moal_private *) netdev_priv(dev);
	u8 max_tx_pending;
	...
	ENTER();
	if (copy_from_user(&max_tx_pending, req->ifr_ifru.ifru_data, sizeof(u8))) {
		PRINTM(MERROR, "Copy from user failed!\n");
		ret = -EFAULT;
	}
	priv->phandle->max_tx_pending = max_tx_pending;

	LEAVE();
	return ret;
}
/**
*wmj+ get max_tx_pending, set max packets can pending in driver.
*@param dev      A pointer to net_device structure
 *  @param req      A pointer to ifreq structure
 *  @return         0 --success, otherwise fail
*/
int
woal_get_max_tx_pending(struct net_device *dev, struct ifreq *req)
{
	int ret = 0;
	moal_private *priv = (moal_private *) netdev_priv(dev);
	u8 max_tx_pending;
	...
	ENTER();
	max_tx_pending = priv->phandle->max_tx_pending;
	if (copy_to_user(req->ifr_ifru.ifru_data, &max_tx_pending, sizeof(u8))) {
		PRINTM(MERROR, "Copy to user failed!\n");
		ret = -EFAULT;
	}
...
woal_11h_channel_check_ioctl(moal_private * priv)
{
    ...
    /* set flag from here */
    priv->phandle->cac_period = MTRUE;
    priv->phandle->meas_start_jiffies = os_ticks_get();
...
woal_set_band(moal_private * priv, char *pband)
{
    ...
    if (*pband == '0') {
        ...
        if (IS_STA_OR_UAP_CFG80211(cfg80211_wext) && priv->wdev &&
            ...
            priv->wdev->wiphy->bands[IEEE80211_BAND_2GHZ] = (struct ieee80211_supported_band*)&cfg80211_band_2ghz;
        }
#endif
    } else if (*pband == '1') {
        ...
        if (IS_STA_OR_UAP_CFG80211(cfg80211_wext) && priv->wdev &&
            priv->wdev->wiphy) {
            priv->wdev->wiphy->bands[IEEE80211_BAND_5GHZ] = (struct ieee80211_supported_band*)&cfg80211_band_5ghz;
