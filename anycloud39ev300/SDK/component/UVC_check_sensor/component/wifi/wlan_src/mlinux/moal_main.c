...
********************************************************/
#ifdef FW_OUTSIDE
#define FIRMWARE_SPIFILENAME      "WIFI"
#endif
...
//wmj+ for skb default priority, 5 is tested better than 7 for video in 1KB packets
#define DEFAULT_TID 5
//wmj<<
...
/** Mfg mode */
const int mfg_mode = 0;
#endif

/** SDIO interrupt mode (0: INT_MODE_SDIO, 1: INT_MODE_GPIO) */
const int intmode = INT_MODE_SDIO;
...
const int max_wfd_bss = DEF_WIFIDIRECT_BSS;
...
const int cfg80211_wext = STA_WEXT_MASK | UAP_WEXT_MASK | STA_CFG80211_MASK | UAP_CFG80211_MASK;

/** Work queue priority */
const int wq_sched_prio = 0;
...
int woal_sta_do_ioctl(struct net_device *dev, struct ifreq *req, int cmd);
...
#ifdef FW_OUTSIDE
extern T_WIFI_INITPARAM wifi_initparam; 
#endif
...
woal_init_sw(moal_handle * handle)
{
    ...
    woal_init_timer(&handle->reassoc_timer,
    ...
    woal_init_timer(&handle->go_timer, woal_go_timer_func, handle);

    handle->is_go_timer_set = MFALSE;
#endif
#endif
//wmj+ for max tx_pending support
...
woal_init_fw_dpc(moal_handle * handle)
{
    ...
    if (handle/*->firmware*/) {
        ...
        PRINTM(MMSG, "WLAN FW is active1\n");
    }
        /** Cal data request */
	...
	//zoutx add
	if (woal_start_irq_scan(handle) != MLAN_STATUS_SUCCESS) {
			PRINTM(MFATAL, "Failed to register irq !\n");
			goto done;
	}
...
#endif
#ifdef FW_OUTSIDE
	cache = wifi_initparam.cache;
	fw_size = wifi_initparam.size;
	if ((NULL == cache) || (0 == fw_size)
		|| (0 > Fwl_SpiFile_ReadToCacheByName(FIRMWARE_SPIFILENAME, cache, &fw_size)))
    {
    	ret = MLAN_STATUS_FAILURE;
	}
	else
	{
	    handle->firmware.data = cache;
		handle->firmware.size = fw_size;
	}
#else
	handle->firmware.data = (u8*)fm_88w;
	handle->firmware.size = sizeof(fm_88w);
#endif
    if (ret != MLAN_STATUS_FAILURE)
    {
		ret = woal_request_fw_dpc(handle, &handle->firmware);
                      ...
                      mlan_buffer * pmbuf, struct sk_buff *skb)
{
	...
	//wmj+ set all packet tid to DEFAULT tid
	tid = DEFAULT_TID;
	//wmj<<
    skb->priority = tid;
...
const struct net_device_ops woal_netdev_ops = {
	...
	woal_sta_do_ioctl,  /*wmj+ for txpending setting*/
/*
    .ndo_open = woal_open,
...
#pragma arm section rodata ="_video_server_rodata_"
...
#define MAX_WMM_QUEUE   4
	...
	{
		PRINTM(MFATAL, "bss type %d not supported\n", bss_type);
        goto error;
	}
	dev->ml_priv = dev + 1;
 ...
 */
int
woal_sta_do_ioctl(struct net_device *dev, struct ifreq *req, int cmd)
{
    int ret = 0;
    ENTER();
    PRINTM(MIOCTL, "sta_do_ioctl: ioctl cmd = 0x%x\n", cmd);
    switch (cmd) {
    //wmj+
#ifdef MAX_TX_PENDING_CTRL
	//wmj+ get  tx_pending number
	case UAP_GET_TX_PENDING:
		ret = woal_get_tx_pending(dev, req);
		break;
	//wmj+ set max tx_pending allowed
	case UAP_SET_MAX_TX_PENDING:
		ret = woal_set_max_tx_pending(dev, req);
		break;
	//wmj+ get max tx_pending allowed
	case UAP_GET_MAX_TX_PENDING:
		ret = woal_get_max_tx_pending(dev, req);
		break;
#endif 
//<<
    default:
    	PRINTM(MIOCTL, "sta_do_ioctl: unknown cmd\n");
        break;
    }
...
woal_tx_timeout(struct net_device *dev)
{
    ...
    PRINTM(MERROR, "%lu : %s (bss=%d): Tx timeout (%d)\n",
           os_ticks_get(), dev->name, priv->bss_index, priv->num_tx_timeout);
...
woal_hard_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
    ...
    PRINTM(MDATA, "%lu : %s (bss=%d): Data <= kernel\n",
           os_ticks_get(), dev->name, priv->bss_index);
...
//wmj+ 20160912 max queued pkts limit here
#ifdef MAX_TX_PENDING_CTRL
		if (atomic_read(&priv->phandle->tx_pending) >= priv->phandle->max_tx_pending)
		{
			PRINTM(MERROR, "packets pending %d exceeded max_tx_pending:%d drop this packets\n", atomic_read(&priv->phandle->tx_pending), priv->phandle->max_tx_pending);
			status = MLAN_STATUS_FAILURE;
		}
		else
		{
...
woal_reset_intf(moal_private * priv, t_u8 wait_option, int all_intf)
{
    ...
    if (!handle->reassoc_on && handle->is_reassoc_timer_set) {
        woal_stop_timer(&handle->reassoc_timer);
        handle->is_reassoc_timer_set = MFALSE;
    }
#endif /* REASSOCIATION */
    ...
    if (handle->is_go_timer_set) {
        woal_stop_timer(&handle->go_timer);
        handle->is_go_timer_set = MFALSE;
    }
#endif
#endif
...
woal_reassociation_thread(void *data)
{
    ...
    //mlan_status status;
    mlan_bss_info bss_info;
    t_u32 timer_val = MOAL_TIMER_1S;
	char *essid, *key;
	struct cfg80211_connect_params smee_para = {0};
	struct cfg80211_connect_params *smee = &smee_para;
    ...
    for (;;) {
        ...
        for (i = 0; i < handle->priv_num && ((priv = handle->priv[i]) != 0); i++) {
			...
			/*set ssid password and try to connect*/	
			memcpy(smee->ssid, priv->prev_ssid_bssid.ssid.ssid, priv->prev_ssid_bssid.ssid.ssid_len);
			smee->ssid[priv->prev_ssid_bssid.ssid.ssid_len] = 0;
			smee->ssid_len = priv->prev_ssid_bssid.ssid.ssid_len;
	
			memcpy(smee->key, priv->prev_passphrase.passphrase, priv->prev_passphrase.passphrase_len);
			smee->key[priv->prev_passphrase.passphrase_len] = 0;
			smee->key_len = priv->prev_passphrase.passphrase_len;
			
			priv->netdev->cfg80211_ops->wifi_connect(priv->wdev->wiphy, priv->netdev,smee);
#if 0
        ...
        if (reassoc_timer_req == MTRUE) {
            ...
            if (priv->set_asynced_essid_flag == MTRUE) {
                ...
                woal_start_timer(&handle->reassoc_timer, timer_val);
            }
        } else {
            priv->set_asynced_essid_flag = MFALSE;
        }
...
woal_reassoc_timer_func(void *context)
{
    ...
    PRINTM(MINFO, "reassoc_timer timeout.\n");
    handle->is_reassoc_timer_set = MFALSE;
...
//wmj+ this routing not used
#if 0

/**
*This function return packets number pending in this layer.
*param pause:  pause status get from ralist , to be setted in this funtion, 0 or 1
*param pkts:   packts in ralist tx queue, to be setted in this function.
*
...
int woal_get_pause_status(OUT u8 *pause, OUT u16 *pkts )
{
	moal_handle *handle;
	...
	handle = card->handle;
	ret = wlan_get_pause_status(handle->pmlan_adapter, pause, pkts);
	LEAVE();
	return ret;
}
...
/**
*This function return WiFi pause status with given sta mac
*param mac:  station mac addr 
*param pause:  pause status get from ralist , to be setted in this funtion, 0 or 1
*param pkts:   packts in ralist tx queue, to be setted in this function.
*
*return:  SUCCESS or FAILURE
...
int woal_get_ra_pause_status(IN u8 *mac, OUT u8 *pause, OUT u16 *pkts )
{
	moal_handle *handle;
	struct sdio_mmc_card *card;
	int ret;
	...
	handle = card->handle;
	ret = wlan_get_ralist_pause_status(handle->pmlan_adapter, mac, pause, pkts);
	LEAVE();
	return ret;
}
#endif
...
//zoutx:  这是给外界调用的判断队列是否已经全部发送完成。
int woal_tx_list_empty(struct sdio_func *func)
{
	    moal_handle *handle;
		struct sdio_mmc_card *card;
		int ret;
		...
		if(!card)
		{
			PRINTM(MFATAL, "Failed to get drvcard!\n");	
			while(1);
			return;
		}
		
		handle = card->handle;
		ret= wlan_wmm_lists_empty(handle->pmlan_adapter);
		//printf("ret = %x\n", ret);
		LEAVE();
		return ret;
}
 ...
 */
uint32_t reassociation_thread_statck[REASSOCIATION_THREAD_STACK_SIZE];
moal_handle *
...
//    }
    ...
    //wmj- 20170309 for there is a main thread run mlan_main_process
