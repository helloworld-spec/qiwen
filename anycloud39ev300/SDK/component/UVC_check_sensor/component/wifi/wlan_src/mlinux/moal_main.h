...
#define	PRINTM_MINFO(...)  do {if (drvdbg & MINFO) printk(KERN_DEBUG __VA_ARGS__);} while(0)
#define	PRINTM_MWARN(...)  do {if (drvdbg & MWARN) printk(KERN_DEBUG __VA_ARGS__);} while(0)
#define	PRINTM_MENTRY(...) do {if (drvdbg & MENTRY) printk(KERN_DEBUG __VA_ARGS__);} while(0)
#define	PRINTM_MFW_D(...)  do {if (drvdbg & MFW_D) printk(KERN_DEBUG __VA_ARGS__);} while(0)
#define	PRINTM_MCMD_D(...) do {if (drvdbg & MCMD_D) printk(KERN_DEBUG __VA_ARGS__);} while(0)
#define	PRINTM_MDAT_D(...) do {if (drvdbg & MDAT_D) printk(KERN_DEBUG __VA_ARGS__);} while(0)
#define	PRINTM_MIF_D(...)  do {if (drvdbg & MIF_D) printk(KERN_DEBUG __VA_ARGS__);} while(0)
#define	PRINTM_MIOCTL(...) do {if (drvdbg & MIOCTL) printk(KERN_DEBUG __VA_ARGS__);} while(0)
#define	PRINTM_MINTR(...)  do {if (drvdbg & MINTR) printk(KERN_DEBUG __VA_ARGS__);} while(0)
#define	PRINTM_MEVENT(...) do {if (drvdbg & MEVENT) printk(__VA_ARGS__);} while(0)
#define	PRINTM_MCMND(...)  do {if (drvdbg & MCMND) printk(KERN_DEBUG __VA_ARGS__);} while(0)
#define	PRINTM_MDATA(...)  do {if (drvdbg & MDATA) printk(KERN_DEBUG __VA_ARGS__);} while(0)
#define	PRINTM_MERROR(...) do {if (drvdbg & MERROR) printk(KERN_ERR __VA_ARGS__);} while(0)
#define	PRINTM_MFATAL(...) do {if (drvdbg & MFATAL) printk(KERN_ERR __VA_ARGS__);} while(0)
#define	PRINTM_MMSG(...)   do {if (drvdbg & MMSG) printk(KERN_ALERT __VA_ARGS__);} while(0)

#define	PRINTM(level,...) PRINTM_##level(__VA_ARGS__)
#else
#define	PRINTM(level,...) do {} while (0)
#endif /* DEBUG_LEVEL1 */
...
/** Log entry point for debugging */
#define	ENTER_INTR()			PRINTM(MENTRY, "Enter: %s\n", \
                                    __func__ )
/** Log exit point for debugging */
#define	LEAVE_INTR()			PRINTM(MENTRY, "Leave: %s\n", \
                                    __func__ )
...
woal_timer_handler(unsigned long fcontext)
{
    ...
    if (timer->timer_is_periodic == MTRUE) {
        os_start_timer(timer->timer_id, /*jiffies + ((timer->time_period * HZ) / 1000)*/timer->time_period);
    } else {
 ...
 */
static inline void
woal_init_timer(pmoal_drv_timer timer,
                      ...
                      void *FunctionContext)
{
    ...
    os_create_timer(&timer->timer_id, 0, TimerFunction, FunctionContext);
 ...
 */
static inline void
woal_start_timer(pmoal_drv_timer timer, t_u32 MillisecondPeriod)
{
	timer->time_period = MillisecondPeriod;
    os_start_timer(timer->timer_id, /*jiffies + (MillisecondPeriod * HZ) / 1000*/MillisecondPeriod);
    timer->timer_is_canceled = MFALSE;
}
 ...
 */
static inline void
woal_stop_timer(moal_drv_timer * timer)
{
    os_stop_timer(timer->timer_id);
    timer->timer_is_canceled = MTRUE;
    timer->time_period = 0;
}
...
#define MLAN_INIT_WORK(_work, _fun)                 init_work(_work, (void *)_fun, _work)
/** Work_queue delayed work initialization */
#define MLAN_INIT_DELAYED_WORK(_work, _fun)         init_work(_work, (void *)_fun, _work)
...
#define MOAL_INIT_SEMAPHORE(x)    	*(x) = mutex_init(__func__ )
/** Initialize semaphore */
...
#define MRVDRV_DEFAULT_WATCHDOG_TIMEOUT (5 * OS_TICK_RATE_HZ)
...
*/
#define MRVDRV_DEFAULT_UAP_WATCHDOG_TIMEOUT (41 * OS_TICK_RATE_HZ)
#endif
...
#define HS_ACTIVE_TIMEOUT  (2 * OS_TICK_RATE_HZ)
#endif
