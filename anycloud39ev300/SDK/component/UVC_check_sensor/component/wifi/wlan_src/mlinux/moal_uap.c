...
woal_uap_dfs_testing(struct net_device *dev, struct ifreq *req)
{
    ...
    if (!param.action) {
        ...
        priv->phandle->cac_period_jiffies = param.usr_cac_period * OS_TICK_RATE_HZ / 1000;
    }
    if (MLAN_STATUS_SUCCESS !=
        woal_request_ioctl(priv, ioctl_req, MOAL_IOCTL_WAIT)) {
        ret = -EFAULT;
...
woal_uap_ap_cfg_parse_data(mlan_uap_bss_param * ap_cfg, t_s8 * buf)
{
    ...
    t_s8 *opt = buf, *value = NULL;
#ifdef UAP_CFG11N	
	int set_11n = 0;//wmj+
	int dot11n_enable = 0; //wmj+
#endif
    ...
    do
	{
		if (!strncmp(opt, "SSID=", strlen("SSID=")))
		{
			value = opt + strlen("SSID=");
			if (set_ssid)
			{
		...
		else if(!strncmp(opt, "11N_MODE=", strlen("11N_MODE=")))
		{
			value = opt + strlen("11N_MODE=");
        	if(set_11n)
        	{
        		PRINTM(MWARN, "Skipping 11n_mode, found again!\n");
                continue;
        	}
			if(!strncmp(value, "1", strlen("1")))
			{
				dot11n_enable = 1;
			}
			set_11n = 1;
		...
		else if (!strncmp(opt, "AP_CFG_END", strlen("AP_CFG_END"))) 
		{
			PRINTM(MERROR, "parse ap config end\n");
			break;
		}
		else
		{
    ...
    }while(*(opt += strlen(opt) + 1));/*config option delimite with '\0'*/
	...
	if(dot11n_enable)
	{
		PRINTM(MMSG, "11n mcs is enabled\n");
		woal_uap_set_11n_status(ap_cfg, MLAN_ACT_ENABLE);
	}
	else
	{
		PRINTM(MMSG, "11n mcs is disabled\n");
		woal_uap_set_11n_status(ap_cfg, MLAN_ACT_DISABLE);
	}
#endif
	//wmj<<
...
#define MIN_AP_CFG_CMD_LEN   16 /* strlen("ASCII_CMD=AP_CFG") */
	...
	//wmj+ for tx power setting debug
	//sys_config.tx_power_level = 15;
...
//wmj put this 11n setting into woal_uap_ap_cfg_parse_data() funtion 
//for giving a independent 11n interface to upper layer
#ifndef UAP_CFG11N 
...
woal_uap_do_ioctl(struct net_device *dev, struct ifreq *req, int cmd)
{
    ...
    case UAP_BSS_CTRL:
        ...
        break;
	//wmj- this cmd used for getting max tx_pending
	//case UAP_WAPI_MSG:
...
//wmj+
void
woal_print_sys_config(mlan_uap_bss_param * config)
{
    printf("**************ap config*******************\n");
	printf("ssid:      %s\n", config->ssid.ssid);
	printf("ssid len:  %d\n", config->ssid.ssid_len);
	printf("channel:   %d\n", config->channel);
	printf("auth mode: %d\n", config->auth_mode);
	printf("protocol:  %d\n", config->protocol);
	printf("key_mgmt:  %d\n", config->key_mgmt);
	printf("key_len:   %d\n", config->wpa_cfg.length);
	printf("passphrase:%s\n", config->wpa_cfg.passphrase);
	printf("**************ap config end*******************\n");
