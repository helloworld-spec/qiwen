...
#define VIDEO_LIB_VERSION			"AK39 Encoder Lib V1.9.18" 
...
//2015.12.11加入宏块级别码率控制&丢帧策略是否打开接口	
	...
	T_U32	roi1Area; // 1为enable，0为disable
	T_U32	roi1Areatop; // roiarea top
	T_U32	roi1Areabot; // roiarea bottom
	T_U32	roi1Arealeft; // roiarea left
	T_U32	roi1Arearight; // roiarea right
	T_S32	roi1DeltaQp; // delta QP
...
typedef struct _VIDEOLIB_PARA
{
	...
	T_S32		quarterPixelMv;
	T_S32		qp_filter_k;
	T_S32		videomode;//设置视频模式，1为VBR，2为CBR
	T_S32		model;//码率控制强度，0为质量最高，100为码率最好
	T_S32		Isize;//I 帧 大小控制
	T_S32		method;//强制I帧控制开关
	T_S32		ROIenable;//ROI 开启，1 enable ,0 disable
	T_S32		ROItop;
...
//T_VOID VideoStream_Enc_get_bps(T_pVOID hVS1, T_VIDEOLIB_ENC_PARA *video_enc_para);

T_VOID VideoStream_Enc_set_GOP(T_pVOID hVS1, T_VIDEOLIB_ENC_PARA *video_enc_para);

T_VOID VideoStream_Enc_set_fps(T_pVOID hVS1, T_VIDEOLIB_ENC_PARA *video_enc_para);

T_VOID VideoStream_Enc_set_videomode(T_pVOID hVS1, T_VIDEOLIB_ENC_PARA *video_enc_para);

T_VOID VideoStream_Enc_set_Rc_Param(T_pVOID hVS1, T_VIDEOLIB_ENC_PARA *video_enc_para);

T_VOID VideoStream_Enc_enviroment(T_pVOID hVS1, T_VIDEOLIB_ENC_PARA *video_enc_para);

T_VOID MJPEG_Enc_qlevel(T_pVOID hVS1, T_VIDEOLIB_PARA *video_enc_para);
