...
int  ak_isp_vo_disable_buffer(enum buffer_id  id)
{
    ...
    isp_dbg("%s enter:id=%d, reg0=0x%x\n", __func__, id, REG32(isp->base + 0x14));
...
#else
		isp->reg_blkaddr5[31] = isp->reg_blkaddr5[28];
		isp->reg_blkaddr5[32] = isp->reg_blkaddr5[29]; //sub_u1 
        isp->reg_blkaddr5[33] = isp->reg_blkaddr5[30];  //sub_v1 
#endif
        break;
    case BUFFER_TWO:
...
#else
		isp->reg_blkaddr1[49] = isp->reg_blkaddr1[40];
		isp->reg_blkaddr1[52] = isp->reg_blkaddr1[43]; //sub_u1 
        isp->reg_blkaddr1[55] = isp->reg_blkaddr1[46];  //sub_v1 
#endif
...
#else
		isp->reg_blkaddr1[50] = isp->reg_blkaddr1[41];
		isp->reg_blkaddr1[53] = isp->reg_blkaddr1[44]; //sub_u1 
        isp->reg_blkaddr1[56] = isp->reg_blkaddr1[47];  //sub_v1 
#endif
        break;
    case BUFFER_FOUR:
...
#else
		isp->reg_blkaddr1[51] = isp->reg_blkaddr1[42];
		isp->reg_blkaddr1[54] = isp->reg_blkaddr1[45]; //sub_u1 
        isp->reg_blkaddr1[57] = isp->reg_blkaddr1[48];  //sub_v1 
#endif
        break;
    default:
        break;
...
int _isp_vo_set_stat_addr(enum buffer_id  id,unsigned long  stat_phy_addr, void *stat_virtual_addr)
{
	...
	{
	case BUFFER_ONE:
		isp->reg_blkaddr5[34] = stat_phy_addr;    //y1
		isp->stat_addr[0] = (T_U32*)stat_virtual_addr;
		break;
	case BUFFER_TWO:
		isp->reg_blkaddr1[58] = stat_phy_addr;   //y2
		isp->stat_addr[1] = (T_U32*)stat_virtual_addr;
		break;
	case BUFFER_THREE:
		isp->reg_blkaddr1[59] = stat_phy_addr;   //y3
		isp->stat_addr[2] = (T_U32*)stat_virtual_addr;
		break;
	case BUFFER_FOUR:
		isp->reg_blkaddr1[60] = stat_phy_addr;   //y4
...
int ak_isp_vo_get_using_frame_buf_id(void)
{
	...
	cmd = REG32(isp->base + 0x14); //lz0499 9_13
	cmd = (cmd>>28)&0x0f;
	cnt = 0;
	for(i=0; i<4; i++)
	{
...
int ak_isp_vi_set_crop(int sx, int sy, int width, int height)
{
    ...
    int i, g;
	int height_block_size=0,height_block_num_flg=0;
...
int ak_isp_vo_set_main_channel_scale(int width, int height)
{
    ...
    //*pReg++ = cmd;
    isp->reg_blkaddr4[47] = cmd;;

    //pp_reg_49
    cmd = 0;
    cmd = (LOW_BITS(isp->ch1_width, 13) << PP_DST1_WIDTH) 
    	| (LOW_BITS(isp->ch1_height, 13) << pp_DST1_HEIGHT);
    //*pReg++ = cmd;
    isp->reg_blkaddr4[48] = cmd;
...
int ak_isp_vo_set_sub_channel_scale(int width, int height)
{
    ...
    //*pReg++ = cmd;
    isp->reg_blkaddr4[49] = cmd;
    
    //pp_reg_51
    cmd = 0;
    cmd = (LOW_BITS(isp->ch2_width, 13) << PP_DST2_WIDTH) 
    	| (LOW_BITS(isp->ch2_height, 13) << pp_DST2_HEIGHT);
    isp->reg_blkaddr4[50] = cmd;
...
int ak_isp_vpp_set_main_chan_mask_area_attr( AK_ISP_MAIN_CHAN_MASK_AREA_ATTR *p_mask)
{
    ...
    //pp_reg_2
    cmd = 0;
    cmd = (LOW_BITS(isp->main_chan_mask_area_para.mask_area[0].start_ypos, 11) << PP_MASK1_S_YPOS)
		| (LOW_BITS(isp->main_chan_mask_area_para.mask_area[0].end_ypos, 11) << PP_MASK1_E_YPOS)
		| (LOW_BITS(isp->main_chan_mask_area_para.mask_area[0].enable, 1) << PP_MASK1_EN);
    ...
    //pp_reg_4
    cmd = 0;
    cmd |= (LOW_BITS(isp->main_chan_mask_area_para.mask_area[1].start_ypos, 11) << PP_MASK2_S_YPOS)
    	|  (LOW_BITS(isp->main_chan_mask_area_para.mask_area[1].end_ypos, 11) << PP_MASK2_E_YPOS)
    	|  (LOW_BITS(isp->main_chan_mask_area_para.mask_area[1].enable, 1) << PP_MASK2_EN) ;
    ...
    cmd |= (LOW_BITS(isp->main_chan_mask_area_para.mask_area[2].start_xpos, 11) << PP_MASK3_S_XPOS)
		|  (LOW_BITS(isp->main_chan_mask_area_para.mask_area[2].end_xpos, 11) << PP_MASK3_E_XPOS);
    isp->reg_blkaddr4[4]=cmd;

    //pp_reg_6
    cmd = 0;
    cmd |= (LOW_BITS(isp->main_chan_mask_area_para.mask_area[2].start_ypos, 11) << PP_MASK3_S_YPOS)
    	|  (LOW_BITS(isp->main_chan_mask_area_para.mask_area[2].end_ypos, 11) << PP_MASK3_E_YPOS)
    	|  (LOW_BITS(isp->main_chan_mask_area_para.mask_area[2].enable, 1) << PP_MASK3_EN);
    ...
    //pp_reg_8
    cmd = 0;
    cmd |=(LOW_BITS(isp->main_chan_mask_area_para.mask_area[3].start_ypos, 11) << PP_MASK4_S_YPOS) 
		| (LOW_BITS(isp->main_chan_mask_area_para.mask_area[3].end_ypos, 11) << PP_MASK4_E_YPOS)
		| (LOW_BITS(isp->main_chan_mask_area_para.mask_area[3].enable, 1) << PP_MASK4_EN);
...
int ak_isp_vpp_set_sub_chan_mask_area_attr(AK_ISP_SUB_CHAN_MASK_AREA_ATTR *p_mask)
{
    ...
    //pp_reg_10
    cmd = 0;
    cmd |= (LOW_BITS(isp->sub_chan_mask_area_para.mask_area[0].start_ypos, 11) << PP_MASK5_S_YPOS)
    	|  (LOW_BITS(isp->sub_chan_mask_area_para.mask_area[0].end_ypos, 11) << PP_MASK5_E_YPOS)
    	|  (LOW_BITS(isp->sub_chan_mask_area_para.mask_area[0].enable, 1) << PP_MASK5_EN);
    ...
    cmd |= (LOW_BITS(isp->sub_chan_mask_area_para.mask_area[1].start_xpos, 11) << PP_MASK6_S_XPOS)
		|  (LOW_BITS(isp->sub_chan_mask_area_para.mask_area[1].end_xpos, 11) << PP_MASK6_E_XPOS);
    isp->reg_blkaddr4[10] = cmd;

	//pp_reg_12
    cmd = 0;
    cmd |= (LOW_BITS(isp->sub_chan_mask_area_para.mask_area[1].start_ypos, 11) << PP_MASK6_S_YPOS)
    	| (LOW_BITS(isp->sub_chan_mask_area_para.mask_area[1].end_ypos, 11) << PP_MASK6_E_YPOS)
    	| (LOW_BITS(isp->sub_chan_mask_area_para.mask_area[1].enable, 1) << PP_MASK6_EN);
    isp->reg_blkaddr4[11] = cmd;
    ...
    isp->reg_blkaddr4[12]=cmd;
    
    //pp_reg_14
    cmd = 0;
    cmd |=(LOW_BITS(isp->sub_chan_mask_area_para.mask_area[2].start_ypos, 11) << PP_MASK7_S_YPOS)
		| (LOW_BITS(isp->sub_chan_mask_area_para.mask_area[2].end_ypos, 11) << PP_MASK7_E_YPOS)
		| (LOW_BITS(isp->sub_chan_mask_area_para.mask_area[2].enable, 1) << PP_MASK7_EN);
    isp->reg_blkaddr4[13] = cmd;
    ...
    isp->reg_blkaddr4[14] = cmd;

    //pp_reg_16
    cmd = 0;
    cmd |= (LOW_BITS(isp->sub_chan_mask_area_para.mask_area[3].start_ypos, 11) << PP_MASK8_S_YPOS)
    	|  (LOW_BITS(isp->sub_chan_mask_area_para.mask_area[3].end_ypos, 11) << PP_MASK8_E_YPOS)
    	|  (LOW_BITS(isp->sub_chan_mask_area_para.mask_area[3].enable, 1) << PP_MASK8_EN);
    isp->reg_blkaddr4[15] = cmd;
...
int ak_isp_vpp_set_mask_area_attr( AK_ISP_MASK_AREA_ATTR *p_mask, MASK_NUM num)
{
    ...
    case MAIN_CHAN_ONE:
		...
		//pp_reg_2
		cmd = 0;
    	cmd |= (LOW_BITS(isp->sub_chan_mask_area_para.mask_area[0].start_ypos, 11) << PP_MASK1_S_YPOS)
    		|  (LOW_BITS(isp->sub_chan_mask_area_para.mask_area[0].end_ypos, 11) << PP_MASK1_E_YPOS)
    		|  (LOW_BITS(isp->sub_chan_mask_area_para.mask_area[0].enable, 1) << PP_MASK1_EN);
...
static unsigned int isp_main_osd_curr_paddr(int osd_chn)
{
	...
	switch (osd_chn) {
		case 0:
			paddr = LOW_BITS(isp->reg_blkaddr4[29], 30);
			break;
		case 1:
			paddr = LOW_BITS(isp->reg_blkaddr4[32], 30);
			break;
		case 2:
			paddr = LOW_BITS(isp->reg_blkaddr4[35], 30);
			break;
		default:
			paddr = 0;
			break;
...
static unsigned int isp_sub_osd_curr_paddr(int osd_chn)
{
	...
	switch (osd_chn) {
		case 0:
			paddr = LOW_BITS(isp->reg_blkaddr4[38], 30);
			break;
		case 1:
			paddr = LOW_BITS(isp->reg_blkaddr4[41], 30);
			break;
		case 2:
			paddr = LOW_BITS(isp->reg_blkaddr4[44], 30);
			break;
		default:
			paddr = 0;
			break;
...
{	
	...
	{
		cmd = REG32(isp->base + 0x14); //lz0499 9_19
		cmd = (cmd>>28)&0x0f;
