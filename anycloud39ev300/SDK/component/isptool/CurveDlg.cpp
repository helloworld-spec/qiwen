...
static void GetCurvePoints(vector<CPoint>* keypts, Point * Points, int level_num)
{
	...
	temp_y = (double)Y_MAX/(double)CURVE_WINDOW_HEIGHT;
	for (i=0; i<n; i++)
	{
...
void CCurveDlg::OnPaint()
{
	...
	if (m_no_key_flag == TRUE)
	{
		...
		for (UINT i = 1; i < m_level_num; i++)
		{
			...
			dc.LineTo(m_CurveFrameRect.left+2+i*step, m_CurveFrameRect.top+2 +(CURVE_WINDOW_WIDTH - gamma_buf[i]* CURVE_WINDOW_HEIGHT / Y_MAX));
		}
...
void CCurveDlg::SetKeyPts(vector<CPoint>* keypts, T_U16 *key, T_U16* curve)
{
	...
	if (0 == cnt || cnt > KEY_POINT_MAX)
	{
		...
		{
			(*keypts).push_back(CPoint(step * i * CURVE_WINDOW_WIDTH / (m_level_num-1), CURVE_WINDOW_HEIGHT - curve[step * i] * CURVE_WINDOW_HEIGHT / Y_MAX));
		}
	...
	else 
	{
		...
		for (i=0; i<KEY_POINT_MAX; i++)
		{
			...
			{
				(*keypts).push_back(CPoint(key[i] * CURVE_WINDOW_WIDTH / (m_level_num-1), CURVE_WINDOW_HEIGHT - curve[key[i]] * CURVE_WINDOW_HEIGHT / Y_MAX));
