...
void CGammaDlg::OnCheckEnable() 
{
	// TODO: Add your control notification handler code here
	m_bGammaEnable = m_GammaCheck.GetCheck();
	m_CurveDlg.SetEnable(m_bGammaEnable);
	m_LargeDlg.SetGammaEnable(m_bGammaEnable);
	if (NULL == m_pMessageWnd) return;
	CBasePage::SendPageMessage(m_pMessageWnd, WM_ENABLE_CHANGE, DIALOG_GAMMA, m_bGammaEnable);
}

int CGammaDlg::SetGammaEnable(BOOL bEnable)
{
	m_GammaCheck.SetCheck(bEnable);
	return 0;
}

void CGammaDlg::OnButtonGet() 
{
	// TODO: Add your control notification handler code here
	if (NULL == m_pMessageWnd) return;
	CBasePage::SendPageMessage(m_pMessageWnd, WM_GET_ISP_INFO, DIALOG_GAMMA, 0);

}

void CGammaDlg::OnButtonSet() 
{
	// TODO: Add your control notification handler code here
	if (NULL == m_pMessageWnd) return;
	CBasePage::SendPageMessage(m_pMessageWnd, WM_SUBMISSION, DIALOG_GAMMA, 0);
}


int CGammaDlg::GetPageInfoSt(int & nPageID, void * pPageInfoSt, int & nStLen)
{
...
int CGammaDlg::SetPageInfoSt(void * pPageInfoSt, int nStLen)
{
	if ((pPageInfoSt == NULL) || (nStLen != sizeof(AK_ISP_INIT_GAMMA))) return -1;
	
	AK_ISP_INIT_GAMMA stGamma = {0};
	ZeroMemory(&stGamma, sizeof(AK_ISP_INIT_GAMMA));
	//ZeroMemory(&stGamma_temp, sizeof(AK_ISP_INIT_GAMMA));

	memcpy(&stGamma, pPageInfoSt, sizeof(AK_ISP_INIT_GAMMA));
	//memcpy(&stGamma_temp, pPageInfoSt, sizeof(AK_ISP_INIT_GAMMA));
...
BOOL CGammaDlg::IsSetApart()
{
	return m_bSetApart;
}


void CGammaDlg::Clean(void) 
{
