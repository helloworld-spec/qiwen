			...
			int *s_count )
{
	...
	*s_count = 0;
	...
	printf("cdh:%s, addr_first:0x%x, addr_last:0x%x\n", __func__, addr_first, addr_last);
	
	/* ak3918.h define CONFIG_SYS_MAX_FLASH_BANKS = 1 */
	...
	for (bank=0,info = &flash_info[0];
		...
		printf("cdh:%s, info->flash_id:0x%x\n", __func__, info->flash_id);
		printf("cdh:%s, info->start[0]:0x%x, info->size:0x%x, b_end:0x%x\n", __func__, info->start[0], info->size, b_end);
		printf("cdh:%s, info->sector_count:0x%x, s_end:0x%x\n", __func__, info->sector_count, s_end);
