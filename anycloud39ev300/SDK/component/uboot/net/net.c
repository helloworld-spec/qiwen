...
static uchar PktBuf[(PKTBUFSRX+1) * PKTSIZE_ALIGN + PKTALIGN]; // cdh:all len+1536*5+64 =7744
...
static void NetInitLoop(void)
{
	...
	if (env_changed_id != env_id) {
		...
		debug_cond(DEBUG_INT_STATE, "cdh:NetInitLoop !=\n"); // cdh:entry ok
	}

	// cdh:get current eth mac address
	if (eth_get_dev()) // cdh:get eth_current net device, /include/net.h
	{
		memcpy(NetOurEther, eth_get_dev()->enetaddr, 6); // cdh:get our mac address
		debug_cond(DEBUG_INT_STATE, "cdh:eth_get_dev\n");  // cdh:entry ok
...
void net_init(void)
{
	...
	if (first_call) {
		...
		NetTxPacket = &PktBuf[0] + (PKTALIGN - 1); // cdh:PKTALIGN=64
		//debug("cdh:NetTxPacket1:0x%x\n", NetTxPacket);
		NetTxPacket -= (ulong)NetTxPacket % PKTALIGN;
		//debug("cdh:NetTxPacket2:0x%x\n", NetTxPacket);
		
		// cdh:PKTBUFSRX=4
		for (i = 0; i < PKTBUFSRX; i++) {
			NetRxPackets[i] = NetTxPacket + (i + 1) * PKTSIZE_ALIGN; // cdh:?????
			//printf("cdh:NetRxPackets[%d]:0x%x\n", i, NetRxPackets[i]);
		}
		
		ArpInit();
		net_clear_handlers();  // cdh:entry ok

		/* Only need to setup buffer pointers once. */
		first_call = 0;
	}

	NetInitLoop();  // cdh:???? i don't know
}
...
int NetLoop(enum proto_t protocol)
{
	...
	// cdh:flag bootstage ethernet start
	bootstage_mark_name(BOOTSTAGE_ID_ETH_START, "eth_start");

	// cdh:initial net
	net_init();

	// cdh:eth_is_on_demand_init function always return 1 (include/net.h)
	if (eth_is_on_demand_init() || protocol != NETCONS) {
		// cdh:stop net 
		eth_halt();
		eth_set_current();
		if (eth_init(bd) < 0) {// cdh:retry initial net mac and phy controller
			eth_halt();
			return -1;
		}
	} else
		eth_init_state_only(bd); // cdh:get net device status, include/net.h

restart:
#ifdef CONFIG_USB_KEYBOARD   // cdh:CONFIG_USB_KEYBOARD=n
	net_busy_flag = 0;
#endif
	net_set_state(NETLOOP_CONTINUE); // cdh: first set netloop status as continue
	...
	// cdh:cmd ping ,net_check_prereq return 0 as ok
	...
	case 0:	// cdh:
		...
		case PING:
			ping_start(); // CDH:from here ,we can entry ping start
			break;
#endif
#if defined(CONFIG_CMD_NFS)
		case NFS:
...
NetSetTimeout(ulong iv, thand_f *f)
{
	if (iv == 0) {
		...
		timeDelta = iv * CONFIG_SYS_HZ / 1000;  // cdh:#define CONFIG_SYS_HZ  		(1000)
	}
}
...
#if defined(CONFIG_CMD_CDP)  // cdh:no
...
#ifdef CONFIG_UDP_CHECKSUM  // cdh: no
		if (ip->udp_xsum != 0) {
			ulong   xsum;
			ushort *sumptr;
			ushort  sumlen;
...
#ifdef CONFIG_NETCONSOLE  // cdh: no
