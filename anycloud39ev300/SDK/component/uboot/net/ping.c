...
static int ping_send(void)
{
	...
	NetArpWaitPacketIP = NetPingIP; // cdh:dst ip addr
...
{
	printf("Using %s device\n", eth_get_name()); // cdh:i don't know how to get ethernet name???
	NetSetTimeout(10000UL, ping_timeout);
//	net_set_arp_handler(nc_wait_arp_handler); // cdh;add
	ping_send();
}
...
void ping_receive(struct ethernet_hdr *et, struct ip_udp_hdr *ip, int len)
{
	...
	switch (icmph->type) {
	case ICMP_ECHO_REPLY: // cdh:ping ok
