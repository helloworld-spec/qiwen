...
#define USED_DMA(x) ((x) >= DMA_ONE_SHOT_LEN && !((x) % DMA_ONE_SHOT_LEN))
...
/**
 * @brief module lock, protect module.
 * 
 * @author Hanyang
 * @modify Lixinhai
 * @date 2011-05-10
...
static void akmci_drv_lock(struct akmci_host *host)
{
	if(host->mci_mode == MCI_MODE_MMC_SD) {
	    ...
	    //do not protect here while do write, coz we already protect in block layer
        if (sd_write_data != 1) {
		    ak_drv_module_protect(DRV_MODULE_SDIO);	    
		}
	}
}
...
static void akmci_drv_unlock(struct akmci_host *host)
{
	if(host->mci_mode == MCI_MODE_MMC_SD) {
        ...
        if (sd_write_data != 1) {
		    ak_drv_module_unprotect(DRV_MODULE_SDIO);
		}
	}
}
...
static void akmci_start_data(struct akmci_host *host, struct mmc_data *data)
{
	...
	if (((data->flags & MMC_DATA_READ) && USED_DMA(host->size)) || 
...
/**
 * @brief  card detect interrupt handle func.
 * 
 * @author Hanyang
 * @date 2011-05-10
 * @param [in] irq number.
