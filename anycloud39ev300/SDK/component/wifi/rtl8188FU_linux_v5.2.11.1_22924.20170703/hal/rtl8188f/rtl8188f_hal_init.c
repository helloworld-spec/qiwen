...
/*
 * rtl8188fu_light_deinit()- prevent some unpredictable reboot flow let FW
 * get stuck in unexpected state when initiate
*/
static u32 rtl8188fu_light_deinit(PADAPTER Adapter)
{
	u8 reg_val = 0;
...
static void rtl8188f_read_chip_version(PADAPTER padapter)
{
	...
	/* deinit before 1st io*/
#ifdef CONFIG_USB_HCI
	rtl8188fu_light_deinit(padapter);
#endif
...
void update_ra_mask_8188f(_adapter *padapter, struct sta_info *psta, struct macid_cfg *h2c_macid_cfg)
{
	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(padapter);

	if (pHalData->fw_ractrl == _TRUE)
		rtl8188f_set_FwMacIdConfig_cmd(padapter,
...
void rtl8188f_set_hal_ops(struct hal_ops *pHalFunc)
{
	...
	pHalFunc->update_ra_mask_handler = update_ra_mask_8188f;
