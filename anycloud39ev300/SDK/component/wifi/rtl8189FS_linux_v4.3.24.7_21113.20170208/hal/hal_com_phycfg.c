...
#ifdef CONFIG_TXPWR_BY_RATE_NOVA_EN
char tx_power_by_rate_table[] = {
"//rtl8188e version = 70\n"
"#[v1][Exact]#\n"
"#[2.4G][A]#\n"
"[1Tx] 0xe08    0x0000ff00         0  0  22 0 // TXAGC codeword (H-byte->L-byte)={1M}\n"
"[1Tx] 0x86c    0xffffff00         20 21 21 0 // TXAGC codeword (H-byte->L-byte)={11M 5.5M 2M}\n"
"[1Tx] 0xe00    0xffffffff         18 19 19 20 // TXAGC codeword (H-byte->L-byte)={18M 12M 9M 6M}\n"
"[1Tx] 0xe04    0xffffffff         14 17 17 18 // TXAGC codeword (H-byte->L-byte)={54M 48M 36M 24M}\n"
"[1Tx] 0xe10    0xffffffff         17 18 18 19 // TXAGC codeword (H-byte->L-byte)=HT_{MCS3 MCS2 MCS1 MCS0}\n"
"[1Tx] 0xe14    0xffffffff         13 16 16 17 // TXAGC codeword (H-byte->L-byte)=HT_{MCS7 MCS6 MCS5 MCS4}\n"
"#[END]#\n"
"0xffff 0xffff"};
#endif
	...
	IN	const char	*pFileName)
{
	...
	if (pHalData->bb_phy_reg_pg == NULL) {
		...
		if (/*rtw_is_file_readable(rtw_phy_para_file_path) == */_TRUE) {
			//rlen = rtw_retrieve_from_file(rtw_phy_para_file_path, pHalData->para_file_buf, MAX_PARA_FILE_BUF_LEN);
			rlen = sizeof(tx_power_by_rate_table);
			_rtw_memcpy(pHalData->para_file_buf, tx_power_by_rate_table, rlen);
#else
		if (rtw_is_file_readable(rtw_phy_para_file_path) == _TRUE) {
			rlen = rtw_retrieve_from_file(rtw_phy_para_file_path, pHalData->para_file_buf, MAX_PARA_FILE_BUF_LEN);
#endif
