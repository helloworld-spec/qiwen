...
)
{
	...
	macid = GET_8188F_C2H_TC2H_APREQ_TXRPT_MACID1(CmdBuf);
	TxOK = GET_8188F_C2H_TC2H_APREQ_TXRPT_TXOK1(CmdBuf);
	TxFail = GET_8188F_C2H_TC2H_APREQ_TXRPT_TXFAIL1(CmdBuf);
	IniRate = GET_8188F_C2H_TC2H_APREQ_TXRPT_INIRATE1(CmdBuf);
...
)
{
	...
	struct sta_info *pbcmc_stainfo = rtw_get_bcmc_stainfo(Adapter);
	_list	*plist, *phead;
	u8 idx = GET_8188F_C2H_SPC_STAT_IDX(CmdBuf);
	...
	psta = pstapriv->c2h_sta;
	psta->sta_stats.tx_retry_cnt = GET_8188F_C2H_SPC_STAT_TYPEB_RETRY1(CmdBuf);
...
void rtl8188f_c2h_packet_handler(PADAPTER padapter, u8 *pbuf, u16 length)
{
	...
	case C2H_AP_REQ_TXRPT:
		C2HTxRPTHandler_8188f(padapter, pdata, C2hEvent.CmdLen);
		break;
	case C2H_SPC_STAT:
		C2HSPC_STAT_8188f(padapter, pdata, C2hEvent.CmdLen);
		break;
...
void rtl8188f_set_hal_ops(struct hal_ops *pHalFunc)
{
    ...
    pHalFunc->reqtxrpt = &rtl8188f_req_txrpt_cmd;
