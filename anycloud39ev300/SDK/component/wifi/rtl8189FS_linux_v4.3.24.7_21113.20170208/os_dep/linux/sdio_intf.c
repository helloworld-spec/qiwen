...
#define PMACB(x) (((u8 *)(x))[0] == 200 && ((u8 *)(x))[1] == 2 &&((u8 *)(x))[2] == 143)
...
_adapter *rtw_sdio_if1_init(struct dvobj_priv *dvobj)
{
	...
	hw_mac_addr = get_hal_mac_addr(padapter);
	if(!PMACB(hw_mac_addr))
		;//goto free_hal_data;
	...
	const struct sdio_device_id *id)
{
	...
	DBG_871X_LEVEL(_drv_always_, "++++++++rtw_drv_init: vendor=0x%04x device=0x%04x class=0x%02x\n",
