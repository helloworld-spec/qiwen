    ...
    , struct sk_buff *ampdu, bool retry)
{
    ...
    do
    {
        ...
        if (!retry)
        {
            // it should check for each mpdu in ampdu
...
static void ssv6200_ampdu_change_retry_frame_rate(struct sk_buff_head *ampdu_skb_retry_queue_p)
{
    ...
    /* 
     * It should fill 3 rate series.
     * The length of ampdu is decided by rate3.
     */
    for (i = 0; i < 3; i++) {   
	    ar = &info->control.rates[i];
        ar->idx = 0;            // MCS0
        ar->count = 4;          // retry count
        ...
        struct sk_buff_head *ampdu_skb_retry_queue_p, bool send_aggr_tx)
{
    ...
    /* 
     * If rate is not MCS rate for the retry frame, it is nessceary to change rate to MCS0.
     * Otherwise, BA windows will keep the retry frame.
     */
    if (max_agg_len == 0) {
        ssv6200_ampdu_change_retry_frame_rate(ampdu_skb_retry_queue_p);
        ampdu_retry_skb = skb_peek(ampdu_skb_retry_queue_p);
        /* 
         * For cabrio, index=15 is MCS0 LGI Len 
         * For turismo, it doesn't need the index
         */
...
void ssv6xxx_ampdu2mpdu(struct ieee80211_hw *hw, struct ieee80211_vif *vif, struct sk_buff *skb)
{
    ...
    //ssv6200_ampdu_tx_stop(tidno, sta, hw);
...
{   
    ...
    // update tx_pkt_run_no
    sc->tx_pkt_run_no ++;
    sc->tx_pkt_run_no &= SSV6XXX_PKT_RUN_TYPE_AMPDU_END;      // only 7 bit is valid , bit 7 for invalid check
	...
	if (find_ampdu_slot != -1) {
        ...
        for (k = 0; k < ampdu_hdr->mpdu_num; k++){
            ssv_sta_priv->ampdu_ssn[find_ampdu_slot].ssn[k] = ampdu_hdr->ssn[k]; //keep mpdu ssn
...
u32 _flush_early_ampdu_q (struct ssv_softc *sc, struct AMPDU_TID_st *ampdu_tid)
{
    ...
    while (skb_queue_len(early_aggr_ampdu_q))
    {
		...
		//release ampdu packet without aggregated mpdu
		if (ampdu_aggr_num == 0) {
...
bool ssv6200_ampdu_tx_handler (struct ieee80211_hw *hw, struct sk_buff *skb)
{
     ...
     * If it can't have aggregation length, the frame need to use mpdu format.
     */
    ...
    //http://192.168.15.14:8080/browse/WSD-209
#if 0
    ...
    /* 
     * When host keeps the less tx frames, it should response the ack directly.
     * This make that kernel sends more tx frames to the host.
     */
    ampdu_tx_frame = atomic_read(&sc->ampdu_tx_frame);
	// p2p mode doesn't support directly ack tx frame
    if ((sc->force_disable_directly_ack_tx != true) &&
        ...
        // 2. report TX completion of this tx skb directly
        ieee80211_tx_status(sc->hw, tx_skb);
        //ieee80211_tx_status_irqsafe(sc->hw, tx_skb);

        // 3. use the copy of tx_skb to do TX
        skb = copy_skb;
...
static int _collect_retry_frames (struct AMPDU_TID_st *ampdu_tid)
{
    ...
    do
    {
        ...
        if (skb == NULL){
            INC_PKT_SN(ssn);	// skip current and check next           
            continue;
        }
		...
		//if exceed the ba windown, mark to DROP, don't enqueue skb to retry queue
...
void ssv6xxx_find_txpktrun_no_from_ssn(struct ssv_softc *sc, u32 ssn,
    struct ssv_sta_priv_data *ssv_sta_priv)
 {
    ...
    for (i = 0; i < MAX_CONCUR_AMPDU; i ++){
        ...
        if (ssv_sta_priv->ampdu_ssn[i].mpdu_num != 0) { // memory slot in use
        ...
        else
        {
            skb_info = (struct SKB_info_st *) (skb->head);
           	...
           	if (found && (sn_bit_map[word_idx] & (1 << bit_idx))) {
                // mark done or retry for sent state only
				if (skb_info->ampdu_tx_status == AMPDU_ST_SENT) { 
...
static u32 _check_timeout (struct AMPDU_TID_st *ampdu_tid, u32 *has_drop)
{
    ...
    do {
        ...
        if (skb == NULL){
            INC_PKT_SN(ssn);	//skip this one and check next one            
            continue;
        }
            
        skb_info = (SKB_info *) (skb->head);
        if (skb_info->ampdu_tx_status != AMPDU_ST_AGGREGATED){
            //   skb_info->aggr_timestamp has not been set yet at AMPDU_ST_AGGREGATED
            cur_jiffies = jiffies;
