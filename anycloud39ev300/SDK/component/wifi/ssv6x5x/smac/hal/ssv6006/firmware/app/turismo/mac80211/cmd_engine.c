...
extern void drv_set_GPIO(u32 value);

static void CmdEngine_InitCali(const struct cfg_host_cmd *HostCmd) {
// Liam ToDo: comment out calibration for temp
//
#if 0
...
static void CmdEngine_Smart_iComm( const struct cfg_host_cmd *HostCmd )
{
    extern int smart_icomm_state;
    extern void resetSmartLink();
    extern void initSmartLink();	
    extern void stopSmartLink();
    extern void channelSwitchInfo(u32 size, const u8 *ch_cfg);
    u32 ch_cfg_size, offset = 0;

    memcpy(&smart_icomm_state, HostCmd->dat8, sizeof(smart_icomm_state));

    offset += sizeof(smart_icomm_state);
    memcpy(&ch_cfg_size, HostCmd->dat8+offset, sizeof(ch_cfg_size));
    offset += sizeof(ch_cfg_size);
    //printf("%s: %d, ch_cfg_size: %d, offset: %d\n", __func__, smart_icomm_state, ch_cfg_size, offset);

    if (smart_icomm_state == START_SMART_ICOMM) {
        channelSwitchInfo(ch_cfg_size, HostCmd->dat8+offset);
        initSmartLink();
    } else if (smart_icomm_state == RESET_SMART_ICOMM) {
        resetSmartLink();
    }
    else {
        stopSmartLink();
...
static void CmdEngine_WSID_OP(const struct cfg_host_cmd *host_cmd) 
{
    printf("It doesn't support cmd [SSV6XXX_HOST_CMD_WSID_OP]\n");
}
...
static void CmdEngine_TxPoll(const struct cfg_host_cmd *HostCmd) 
{
    u32 type = (u32)HostCmd->RSVD0;
    
    switch (type) {
        case SSV6XXX_TX_POLL_START:
            StartTxStuckTimer(); 
            break;
        case SSV6XXX_TX_POLL_RESET:
            ResetTxStuckCnt();
            break;
        case SSV6XXX_TX_POLL_STOP:
            StopTxStuckTimer(); 
            break;
        default:
            printf("%s() Not support the type %d\n", __FUNCTION__, type);
            break;
    }
}

static void CmdEngine_SoftBeacon(const struct cfg_host_cmd *HostCmd) 
{
    u32 type = (u32)HostCmd->RSVD0;
    
    switch (type) {
        case SSV6XXX_SOFT_BEACON_START:
            StartSoftBeaconTimer(); 
            break;
        case SSV6XXX_SOFT_BEACON_STOP:
            StopSoftBeaconTimer(); 
            break;
        default:
            printf("%s() Not support the type %d\n", __FUNCTION__, type);
...
#ifdef CONFIG_SSV_SMARTLINK
    {SSV6XXX_HOST_CMD_SMART_ICOMM,			 CmdEngine_Smart_iComm			  },
#endif //CONFIG_SSV_SMARTLINK
    {SSV6XXX_HOST_CMD_WSID_OP,               CmdEngine_WSID_OP                },
#ifdef CONFIG_P2P_NOA	
	{SSV6XXX_HOST_CMD_SET_NOA,               CmdEngine_SetNoa                 },
#endif
    {SSV6XXX_HOST_CMD_TX_POLL,				 CmdEngine_TxPoll                 },
    {SSV6XXX_HOST_CMD_SOFT_BEACON,           CmdEngine_SoftBeacon             },
};
