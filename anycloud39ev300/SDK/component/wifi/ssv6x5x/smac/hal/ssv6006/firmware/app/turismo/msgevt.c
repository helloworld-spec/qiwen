...
static MsgEvent _msg_event[MBOX_MAX_MSG_EVENT];

s32 msg_evt_init(void)
{
    //u32       index, size;
    MsgEvent *msg_event_iter;
    ...
    sg_msgevt_free_list = _msg_event;
 
    msg_event_iter = _msg_event;
    while (msg_event_iter != &_msg_event[MBOX_MAX_MSG_EVENT - 1]) 
    {
        MsgEvent *next_event = msg_event_iter + 1;
        msg_event_iter->MsgData = (u32)next_event;
        msg_event_iter = next_event;
    }
    msg_event_iter->MsgData = (u32)NULL;

    #if 0
    for(index = 0; index < (MBOX_MAX_MSG_EVENT - 1); index++)
    {
        sg_msgevt_free_list[index].MsgData = (u32)&sg_msgevt_free_list[index+1];
    }
...
s32 msg_evt_post_data1 (OsMsgQ msgevq, msgevt_type msg_type, u32 msg_data, bool bPostFront)
{
    ...
    if (OS_SUCCESS == (  bPostFront 
                       ? msg_evt_post_front(msgevq, msg_ev) 
                       : msg_evt_post(msgevq, msg_ev)))
        return OS_SUCCESS;
