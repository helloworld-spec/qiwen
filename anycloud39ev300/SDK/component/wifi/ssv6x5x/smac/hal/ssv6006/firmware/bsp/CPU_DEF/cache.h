...
static inline unsigned long CACHE_SET(enum cache_t cache){
	...
	if(cache == ICACHE)
		return 64 << ((GET_SYS_ICM_CFG() & ICM_CFG_mskISET) >> ICM_CFG_offISET);
	else
		return 64 << ((GET_SYS_DCM_CFG() & DCM_CFG_mskDSET) >> DCM_CFG_offDSET);
}

static inline unsigned long CACHE_WAY(enum cache_t cache){

	if(cache == ICACHE)
		return 1 + ((GET_SYS_ICM_CFG() & ICM_CFG_mskIWAY) >> ICM_CFG_offIWAY);
	else
		return 1 + ((GET_SYS_DCM_CFG() & DCM_CFG_mskDWAY) >> DCM_CFG_offDWAY);
}

static inline unsigned long CACHE_LINE_SIZE(enum cache_t cache){

	if(cache == ICACHE)
		return 8 << (((GET_SYS_ICM_CFG() & ICM_CFG_mskISZ) >> ICM_CFG_offISZ) - 1);
	else
		return 8 << (((GET_SYS_DCM_CFG() & DCM_CFG_mskDSZ) >> DCM_CFG_offDSZ) - 1);
}
