...
int vsprintf(char *buffer, const char *fmt, va_list args){
	...
	int ret_val = do_printf(fmt, args, vsprintf_help, (void *)buffer, 0);
	buffer[ret_val] = '\0';
...
int vsnprintf(char *buffer, size_t size, const char *fmt, va_list args){
	...
	int ret_val = do_printf(fmt, args, vsprintf_help, (void *)buffer, size);
	if (ret_val < size)
		buffer[ret_val] = '\0';
	else
		buffer[size - 1] = '\0';
...
int snprintf(char *buffer, size_t size, const char *fmt, ...){
	...
	va_start(args, fmt);
	ret_val = vsnprintf(buffer, size, fmt, args);
	va_end(args);
...
int vprintf(const char *fmt, va_list args){
	...
	return do_printf(fmt, args, vprintf_help, (void *)0, 0);
}
