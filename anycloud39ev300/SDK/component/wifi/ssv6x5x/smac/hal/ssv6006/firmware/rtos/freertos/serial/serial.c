...
volatile uint32_t             gOsInFatal = 1;
...
/* Serial driver control functions must be set before using this driver */
static const sSerialDrv *serial_drv = NULL;

void RxISR (void);
void TxISR (void);

/*-----------------------------------------------------------*/
xComPortHandle xSerialPortInitDrv (const sSerialDrv * const drv)
{
	serial_drv = drv;
	return NULL;
} // end of - xSerialPortInitDrv -
...
xComPortHandle xSerialPortInitMinimal( unsigned long ulWantedBaud, unsigned portBASE_TYPE uxQueueLength )
{
	...
	portENTER_CRITICAL();
	{
		...
		serial_drv->init(ulWantedBaud, serBITS_8, serSTOP_1, serNO_PARITY);
		
		/* Register interrupt handler */
		serial_drv->register_tx_int(TxISR);
		serial_drv->register_rx_int(RxISR);
		serial_drv->enable_rx_int();
		//intc_peri_enable(IRQ_PERI_UART0_TX, UART0_TxISR);
		//intc_peri_enable(IRQ_PERI_UART0_RX, UART0_RxISR);
...
signed portBASE_TYPE xSerialGetChar( xComPortHandle pxPort, signed char *pcRxedChar, TickType_t xBlockTime )
{
	...
	if (OSIntNesting || gOsInFatal || (xRxedChars == NULL))
		do {
			int32_t	ret = serial_drv->rx_char();
			if (ret == (-1))
				continue;
...
signed portBASE_TYPE xSerialPutChar( xComPortHandle pxPort, signed char cOutChar, TickType_t xBlockTime )
{
	...
	// In case of serial driver has not been initialized yet or invoked in interrupt.
	if (OSIntNesting || gOsInFatal || xCharsForTx == NULL)
	{
		do {
			if (serial_drv->is_tx_fifo_full())
			...
			if (cOutChar == '\n')
			{
				//drv_uart_tx_0(0x0a);
				//drv_uart_tx_0(0x0d);
				serial_drv->tx_char(0x0a);
				serial_drv->tx_char(0x0d);
			}
			else
				//drv_uart_tx_0(cOutChar);
				serial_drv->tx_char(cOutChar);
			return pdPASS;
		} while (1);
	}
...
void vSerialPutString( xComPortHandle pxPort, const signed char * pcString, unsigned short usStringLength )
{
	...
	if (OSIntNesting || gOsInFatal || xCharsForTx == NULL)
	{
		...
		for (i = 0; i < usStringLength; i++) 
		{
			...
			while (serial_drv->is_tx_fifo_full()) {}
			if (pcString[i] == '\n')
			{
				//drv_uart_tx_0(0x0a);
				//drv_uart_tx_0(0x0d);
	...
	portENTER_CRITICAL();
	{
		...
		unsigned short usFIFOLength = serial_drv->is_tx_fifo_full();
...
signed portBASE_TYPE	xTaskWoken = pdFALSE;
	...
	if( xQueueIsQueueEmptyFromISR(xCharsForTx) == pdTRUE)
	{
		...
		serial_drv->disable_tx_int();
	}
	else
	{	
		signed portBASE_TYPE	_xTaskWoken = pdFALSE;
		//drv_uart_tx_0('T');
		//unsigned portLONG ulNumCharToSend = drv_uart_get_uart_fifo_length_0();
		unsigned portLONG ulNumCharToSend = serial_drv->get_fifo_length();
		while (ulNumCharToSend--) {
			if( xQueueReceiveFromISR(xCharsForTx, &cChar, &_xTaskWoken) == pdTRUE )
			{
				/* There was another character queued - transmit it now. */
				//drv_uart_tx_0(cChar);
				serial_drv->tx_char(cChar);
				if (_xTaskWoken) 
					xTaskWoken = pdTRUE;
			}
			else
