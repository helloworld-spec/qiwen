...
##
printf("static u32 ssv_firmware_version = %d;\n\n", $svn_root_version);
printf("#define SSV_FIRMWARE_URl \"$svn_root_url\"\n");

use Sys::Hostname;
my $host = hostname();
printf("#define FRIMWARE_COMPILERHOST \"$host\"\n");

use POSIX qw(strftime);
my $date = strftime "%m-%d-%Y-%H:%M:%S", localtime;
printf("#define FRIMWARE_COMPILERDATE \"$date\"\n");
##

use Config;
printf("#define FRIMWARE_COMPILEROS \"$Config{osname}\"\n");
printf("#define FRIMWARE_COMPILEROSARCH \"$Config{archname}\"\n");
