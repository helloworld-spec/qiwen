...
static int ssv6006c_chg_xtal_freq_offset(struct ssv_hw *sh)
{
    if (sh->efuse_bitmap & BIT(EFUSE_CRYSTAL_FREQUENCY_OFFSET)) {
        SET_RG_XO_CBANKI(sh->cfg.crystal_frequency_offset);
        SET_RG_XO_CBANKO(sh->cfg.crystal_frequency_offset);
    }

    return 0;
}

static int ssv6006c_chg_bbscale(struct ssv_hw *sh)
{
    int value = 0;
...
#define OFDM_RATE_GAIN_BPSK_OFFSET               (0)  
    ...
    if (sh->efuse_bitmap & BIT(EFUSE_RATE_TABLE_1)) {
        ...
        if (value < SIZE_B_RATE_GAIN_TBL) 
            SET_RG_BB_SCALE_BARKER_CCK(b_rate_gain_tbl[value]);
    }
    ...
    if (sh->efuse_bitmap & BIT(EFUSE_RATE_TABLE_2)) {
        ...
        if (value <= MAX_OFDM_RATE_GAIN_INDEX) {
            SET_RG_BB_SCALE_LEGACY_64QAM(ofdm_rate_gain_tbl[value+OFDM_RATE_GAIN_64QAM_OFFSET]);
            SET_RG_BB_SCALE_LEGACY_16QAM(ofdm_rate_gain_tbl[value+OFDM_RATE_GAIN_16QAM_OFFSET]);
            SET_RG_BB_SCALE_LEGACY_QPSK(ofdm_rate_gain_tbl[value+OFDM_RATE_GAIN_QPSK_OFFSET]);
            SET_RG_BB_SCALE_LEGACY_BPSK(ofdm_rate_gain_tbl[value+OFDM_RATE_GAIN_BPSK_OFFSET]);
        }
    }
    ...
    if (sh->efuse_bitmap & BIT(EFUSE_RATE_TABLE_2)) {
        ...
        if (value < MAX_OFDM_RATE_GAIN_INDEX) {
            SET_RG_BB_SCALE_HT20_64QAM(ofdm_rate_gain_tbl[value+OFDM_RATE_GAIN_64QAM_OFFSET]);
            SET_RG_BB_SCALE_HT20_16QAM(ofdm_rate_gain_tbl[value+OFDM_RATE_GAIN_16QAM_OFFSET]);
            SET_RG_BB_SCALE_HT20_QPSK(ofdm_rate_gain_tbl[value+OFDM_RATE_GAIN_QPSK_OFFSET]);
            SET_RG_BB_SCALE_HT20_BPSK(ofdm_rate_gain_tbl[value+OFDM_RATE_GAIN_BPSK_OFFSET]);
        }
    }
    ...
    if (sh->efuse_bitmap & BIT(EFUSE_RATE_TABLE_1)) {
        ...
        if (value < MAX_OFDM_RATE_GAIN_INDEX) {
            SET_RG_BB_SCALE_HT40_64QAM(ofdm_rate_gain_tbl[value+OFDM_RATE_GAIN_64QAM_OFFSET]);
            SET_RG_BB_SCALE_HT40_16QAM(ofdm_rate_gain_tbl[value+OFDM_RATE_GAIN_16QAM_OFFSET]);
            SET_RG_BB_SCALE_HT40_QPSK(ofdm_rate_gain_tbl[value+OFDM_RATE_GAIN_QPSK_OFFSET]);
            SET_RG_BB_SCALE_HT40_BPSK(ofdm_rate_gain_tbl[value+OFDM_RATE_GAIN_BPSK_OFFSET]);
        }
    }
...
{
    ssv6006c_chg_xtal_freq_offset(sh);
    ssv6006c_chg_bbscale(sh);
