        ...
        struct ssv_minstrel_sta_priv *minstrel_sta_priv, struct ieee80211_tx_rate *ar) 
{
    ...
    for (i = 0; i < sc->hw->max_rates; i++) {
        ...
        ar[i].count = (sh->cfg.rc_retry_set >> i*4) & 0xF; // 4 bit is valid for retry
        switch (sh->cfg.rc_phy_mode) {   //N mode
            case 3: //N mode
                /* update tx aggr */
                minstrel_sta_priv->update_aggr_check = true;
          
                ar[i].idx = (sh->cfg.rc_rate_idx_set >> i* 4) & 0x7; // only 3 bit is valid
            ...
            case 2: //G mode
                if (curchan->band == INDEX_80211_BAND_2GHZ)
                    ar[i].idx = 4 + ((sh->cfg.rc_rate_idx_set >> i* 4) & 0x7); //4 g-rate offset
                else
				...
				void *priv_sta, struct ieee80211_tx_rate_control *txrc)
{
	...
	//if (!mrr && sample && (minstrel_sta_priv->ratelist[ndx].probability > MINSTREL_FRAC(95, 100)))
    ...
    // wait for forbid time to reduce consecutive sampling up fail
    if ((sample)&&
       ((time_before(jiffies,
...
static void *ssv_minstrel_alloc_sta(void *priv, struct ieee80211_sta *sta, gfp_t gfp)
{
	...
	/* setup mapping, sta_priv->rc_info <--> ssv_minstrel_sta_priv*/
...
static void *ssv_minstrel_alloc(struct ieee80211_hw *hw, struct dentry *debugfsdir)
{
	...
	/* 
	 * Set max retry conut and Mutliple Rate Retry 
	 * Refer to the ssv6006 define
	 */
