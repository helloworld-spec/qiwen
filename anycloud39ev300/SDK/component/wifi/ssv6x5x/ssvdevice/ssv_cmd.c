...
static int ssv_cmd_reg(struct ssv_softc *sc, int argc, char *argv[])
{
    ...
    //struct ssv_softc *sc = ssv_dbg_ctrl_hci->shi->sc;
    int ret = 0;
#endif
    struct ssv_cmd_data *cmd_data = &sc->cmd_data;
...
void snprintf_res(struct ssv_cmd_data *cmd_data, const char *fmt, ... )
{
    ...
    int buf_left;
    va_list args;
    //struct ssv_cmd_data *cmd_data = &ssv_dbg_ctrl_hci->shi->sc->cmd_data;
    char *ssv6xxx_result_buf = cmd_data->ssv6xxx_result_buf;


    //cmd_data = &ssv_dbg_ctrl_hci->shi->sc->cmd_data;
...
static int ssv_cmd_iqk (struct ssv_softc *sc, int argc, char *argv[]) {
    ...
    if ((argc == 3) && (strcmp(argv[1], "cfg-pa") == 0)) {
        ...
        // snprintf_res("## GN mode Rx error rate = (%06d/%06d) = %4.4f\n", rxcnt_error, rxcnt_total, rxcnt_error_percet);
        ...
        // snprintf_res("## B Rx error rate = (%06d/%06d) = %4.4f\n", rxcnt_error, rxcnt_total, rxcnt_error_percet);
...
int ssv_cmd_submit(struct ssv_cmd_data *cmd_data, char *cmd)
{
    ...
    if (ssv_dbg_ctrl_hci == NULL){
        printk("SSV device not exist!!\n");
        return 0;         
    }
