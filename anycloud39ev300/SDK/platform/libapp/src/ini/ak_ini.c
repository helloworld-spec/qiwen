...
#define CONFIG_NOTE_SIZE    	256

struct note_info{
	char info[CONFIG_NOTE_SIZE];
...
/**
 * add_new_cfg: add new config info
 * @cfg_path[in]: config path
 * return: new config info pointer; NULL failed
 */
static struct config_info* add_new_cfg(const char *cfg_path)
{
	...
	ak_thread_mutex_init(&p_cfg->lock, NULL);
...
static int cfg_save(const void *handle)
{
	...
	if(p_cfg->update) {
		...
		list_for_each_entry(p_title, &p_cfg->title_head, list) {
			/* write title's notes */
			list_for_each_entry(p_note, &p_title->note_head, list) {
				sprintf(file_buf,"%s\n", p_note->info);
				write_file(file_buf, file_id);
				size += strlen(p_note->info) + 1;
			}
			...
			/* traverse title's item */
			list_for_each_entry(p_item, &p_title->item_head, list) {
				/* write item's notes */
				list_for_each_entry(p_note, &p_item->note_head, list) {
					sprintf(file_buf,"%s\n", p_note->info);
					write_file(file_buf, file_id);
					size += strlen(p_note->info) + 1;
				}
...
static int cfg_destroy(struct config_info *p_cfg)
{
	int ret = 0;
	struct title_info *title_entry = NULL;
	...
	struct note_info *note_entry = NULL;
	struct note_info *note_ptr = NULL;

	if(p_cfg->update) {
		ak_print_notice_ex("updating config before destroy\n");
		ret = cfg_save((const void *)p_cfg);
	}

	ak_thread_mutex_lock(&p_cfg->lock);
	list_for_each_entry_safe(title_entry, title_ptr, &p_cfg->title_head, list){
		/* delete items of the title */
		list_for_each_entry_safe(item_entry, item_ptr,
			&title_entry->item_head, list){
			list_for_each_entry_safe(note_entry, note_ptr,
				&item_entry->note_head, list){
				list_del(&note_entry->list);
				free(note_entry);
			}
...
void* ak_ini_init(const char *file)
{
	...
	if(0 == init_flg){
		ak_thread_mutex_init(&g_config_lock, NULL);
	...
	while(!file_eof(file_id)){
        ...
        if(';' == line_data[index]){
			if (3 == note_cnt) {
				ak_print_normal_ex("notes more than 3 lines!\n");
				continue;
			}

			p_note[note_cnt] = (struct note_info *)calloc(1, sizeof(struct note_info));
			if(NULL == p_note[note_cnt]){
				goto anyka_config_init_err_tbl;
			}

            strcpy(p_note[note_cnt]->info, &line_data[index]);
			note_cnt++;
			continue;
		}
        ...
        if('[' == line_data[index]){
			...
			INIT_LIST_HEAD(&p_title->note_head);

			if (note_cnt) {
				for (i=0; i<note_cnt; i++)
					list_add_tail( &p_note[i]->list, &p_title->note_head);
		...
		*find = 0;
		...
		INIT_LIST_HEAD(&p_item->note_head);

		if (note_cnt) {
			for (i=0; i<note_cnt; i++)
				list_add_tail(&p_note[i]->list, &p_item->note_head);
...
anyka_config_init_err_tbl:
	...
	list_for_each_entry_safe(p_title, p_title2, &p_cfg->title_head, list){
		list_for_each_entry_safe(p_item, p_item2, &p_title->item_head, list){
			list_for_each_entry_safe(note_entry, note_ptr, &p_item->note_head, list){
				list_del(&note_entry->list);
				free(note_entry);
			}
		...
		list_for_each_entry_safe(note_entry, note_ptr, &p_title->note_head, list){
			list_del(&note_entry->list);
			free(note_entry);
		}
		...
		const char *item, const char *value)
{
	...
	ak_thread_mutex_lock(&p_cfg->lock);
	list_for_each_entry(p_title, &p_cfg->title_head, list){
		/* match the appointed title */
		if(strcmp(title_name, p_title->name) == 0){
			list_for_each_entry(p_item, &p_title->item_head, list){
				/* match the appointed item */
...
int ak_ini_del_item(const void *handle, const char *title, const char *item)
{
	...
	/* match and delete the appointed item */
	ak_thread_mutex_lock(&p_cfg->lock);
	list_for_each_entry_safe(title_entry, title_ptr, &p_cfg->title_head, list){
...
int ak_ini_del_title(const void *handle, const char *title)
{
	...
	/* match and delete the appointed title */
	ak_thread_mutex_lock(&p_cfg->lock);
	list_for_each_entry_safe(title_entry, title_ptr, &p_cfg->title_head, list){
		if(strcmp(title_name, title_entry->name) == 0){
			list_for_each_entry_safe(item_entry, item_ptr,
