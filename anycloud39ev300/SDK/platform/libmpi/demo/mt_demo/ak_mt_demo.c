...
static void* mt_ctrl_thread(void* param)
{
	...
	while (mt_ctrl.run_flag) {
        ...
        box_attr.left = (rect.pt.px - 1) * vid_width / MD_SIZE;
        box_attr.top = (rect.pt.py - 1) * vid_height / md_v_size;
        box_attr.width= rect.width * vid_width / MD_SIZE;
        box_attr.height = rect.height * vid_height / md_v_size;
		ak_vi_put_rect(mt_ctrl.vi_handle,VIDEO_CHN_MAIN,&box_attr,0xff0000);
