...
typedef struct
{
    ...
    T_S8                       m_bNeedSYN;  //synchronize flag：0,1,2
    ...
    T_U32                   m_ulTimeScale;  //time scale,时间尺度(它的倒数表示传入的时间戳的单位).例如m_ulTimeScale=1000,表示时间单位为(1/1000)s,即1ms
    ...
    T_U16   m_nAudioPacksPerChunk;          //对于MP4文件，设置每个chunk的音频帧数.这个变量觉决定连续存储的音频数据长度
...
typedef struct
{
    T_pDATA m_pStreamBuf;    //isKeyframs(4bytes)+TimeStamp(4bytes)+ videoStream
    T_U32   m_ulStreamLen;
    T_U32   m_ulTimeStamp;
 ...
 * @return T_S8*
 */
T_S8* MediaLib_Mux_GetVersionInfo(T_VOID);
...
/**
 * @brief repair AVI file, such as apending idx1 information and filesize etc.
 *
 * @author Tang_Xuechai
 * @param   avifile     [in]    input defective avi file
 * @param   idxfile     [in]    input index file. "0" is permitted and suggested
 * @param   outfile     [in]    output perfect avi file. When outfile=avifile, avifile should be opened by "_O_RDWR | _O_BINARY"
 * @param   m_pCBFunc   [in]    call back functions
 * @return T_BOOL
 * @retval  AK_FALSE    process fail
 * @retval  AK_TRUE   process sucess
 */
T_BOOL AVI_FilePerfect(T_U32 avifile, T_U32 idxfile, T_U32 outfile, T_MEDIALIB_CB *m_pCBFunc);

/**
 * @brief repair MP4 file
 *
 * @author WangChi
 * @param   mp4file     [in]    input defective MP4 file
 * @param   moovfile    [in]    input moov file
 * @param   stblfile    [in]    input stbl file
 * @param   outfile     [in]    output perfect MP4 file. When outfile=mp4file, mp4file should be opened by "_O_RDWR | _O_BINARY"
 * @param   m_pCBFunc   [in]    call back functions
 * @return T_BOOL
 * @retval  AK_FALSE    process fail
 * @retval  AK_TRUE   process sucess
 */
T_BOOL MP4_FilePerfect(T_S32 mp4file, T_S32 moovfile, T_S32 stblfile, T_S32 outfile, T_MEDIALIB_CB *m_pCBFunc);
#ifdef __cplusplus
}
#endif
