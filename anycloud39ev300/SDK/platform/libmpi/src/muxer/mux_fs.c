...
/*
 * find_node_index - find the corresponding index by file-stream pointer
 * fp[IN]: pointer to index file
 * return: correct index on success, -1 failed.
 */
...
/** stat_mux_write - stat current mux file info
 * @write_len[IN]: current write len
...
static long write_data_to_file(long mux_fp, const unsigned char *data, long size)
{
	...
	while((cur_size > 0)) {
		/* write file and calculate used time */
		...
		/* hint writen for a long time */
        if(tick > 300) {
			ak_print_normal("write too long %ld(ms) sz=%d\n", tick, write_size);
        }
...
void mux_fs_init(FILE *record_fp, FILE *index_fp)
{
	...
	if(!mux_fs.run_flag) {
		...
		mux_fs.write_err = 0;
    	ak_thread_mutex_init(&(mux_fs.mutex), NULL);
	}
